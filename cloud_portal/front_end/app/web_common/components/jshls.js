...
(function () {
    ...
    window.JsHlsAPI = function () {
        ...
        this.initHlsEvents = function (hls) {
            var jshlsApi = this;
            hls.on(Hls.default.Events.MEDIA_ATTACHED, function () {
                events.video.push({time: performance.now() - events.t0, type: 'Media attached'});
            });
            hls.on(Hls.default.Events.MEDIA_DETACHED, function () {
                events.video.push({time: performance.now() - events.t0, type: 'Media detached'});
            });
            hls.on(Hls.default.Events.FRAG_PARSING_INIT_SEGMENT, function (event, data) {
                var nxevent = {time: performance.now() - events.t0, type: data.id + ' init segment'};
                events.video.push(nxevent);
            });
            hls.on(Hls.default.Events.LEVEL_SWITCHING, function (event, data) {
                events.level.push({
            ...
            hls.on(Hls.default.Events.MANIFEST_PARSED, function (event, data) {
                var nxevent = {
                    type: 'manifest',
                    name: '',
                    start: 0,
                ...
                events.load.push(nxevent);
            });
            hls.on(Hls.default.Events.MANIFEST_PARSED, function (event, data) {
                stats = {levelNb: data.levels.length, levelParsed: 0};
            });
            hls.on(Hls.default.Events.AUDIO_TRACK_SWITCHING, function (event, data) {
                var nxevent = {time: performance.now() - events.t0, type: 'audio switching', name: '@' + data.id};
                events.video.push(nxevent);
                lastAudioTrackSwitchingIdx = events.video.length - 1;
            });
            hls.on(Hls.default.Events.AUDIO_TRACK_SWITCHED, function (event, data) {
                var nxevent = {time: performance.now() - events.t0, type: 'audio switched', name: '@' + data.id};
                if (lastAudioTrackSwitchingIdx !== undefined) {
                    events.video[lastAudioTrackSwitchingIdx].duration = nxevent.time - events.video[lastAudioTrackSwitchingIdx].time;
                    lastAudioTrackSwitchingIdx = undefined;
                }
                events.video.push(nxevent);
            });
            hls.on(Hls.default.Events.LEVEL_LOADED, function (event, data) {
            ...
            hls.on(Hls.default.Events.AUDIO_TRACK_LOADED, function (event, data) {
                events.isLive = data.details.live;
                var nxevent = {
                    type: 'audio track',
                    id: data.id,
            ...
            hls.on(Hls.default.Events.FRAG_BUFFERED, function (event, data) {
                var nxevent = {
            ...
            hls.on(Hls.default.Events.LEVEL_SWITCHED, function (event, data) {
                var nxevent = {time: performance.now() - events.t0, type: 'level switched', name: data.level};
                events.video.push(nxevent);
            });
            
            hls.on(Hls.default.Events.BUFFER_CREATED, function (event, data) {
                // TODO: do we need this? --TT
                // tracks = data.tracks;
            });
            hls.on(Hls.default.Events.FRAG_CHANGED, function (event, data) {
                var nxevent = {
                    time: performance.now() - events.t0,
                    type: 'frag changed',
            ...
            hls.on(Hls.default.Events.FRAG_LOAD_EMERGENCY_ABORTED, function (event, data) {
                if (stats) {
            ...
            hls.on(Hls.default.Events.FRAG_DECRYPTED, function (event, data) {
            ...
            hls.on(Hls.default.Events.ERROR, function (event, data) {
                ...
                switch (data.details) {
                    case Hls.default.ErrorDetails.MANIFEST_LOAD_ERROR:
                        try {
                        ...
                        break;
                    case Hls.default.ErrorDetails.MANIFEST_LOAD_TIMEOUT:
                        console.log('timeout while loading manifest');
                        break;
                    case Hls.default.ErrorDetails.MANIFEST_PARSING_ERROR:
                        console.log('error while parsing manifest:' + data.reason);
                        break;
                    case Hls.default.ErrorDetails.LEVEL_LOAD_ERROR:
                        console.log('error while loading level playlist');
                        break;
                    case Hls.default.ErrorDetails.LEVEL_LOAD_TIMEOUT:
                        console.log('timeout while loading level playlist');
                        break;
                    case Hls.default.ErrorDetails.LEVEL_SWITCH_ERROR:
                        console.log('error while trying to switch to level ' + data.level);
                        break;
                    case Hls.default.ErrorDetails.FRAG_LOAD_ERROR:
                        console.log('error while loading fragment ' + data.frag.url);
                        break;
                    case Hls.default.ErrorDetails.FRAG_LOAD_TIMEOUT:
                        console.log('timeout while loading fragment ' + data.frag.url);
                        break;
                    case Hls.default.ErrorDetails.FRAG_LOOP_LOADING_ERROR:
                        console.log('Frag Loop Loading Error');
                        break;
                    case Hls.default.ErrorDetails.FRAG_DECRYPT_ERROR:
                        console.log('Decrypting Error:' + data.reason);
                        break;
                    case Hls.default.ErrorDetails.FRAG_PARSING_ERROR:
                        console.log('Parsing Error:' + data.reason);
                        break;
                    case Hls.default.ErrorDetails.KEY_LOAD_ERROR:
                        console.log('error while loading key ' + data.frag.decryptdata.uri);
                        break;
                    case Hls.default.ErrorDetails.KEY_LOAD_TIMEOUT:
                        console.log('timeout while loading key ' + data.frag.decryptdata.uri);
                        break;
                    case Hls.default.ErrorDetails.BUFFER_APPEND_ERROR:
                        console.log('Buffer Append Error');
                        break;
                    case Hls.default.ErrorDetails.BUFFER_ADD_CODEC_ERROR:
            ...
            hls.on(Hls.default.Events.BUFFER_APPENDING, function (event, data) {
                if (dumpfMP4) {
                    fmp4Data[data.type].push(data.data);
                }
            });
            hls.on(Hls.default.Events.FPS_DROP, function (event, data) {
                var evt = {
                    time: performance.now() - events.t0,
                    type: 'frame drop',
        ...
        this.handleVideoEvent = function (evt) {
            ...
            var nxevent = {time: performance.now() - evt.t0, type: evt.type, name: data};
            events.video.push(nxevent);
            if (evt.type === 'seeking') {
                lastSeekingIdx = events.video.length - 1;
            }
            if (evt.type === 'seeked') {
                events.video[lastSeekingIdx].duration = nxevent.time - events.video[lastSeekingIdx].time;
            }
        };
    ...
    window.JsHlsAPI.prototype.kill = function () {
        this.hls.destroy();
    };
    
    window.JsHlsAPI.prototype.play = function (offset) {
    ...
    window.JsHlsAPI.prototype.pause = function () {
        this.video.pause();
    };
    
    window.JsHlsAPI.prototype.volume = function (volumeLevel) {
        this.video.volume = volumeLevel / 100;
    };
    
    window.JsHlsAPI.prototype.load = function (url) {
    ...
    window.JsHlsAPI.prototype.initVideoHandlers = function () {
