...
class MessageAdmin(admin.ModelAdmin):
    ...
    def clean_old_messages(self, request, queryset):
        ...
        cutoff_date = datetime.now() - timedelta(days=settings.CLEAR_OLDER_THAN_X_DAYS)
        Message.objects.filter(send_date__lt=cutoff_date).delete()

    clean_old_messages.short_description = "Remove messages older than {} days".format(settings.CLEAR_OLDER_THAN_X_DAYS)
...
class TaskResultAdmin(admin.ModelAdmin):
    ...
    clean_old_tasks.short_description = "Remove tasks older than {} days".format(settings.CLEAR_OLDER_THAN_X_DAYS)
