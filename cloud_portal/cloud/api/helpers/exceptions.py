...
class APIException(Exception):
    ...
    def __init__(self, error_code, error_text, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR):
...
class APIInternalException(APIException):
    ...
    def __init__(self, error_code, error_text):
        super(APIException, self).__init__(error_code, error_text, status_code=status.HTTP_500_INTERNAL_SERVER_ERROR)


class APIServiceException(APIException):
    # 503 error - service unavailable
    def __init__(self, error_code, error_text):
        super(APIException, self).__init__(error_code, error_text, status_code=status.HTTP_503_SERVICE_UNAVAILABLE)


class APINotFoundException(APIException):
    # 404 error - service unavailable
    def __init__(self, error_code, error_text):
        super(APIException, self).__init__(error_code, error_text, status_code=status.HTTP_404_NOT_FOUND)


class APINotAuthorisedException(APIException):
    # 401 error - service unavailable
    def __init__(self, error_code, error_text):
        super(APIException, self).__init__(error_code, error_text, status_code=401)


class APIRequestException(APIException):
    # 400 error - bad request
    def __init__(self, error_code, error_text):
        super(APIException, self).__init__(error_code, error_text, status_code=400)


class APILogicException(APIException):
    # 200. but some error occurred
    def __init__(self, error_code, error_text):
        super(APIException, self).__init__(error_code, error_text, status_code=200)


def validate_response(func):
    def validate_error(response_data):
        if 'resultCode' not in response_data or 'errorText' not in response_data:
                raise APIInternalException(status.HTTP_500_INTERNAL_SERVER_ERROR,
                                           'No valid error message from cloud_db')
    ...
    def validator(*args, **kwargs):
        ...
        try:
            ...
            raise APIInternalException(status.HTTP_500_INTERNAL_SERVER_ERROR, 'No JSON data from cloud_db')

        # 2. Check 500 status  - raise APIInternalException
        if response.status_code == status.HTTP_500_INTERNAL_SERVER_ERROR:
            validate_error(response_data)
            raise APIInternalException(response_data['resultCode'], response_data['errorText'])

        # 3. Check 503 status - raise APIServiceException
        if response.status_code == status.HTTP_503_SERVICE_UNAVAILABLE:
            validate_error(response_data)
            raise APIServiceException(response_data['resultCode'], response_data['errorText'])

        # 4. Check 404 status - raise APINotFoundException
        if response.status_code == status.HTTP_404_NOT_FOUND:
            validate_error(response_data)
            raise APINotFoundException(response_data['resultCode'], response_data['errorText'])

        # 5. Check 401 status - raise APINotAuthorisedException
        if response.status_code == status.HTTP_401_UNAUTHORIZED:
            validate_error(response_data)
            raise APINotAuthorisedException(response_data['resultCode'], response_data['errorText'])

        # 6. Check 400 status - raise APIRequestException
        if response.status_code == status.HTTP_400_BAD_REQUEST:
            validate_error(response_data)
            raise APIRequestException(response_data['resultCode'], response_data['errorText'])

        # 7. Check error_code status - raise APILogicException
        if 'resultCode' in response_data and response_data['resultCode'] != 0:
            validate_error(response_data)
            raise APILogicException(response_data['resultCode'], response_data['errorText'])
    ...
    def handler(*args, **kwargs):
        ...
        try:
            ...
            logger.error('APIException. \nStatus: ' + error.status_code +
                         ' \nResponse: \n' + json.dumps(error.response, indent=4, separators=(',', ': ')) +
