...
class Impl: public DetectionPlugin
{
public:
    ...
    Rect outRects[], int maxRectsCount, int* outRectsCount, int64_t* outPtsUs)
{
    ...
    *outRectsCount = (int) rectsFromGie.size();
    ...
    for (int i = 0; i < rectsFromGie.size(); ++i)
    {
        ...
        DetectionPlugin::Rect& r = outRects[i];
...
DetectionPlugin* DetectionPlugin::createImpl(const Params& params)
{
    ini().reload();
