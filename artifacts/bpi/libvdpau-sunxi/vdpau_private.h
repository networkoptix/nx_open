...
// TODO: #mshevchenko Consider moving to nx/debug/utils_c.h
#if defined(DEBUG)
	#include <stdio.h>

	#define VDPAU_DBG(format, ...) fprintf(stderr, "[VDPAU SUNXI] " format "\n", ##__VA_ARGS__)

	#define VDPAU_DBG_ONCE(format, ...) do \
    { \
        static uint8_t VDPAU_DBG_once = 0; \
        if (!VDPAU_DBG_once) \
        { \
            fprintf(stderr, "[VDPAU SUNXI] " format "\n", ##__VA_ARGS__); \
            VDPAU_DBG_once = 1; \
        } \
    } while(0)

	// Log execution of a line - to use, put double-L at the beginning of the line.
	#define LL fprintf(stderr, "####### line %d, file %s\n", __LINE__, __FILE__);
#else // DEBUG
