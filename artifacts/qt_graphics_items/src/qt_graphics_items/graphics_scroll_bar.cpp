...
QStyle::SubControl GraphicsScrollBarPrivate::newHoverControl(const QPointF &pos)
{
    ...
    hoverControl = q->style()->hitTestComplexControl(QStyle::CC_ScrollBar, &opt, pos.toPoint(), nullptr);
    if (hoverControl == QStyle::SC_None)
        hoverRect = QRect();
    else
        hoverRect = q->style()->subControlRect(QStyle::CC_ScrollBar, &opt, hoverControl, nullptr);
    return hoverControl;
}
...
void GraphicsScrollBarPrivate::stopRepeatAction()
{
    ...
    QStyleOptionSlider opt;
    q->initStyleOption(&opt);
    q->update(q->style()->subControlRect(QStyle::CC_ScrollBar, &opt, tmp, nullptr));
}
...
{
    if (!style()->styleHint(QStyle::SH_ScrollBar_ContextMenu, 0, nullptr)) {
...
QSizeF GraphicsScrollBar::sizeHint(Qt::SizeHint which, const QSizeF &constraint) const {
    ...
    int scrollBarExtent = style()->pixelMetric(QStyle::PM_ScrollBarExtent, &opt, nullptr);
    int scrollBarSliderMin = style()->pixelMetric(QStyle::PM_ScrollBarSliderMin, &opt, nullptr);
    QSize size;
    ...
    return style()->sizeFromContents(QStyle::CT_ScrollBar, &opt, size, nullptr);
}
...
void GraphicsScrollBar::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *widget)
{
    ...
    style()->drawComplexControl(QStyle::CC_ScrollBar, &opt, painter, nullptr);
}
...
void GraphicsScrollBar::mousePressEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    bool midButtonAbsPos = style()->styleHint(QStyle::SH_ScrollBar_MiddleClickAbsolutePosition, 0, nullptr);
    QStyleOptionSlider opt;
    initStyleOption(&opt);

    if (d->maximum == d->minimum // no range
        || (event->buttons() & (~event->button())) // another button was clicked before
        || !(event->button() == Qt::LeftButton || (midButtonAbsPos && event->button() == Qt::MiddleButton)))
        return;

    d->pressedControl = style()->hitTestComplexControl(QStyle::CC_ScrollBar, &opt, event->pos().toPoint(), nullptr);
    ...
    if ((d->pressedControl == QStyle::SC_ScrollBarAddPage
        ...
        && ((midButtonAbsPos && event->button() == Qt::MiddleButton)
            || (style()->styleHint(QStyle::SH_ScrollBar_LeftClickAbsolutePosition, &opt, nullptr)
    ...
    update(style()->subControlRect(QStyle::CC_ScrollBar, &opt, d->pressedControl, nullptr));
...
void GraphicsScrollBar::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    ...
    if (d->pressedControl == QStyle::SC_ScrollBarSlider) {
    ...
    } else if (!style()->styleHint(QStyle::SH_ScrollBar_ScrollWhenPointerLeavesControl, &opt, nullptr)) {
        if (style()->styleHint(QStyle::SH_ScrollBar_RollBetweenButtons, &opt, nullptr)
                && d->pressedControl & (QStyle::SC_ScrollBarAddLine | QStyle::SC_ScrollBarSubLine)) {
            QStyle::SubControl newSc = style()->hitTestComplexControl(QStyle::CC_ScrollBar, &opt, event->pos().toPoint(), nullptr);
            ...
            if (newSc & (QStyle::SC_ScrollBarAddLine | QStyle::SC_ScrollBarSubLine)) {
                d->pointerOutsidePressedControl = false;
                QRect scRect = style()->subControlRect(QStyle::CC_ScrollBar, &opt, newSc, nullptr);
                scRect |= style()->subControlRect(QStyle::CC_ScrollBar, &opt, d->pressedControl, nullptr);
        ...
        QRectF pressedRect = style()->subControlRect(QStyle::CC_ScrollBar, &opt, d->pressedControl, nullptr);
