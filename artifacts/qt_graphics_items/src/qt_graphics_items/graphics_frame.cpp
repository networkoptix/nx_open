...
void GraphicsFramePrivate::updateStyledFrameWidths()
{
    ...
    const QRect cr = q->style()->subElementRect(QStyle::SE_ShapedFrameContents, &opt, nullptr);
...
GraphicsFrame::GraphicsFrame(QGraphicsItem* parent, Qt::WindowFlags flags):
    GraphicsWidget(*new GraphicsFramePrivate(), parent, flags)
{
}

GraphicsFrame::GraphicsFrame(
    GraphicsFramePrivate& dd, QGraphicsItem* parent, Qt::WindowFlags flags)
    :
    GraphicsWidget(dd, parent, flags)
{
}
...
void GraphicsFrame::paint(QPainter *painter, const QStyleOptionGraphicsItem *, QWidget *)
{
    ...
    style()->drawControl(QStyle::CE_ShapedFrame, &opt, painter, nullptr);
}
...
void GraphicsFrame::initStyleOption(QStyleOption *option) const
{
    ...
    if (QStyleOptionFrame *frameOption = qstyleoption_cast<QStyleOptionFrame *>(option)) {
        ...
        if (QStyleOptionFrame *frameOptionV3 = qstyleoption_cast<QStyleOptionFrame *>(frameOption))
