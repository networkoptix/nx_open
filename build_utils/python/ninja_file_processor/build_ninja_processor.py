...
BuildLineData = namedtuple(
    'BuildLineData',
    [
        'outputs', 'implicit_outputs',
        'command', 'dependencies', 'implicit_dependencies', 'order_only_dependencies',
    ]
)
...
class BuildNinjaFileProcessor(NinjaFileProcessor):
    ...
    def _consume_line_object(self, line_object: Line) -> None:
        if line_object.type == LineType.BUILD:
            ...
            current_outputs = {*line_object.parsed.outputs, *line_object.parsed.implicit_outputs}
    ...
    def _collect_transitive_dependencies(self, target: str) -> set:
        ...
        State = namedtuple('State', 'output_data dependencies')
        ...
        state_stack = []
        current_target_data = target_line.parsed
        current_dependencies = {
            *current_target_data.dependencies,
            *current_target_data.implicit_dependencies,
            *current_target_data.order_only_dependencies}
        ...
        while True:
            for dependency in current_dependencies:
                ...
                target_line = self._get_target_line_by_name(dependency)
                if target_line is not None:
                    break  # Out of the current_dependencies loop.
            else:
                if not state_stack:
                    break # Out of the main loop.

                current_state = state_stack.pop()
                current_target_data = current_state.output_data
                current_dependencies = current_state.dependencies
                continue

            # "dependency" is always defined since the empty "current_dependencies" case is
            # processed in the "else" block above.
            transitive_dependencies.add(dependency)  # pylint:disable=undefined-loop-variable

            state_stack.append(
                State(output_data=current_target_data, dependencies=current_dependencies))
            current_target_data = target_line.parsed
            current_dependencies = {
                *current_target_data.dependencies,
                *current_target_data.implicit_dependencies,
        ...
        # Don't need explicit dependecies in the result.
        transitive_dependencies.difference_update(current_target_data.dependencies)

        # Remove phony targets from the dependencies.
    ...
    def _replace_target(self,
        ...
        if dependencies is not None:
            target_line_data = target_line_data._replace(dependencies=dependencies)

        if implicit_dependencies is not None:
            target_line_data = target_line_data._replace(
                implicit_dependencies=implicit_dependencies)
