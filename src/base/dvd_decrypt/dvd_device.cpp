 ...
 * $Id: device.c 229 2009-09-02 22:01:07Z sam $
 *
...
static int aspi_open( dvdcss_t dvdcss, char const * psz_device )
{
    ...
    fd = (w32_aspidev*) malloc( sizeof( struct w32_aspidev ) );
    if( fd == NULL )
    {
...
static int libc_seek( dvdcss_t dvdcss, int i_blocks )
{
    ...
    i_seek = lseek_func( dvdcss->i_read_fd, i_seek, SEEK_SET );
...
static int aspi_read_internal( int i_fd, void *p_data, int i_blocks )
{
    ...
    ssc.SRB_PostProc = (unsigned long*) hEvent;
    ...
    if( i_blocks > 32 )
    {
        ...
        if( aspi_read_internal( i_fd,
                                (quint8*) p_data + 32 * DVDCSS_BLOCK_SIZE,
                                i_blocks - 32) < 0 )
        {
            return -1;
        }
