...
QDir applicationRootDir(const QDir& binDir)
{
    ...
    const int stepsUp = QnClientInstallationsManager::binDirSuffix()
        .split(L'/', Qt::SkipEmptyParts).size();
    ...
    m_clientVersion(nx::build_info::vmsVersion())
{
    ...
    // TODO: #sivanov #dklychkov Move function calls inside osCheck() method.

    auto processOperation =
        [this, &results](Operation operation, bool result)
        {
            results.append(osCheck(operation, result));
        };
    ...
    if (!isDeveloperBuildOnWindows())
    {
        ...
        processOperation(Operation::registerUriHandler, uriHandlerUpdateResult.success);

        processOperation(Operation::updateApplauncher, updateApplauncher());
        ...
        if (updateHelpFilesNeeded)
        {
            ...
            {
                processOperation(Operation::updateHelpFile, updateHelpFile(helpFileDescription));
            }
        }
    }
    ...
    {
        processOperation(Operation::updateMinilauncher,
            updateMinilauncherOnWindows(hasAdminRights));

        if (results.contains(Result::insufficientPermissions))
    ...
    const QString& sourceMinilauncherPath)
{
    ...
    {
        NX_ERROR(this, "Version could not be written to file: %1.", versionFile);
        NX_ERROR(this, "Failed to update Minilauncher.");
        return false;
    }
