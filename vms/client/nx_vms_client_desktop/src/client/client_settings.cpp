    ...
    switch(id)
    {
        ...
        case SERVER_CERTIFICATE_VALIDATION_LEVEL:
        {
            ...
            const auto defaultLevel =
                defaultValue.value<nx::vms::client::core::ServerCertificateValidationLevel>();
    ...
    switch(id)
    {
        case LAST_USED_CONNECTION:
        {
            QString asJson = QString::fromStdString(
                nx::reflect::json::serialize(value.value<ConnectionData>()));
        ...
        {
            const auto& level =
                value.value<nx::vms::client::core::ServerCertificateValidationLevel>();
            base_type::writeValueToSettings(
                settings, id, QString::fromStdString(nx::reflect::toString(level)));
            break;
        }
