    ...
    QAbstractItemModel* model, const QModelIndex& index, QWidget* parent) const
{
    ...
    switch (index.column())
    {
        ...
        case QnIOPortsViewModel::DurationColumn:
        {
            ...
            spinBox->setSuffix(' ' + QnTimeStrings::suffix(QnTimeStrings::Suffix::Seconds));
...
QSize QnIoPortItemDelegate::sizeHint(QWidget* widget, const QModelIndex& index) const
{
    ...
    size.setWidth(lineEdit->fontMetrics().horizontalAdvance(lineEdit->text())
        + (nx::style::Metrics::kStandardPadding + kFrameWidth) * 2);
