...
bool QnDwmPrivate::isSupported() {
    ...
    if(!result)
        NX_ASSERT(false, "Compile-time and run-time Qt versions differ (%1 != %2), DWM will be disabled.", qVersion(), QT_VERSION_STR);
#else
    result = false;
#endif // QN_HAS_DWM
...
void QnDwmPrivate::init(QWidget *widget) {
    this->widget = widget;
    hasApi = widget != nullptr && qn_dwm_isSupported();
    ...
    hasDwm =
        ...
        dwmGetColorizationColor != nullptr &&
...
void QnDwmPrivate::updateFrameStrut() {
    QWidgetPrivate *widgetPointer = qt_widget_private(widget);
    QTLWExtra *tlwExtra = widgetPointer->maybeTopData();
    if(tlwExtra != nullptr) {
...
bool QnDwm::extendFrameIntoClientArea(const QMargins &margins) {
    ...
    if(SUCCEEDED(status)) {
        ...
        if(result == 0) {
            ...
            if(error != 0)
                NX_WARNING(this, "Failed to set window background brush to transparent, error code '%1'.", error);
        }
    }
...
bool QnDwm::setCurrentFrameMargins(const QMargins &margins) {
    ...
    status = SetWindowPos(hwnd, nullptr, 0, 0, 0, 0, SWP_NOMOVE | SWP_NOSIZE | SWP_NOZORDER | SWP_NOACTIVATE | SWP_FRAMECHANGED);
    if(!SUCCEEDED(status))
        return false;
