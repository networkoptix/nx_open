    ...
    , m_navigationBar(nullptr)
{
    installEventHandler({this}, {QEvent::Resize, QEvent::Move},
        this, &QnCalendarWidget::geometryChanged);
...
int QnCalendarWidget::y() const
{
    return pos().y();
}

void QnCalendarWidget::setY(int y)
{
    const auto pos_ = pos();
...
{
    QnTimePeriod localPeriod(date.startOfDay().toMSecsSinceEpoch() - m_localOffset, DAY);
...
{
    qint64 enabledStartMs = minimumDate().startOfDay().toMSecsSinceEpoch();
    qint64 enabledEndMs = maximumDate().startOfDay().addDays(1).toMSecsSinceEpoch();
