...
struct CompoundCondition
{
    ...
    bool operator()(QGraphicsItem *item) const {
        return condition(item) && dynamic_cast<GraphicsItem *>(item) != nullptr;
    }
...
public:
    ...
    /**
     * \returns                        Instrument manager for this instrument, or nullptr if none.
     */
    ...
    /**
     * Note that this function returns nullptr if this instrument isn't installed to
...
protected:
    friend class InstrumentManagerPrivate; /* Clashes with our internals. */
    template<class T>
    ...
    /**
     * \param viewport                 Viewport. Must not be nullptr and must actually be a viewport.
     ...
     *
     * Scene may be nullptr inside this call, which would mean that scene is being
     * destroyed.
     */
     ...
     *
     * It is guaranteed that scene will not be nullptr inside this call.
     *
