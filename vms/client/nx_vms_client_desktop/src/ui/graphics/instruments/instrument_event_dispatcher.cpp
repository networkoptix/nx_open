...
void InstrumentEventDispatcher<T>::uninstallInstrumentInternal(Instrument *instrument, T *target) {
    ...
    foreach(QEvent::Type eventType, instrument->watchedEventTypes(TARGET_TYPE)) {
        ...
        if(!dispatching()) {
             ...
             * So don't make any structural modifications, just nullptr out the
             * uninstalled instrument. */
            instruments[instruments.indexOf(instrument)] = nullptr;
        }
    }
}
...
void InstrumentEventDispatcher<T>::installInstrument(Instrument *instrument, InstallationMode::Mode mode, Instrument *reference) {
    NX_ASSERT(instrument != nullptr);

    if (m_instruments.contains(instrument)) {
        NX_ASSERT(false, "Instrument '%1' is already registered with this instrument event dispatcher.", instrument->metaObject()->className());
        return;
    }

    if (instrument->thread() != this->thread()) {
        NX_ASSERT(false, "Cannot install instrument for an instrument event dispatcher in a different thread.");
        return;
    }
...
void InstrumentEventDispatcher<T>::uninstallInstrument(Instrument *instrument) {
    ...
    if (!m_instruments.contains(instrument)) {
        NX_ASSERT(false, "Instrument '%1' is not registered with this instrument event dispatcher.", instrument->metaObject()->className());
        return;
    }
...
void InstrumentEventDispatcher<T>::registerTarget(T *target) {
    NX_ASSERT(target != nullptr);

    if(m_typeInfoByTarget.contains(target)) {
        NX_ASSERT(false, "Given target is already registered with this instrument event dispatcher.");
        return;
    }
    ...
    foreach(Instrument *instrument, m_instruments)
        installInstrumentInternal(instrument, target, InstallFirst, nullptr);
}
...
void InstrumentEventDispatcher<T>::unregisterTarget(T *target) {
    ...
    if(!m_typeInfoByTarget.contains(target)) {
        NX_ASSERT(false, "Given target is not registered with this instrument event dispatcher.");
        return;
    }
...
#ifdef _DEBUG
        if(!m_typeInfoByTarget.contains(target))
            NX_ASSERT(false, "Received an event from a target that is not registered with this instrument event dispatcher.");
#endif
        return false;
    }
