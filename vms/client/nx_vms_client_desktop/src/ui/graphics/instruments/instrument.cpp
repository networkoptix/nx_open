    ...
    base_type(parent)
{
    ...
    if(type < 0 || type >= WatchedTypeCount) {
        NX_ASSERT(false, "Invalid watched type %1", type);
...
void Instrument::setItemConditions(const QList<InstrumentItemCondition *> &itemConditions) {
    ...
    if(itemConditions.contains(nullptr)) {
        NX_ASSERT(false, "Given item conditions list contains nullptr conditions.");
        effectiveItemConditions.removeAll(nullptr);
    }

    if (isInstalled())
    {
        NX_ASSERT(false, "Cannot change item conditions of an installed instrument.");

        // We must claim ownership.
        m_ownedItemConditions.unite(nx::utils::toQSet(effectiveItemConditions));
        return;
    }

    qDeleteAll(m_ownedItemConditions);
    m_ownedItemConditions = nx::utils::toQSet(effectiveItemConditions);
    m_itemConditions = effectiveItemConditions;
}

void Instrument::addItemCondition(InstrumentItemCondition *itemCondition) {
    if (!NX_ASSERT(itemCondition))
        return;
...
QList<QGraphicsItem *> Instrument::items(QGraphicsView *view, const QPointF &scenePos) const {
    if (m_scene == nullptr) {
        if (!isInstalled()) {
            NX_ASSERT(false, "Instrument is not installed.");
        } else {
            NX_ASSERT(false, "Scene is being destroyed.");
        }
...
QList<QGraphicsItem *> Instrument::items(const QGraphicsSceneMouseEvent *event) const {
    ...
    QGraphicsView *view = viewport == nullptr ? nullptr : qobject_cast<QGraphicsView *>(viewport->parent());
