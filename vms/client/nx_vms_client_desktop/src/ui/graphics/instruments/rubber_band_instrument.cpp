...
void RubberBandInstrument::dragMove(DragInfo *info) {
    ...
    if (info->modifiers() & Qt::ShiftModifier) {
        ...
        scene()->setSelectionArea(selectionArea, Qt::ReplaceSelection, view->rubberBandSelectionMode(), view->viewportTransform());
    }
}
...
void RubberBandInstrument::finishDrag(DragInfo *info) {
    ...
    rubberBand()->setViewport(nullptr);
    rubberBand()->setVisible(false);
}
