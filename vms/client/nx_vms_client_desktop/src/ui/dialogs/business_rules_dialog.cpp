...
protected:
    ...
    virtual bool lessThan(const QModelIndex& left, const QModelIndex& right) const override
    {
        switch (Column(sortColumn()))
        {
            ...
            case Column::event:
                return lessThanByRole<vms::api::EventType>(left,
                    right,
                    Qn::EventTypeRole,
            ...
            case Column::action:
                return lessThanByRole<vms::api::ActionType>(left,
                    right,
                    Qn::ActionTypeRole,
...
private:
    ...
    template<typename T>
    bool lessThanByRole(const QModelIndex& left,
        const QModelIndex& right,
        int role,
        std::function<bool(T left, T right)> comp = std::less<T>()) const
    {
...
void QnBusinessRulesDialog::at_resetDefaultsButton_clicked()
{
    ...
    {
        connection->getEventRulesManager(nx::network::rest::kSystemSession)->resetBusinessRules(
...
bool QnBusinessRulesDialog::saveAll()
{
    ...
    {
        int handle = connection->getEventRulesManager(nx::network::rest::kSystemSession)->deleteRule(
            id,
