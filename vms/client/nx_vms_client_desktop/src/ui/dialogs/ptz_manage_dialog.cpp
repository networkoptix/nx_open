...
void QnPtzManageDialog::saveData()
{
    ...
    if (nx::vms::client::core::ptz::helpers::getSortedPresets(controller(), presets))
        m_model->setPresets(presets);
    ...
    if (controller()->getActiveObject(&ptzObject) && ptzObject.type == Qn::TourPtzObject)
    {
        ...
        {
            // #TODO: Implement a special api for this.
            // Reset active object.
            controller()->continuousMove({});
        }
    }
}

DataFields QnPtzManageDialog::requiredFields() const
{
    return DataField::presets | DataField::tours | DataField::homeObject;
}

void QnPtzManageDialog::updateFields(DataFields fields)
{
...
void QnPtzManageDialog::at_addTourButton_clicked()
{
    ...
    // TODO: #sivanov Duplicate code with QnPtzTourWidget.
