...
void QnAttachToVideowallDialog::updateLicencesUsage()
{
    ...
    if (nx::vms::common::saas::saasInitialized(systemContext))
    {
        ...
        if (isShutdown)
        {
            ...
            ui->licensesLabel->setText(
                tr("Site shut down. To attach to a Video Wall, the Site should be in active "
    ...
    ConfigureVideoWallLicenseUsageHelper helper(systemContext);
    helper.setScreenCountChange(localScreensChange);
