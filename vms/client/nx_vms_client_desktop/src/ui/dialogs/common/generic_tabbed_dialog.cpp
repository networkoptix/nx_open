...
void QnGenericTabbedDialog::addPage(int key, QnAbstractPreferencesWidget *page, const QString &title)
{
    ...
    for(const Page &page: m_pages) {
       if (page.key != key)
        ...
        NX_ASSERT(false,
            "QnGenericTabbedDialog '%1' already contains %2 as %3",
            metaObject()->className(),
            page.widget->metaObject()->className(),
            key);
        return;
    }
...
void QnGenericTabbedDialog::setPageVisible(int key, bool visible)
{
    ...
    NX_ASSERT(false, "QnGenericTabbedDialog '%1' does not contain %2", metaObject()->className(), key);
}
