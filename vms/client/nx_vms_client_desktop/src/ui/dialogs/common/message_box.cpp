...
void QnMessageBoxPrivate::init()
{
    ...
    connect(q->ui->mainLabel, &QLabel::linkActivated, q, &QnMessageBox::linkActivated);
...
QDialogButtonBox::StandardButton QnMessageBox::information(
    ...
    QDialogButtonBox::StandardButton defaultButton)
{
    return execute(parent, Icon::Information, text, extras, buttons, defaultButton);
}
...
QDialogButtonBox::StandardButton QnMessageBox::warning(
    ...
    QDialogButtonBox::StandardButton defaultButton)
{
    return execute(parent, Icon::Warning, text, extras, buttons, defaultButton);
}
...
QDialogButtonBox::StandardButton QnMessageBox::question(
    ...
    QDialogButtonBox::StandardButton defaultButton)
{
    return execute(parent, Icon::Question, text, extras, buttons, defaultButton);
}
...
QDialogButtonBox::StandardButton QnMessageBox::critical(
    ...
    QDialogButtonBox::StandardButton defaultButton)
{
    return execute(parent, Icon::Critical, text, extras, buttons, defaultButton);
}
...
QDialogButtonBox::StandardButton QnMessageBox::success(
    ...
    QDialogButtonBox::StandardButton defaultButton)
{
    return execute(parent, Icon::Success, text, extras, buttons, defaultButton);
}
...
QPixmap QnMessageBox::getPixmapByIconId(Icon icon)
{
...
void QnMessageBox::setIcon(Icon icon)
{
    Q_D(QnMessageBox);
    if (d->icon == icon)
        return;
...
void QnMessageBox::setInformativeText(const QString& text, bool split)
{
    ...
    setInformativeText(lines);
}

void QnMessageBox::setInformativeText(const QStringList &lines)
{
    Q_D(QnMessageBox);
    ...
    for (const QString& line: lines)
    {
        ...
        if (line.contains("http")) //< Make both external and internal links work.
            ...
            connect(label, &QLabel::linkActivated, this, &QnMessageBox::linkActivated);
    ...
    switch (layout)
    {
        ...
        case QnMessageBox::Layout::BeforeAdditionalInfo:
            ...
            ui->verticalLayout->insertWidget(
                ...
                alignment);
            break;
        case QnMessageBox::Layout::ButtonBox:
            if (auto buttonsLayout = qobject_cast<QHBoxLayout*>(ui->buttonBox->layout());
                NX_ASSERT(buttonsLayout, "Invalid QDialogButtonBox layout"))
            {
                buttonsLayout->insertWidget(/*index*/0, widget, stretch, alignment);
            }
            break;
        default:
            break;
