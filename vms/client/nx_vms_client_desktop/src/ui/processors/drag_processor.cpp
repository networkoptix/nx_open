...
void DragProcessor::reset() {
    transition(static_cast<QMouseEvent *>(nullptr), Waiting);
}

void DragProcessor::setHandler(DragProcessHandler* handler)
{
    if (handler && handler->m_processor)
    {
        NX_ASSERT(handler->m_processor == this, "Given handler is assigned to another processor.");
        return;
    }
...
void DragProcessor::setStartDragDistance(int startDragDistance) {
    if(startDragDistance <= 0) {
        NX_ASSERT(false, "Invalid non-positive start drag distance '%1'.", startDragDistance);
        return;
    }
...
void DragProcessor::checkThread(QObject *object) {
    ...
    if(object->thread() != thread()) {
        NX_ASSERT(false, "Processing an event for an object from another thread.");
        return;
    }
#else
    Q_UNUSED(object)
...
void DragProcessor::transitionInternalHelper(State newState) {
    ...
    case Waiting:
        ...
        if(transitionCounter == m_transitionCounter) {
            ...
            m_info.m_view = nullptr;
            m_info.m_scene = nullptr;
...
void DragProcessor::timerEvent(QTimerEvent *event) {
    transition(static_cast<QMouseEvent *>(nullptr), Running);
...
void DragProcessor::redrag() {
    if(m_state != Running) {
        NX_ASSERT(false, "Cannot issue a drag command while not dragging.");
        return;
    }

    drag(nullptr, m_info.m_mouseScreenPos, m_info.m_mouseScenePos, m_info.m_mouseItemPos, true);
}
