    ...
    m_playbackPositionBlinkTimer(new QTimer(this))
{
    ...
    connect(m_focusListenerInstrument, &FocusListenerInstrument::focusItemChanged, this,
        [this]()
        {
            ...
            const static std::vector<menu::IDType> kActionsToDisable = {
            ...
            const static auto keySequences =
                [this]()
                {
                    ...
                    std::vector<std::pair<menu::IDType, QKeySequence>> result;
...
void QnWorkbenchDisplay::initialize(QGraphicsScene* scene, QGraphicsView* view)
{
    ...
    {
        //
        m_gridBackgroundItem = new QnGridBackgroundItem(m_scene, context());
...
void QnWorkbenchDisplay::at_workbench_currentLayoutAboutToBeChanged()
{
    ...
    // are not in the pool.
    if (!context()->action(menu::ToggleShowreelModeAction)->isChecked())
    {
