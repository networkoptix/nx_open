...
void QnPtzManageModel::setHotkeys(const PresetIdByHotkey &hotkeys) {
...
const PresetIdByHotkey& QnPtzManageModel::hotkeys() const {
    return m_hotkeys;
}
...
bool QnPtzManageModel::setData(const QModelIndex &index, const QVariant &value, int role) {
    ...
    if (role == Qt::CheckStateRole && index.column() == HomeColumn) {
        ...
        if (!ok || ((hotkey > 9 || hotkey < 0) && hotkey != kNoHotkey))
            return false;
...
QVariant QnPtzManageModel::titleData(RowType rowType,  int column, int role) const {
    ...
    case Qt::BackgroundRole:
        return colorTheme()->color("ptz.manage.title");
    case Qt::FontRole:
        {
            QFont f;
            f.setBold(true);
...
QVariant QnPtzManageModel::presetData(const QnPtzPresetItemModel &presetModel, int column, int role) const {
    ...
    case Qt::EditRole:
        ...
        case HotkeyColumn: {
            int hotkey = m_hotkeys.key(presetModel.preset.id, kNoHotkey);
            return hotkey < 0 ? tr("None") : QString::number(hotkey);
        }
        case HomeColumn:
            // TODO: #sivanov Use value from home PTZ controller.
            if (role == Qt::ToolTipRole)
                return tr("This preset will be activated after %n minutes of inactivity", 0, 2);
            break;
        case DetailsColumn:
            return QVariant();
        default:
...
QVariant QnPtzManageModel::tourData(const QnPtzTourItemModel &tourModel, int column,  int role) const {
    ...
    case Qt::EditRole:
        ...
        case HotkeyColumn:
            {
                int hotkey = m_hotkeys.key(tourModel.tour.id, kNoHotkey);
                return hotkey < 0 ? tr("None") : QString::number(hotkey);
            }
        case DetailsColumn: {
    ...
    case Qt::BackgroundRole:
        ...
        case OtherInvalidTour:
            return colorTheme()->color("ptz.manage.invalid");
        case DuplicatedLinesTour:
            return colorTheme()->color("ptz.manage.warning");
        default:
            break;
        }
        break;
