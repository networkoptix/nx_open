...
protected:
    void baseFinished(Command command, const QVariant& data);
    void baseChanged(DataFields fields);

private:
    const QnPtzControllerPtr& baseController(const Options& options) const;
