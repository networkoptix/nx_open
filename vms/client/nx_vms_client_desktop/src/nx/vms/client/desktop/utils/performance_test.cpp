...
void PerformanceTest::detectLightMode()
{
    ...
    if (nx::build_info::isLinux())
    {
        ...
        QRegularExpression poorCpuRegExp(
            QRegularExpression::anchoredPattern(QLatin1String("Intel\\(R\\) (Atom\\(TM\\)|Celeron\\(R\\)) CPU .*")));
        poorCpu = poorCpuRegExp.match(cpuName).hasMatch();
        ...
        if (context->create() && context->makeCurrent(surface.get()))
        {
            ...
            QRegularExpression poorRendererRegExp(
                QRegularExpression::anchoredPattern(QLatin1String("Gallium .* on llvmpipe .*|Mesa DRI Intel\\(R\\) Bay Trail.*")));
            poorGpu = poorRendererRegExp.match(renderer).hasMatch();
