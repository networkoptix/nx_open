...
QString TableView::jsonContents() const
{
    const QAbstractItemModel* const modelPtr = model();
    const int rows = modelPtr->rowCount();
    const int columns = modelPtr->columnCount();

    QVariantMap result;
    QVariantList headers;
    for (int column = 0; column < columns; column++)
        headers.append(modelPtr->headerData(column, Qt::Horizontal));
    ...
    for (int row = 0; row < rows; row++)
    {
        QStringList rowData;
        for (int column = 0; column < columns; column++)
            rowData.append(modelPtr->index(row, column).data().toString());
        tableData.push_back(QVariant(rowData));
    }
    result.insert("data", tableData);

    return QString(QJsonDocument::fromVariant(result).toJson());
}
