...
void ConnectToCloudTool::requestCloudAuthData()
{
    ...
    m_oauthLoginDialog = new OauthLoginDialog(m_parent, /*clientType*/ "connect");
...
void ConnectToCloudTool::onLocalSessionTokenReady()
{
    ...
    if (!NX_ASSERT(commonModule()->currentServer())
        || !commonModule()->currentServer()->hasInternetAccess())
    {
