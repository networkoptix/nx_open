 ...
 * Scope of an action.
 *
 * Scope defines the menus in which an action can appear, and target for getting the action's
 * parameters in case it was triggered with a hotkey.
 */
enum ActionScope
{
    ...
    TreeScope = 0x00000004,             /**< Action appears in resource tree context menu. */
    TableScope = 0x00000008,            /**< Action appears in table-based resource list context menu. */
    TimelineScope = 0x00000010,         /**< Action appears in slider context menu. */
    TitleBarScope = 0x00000020,         /**< Action appears title bar context menu. */
    NotificationsScope = 0x00000040,
...
enum ActionFlag
{
     ...
     * It is up to the user to ensure that proper action conditions make it impossible for several
     * actions to be triggered by this hotkey.
     */
    ...
    /**
     * Action can be pulled into enclosing menu if it is the only one in its submenu.
     * It may have different text in this case.
     */
    Pullable = 0x00400000,
     ...
     * not resource widgets).
     */
    VideoWallReviewScene = SceneScope,

    /** Action can appear in left panel resource tree context menu. */
    Tree = TreeScope,

    /** Action can appear in table-based resource list context menu. */
    Table = TableScope,
