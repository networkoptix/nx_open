...
struct IntercomManager::Private: public QObject
{
    ...
    Private(IntercomManager* owner):
        q(owner)
    {
        connect(q->commonModule()->resourceAccessProvider(),
            ...
            [this](const QnResourceAccessSubject& subject, const QnResourcePtr& resource)
            {
                ...
                if (q->commonModule()->resourceAccessProvider()->hasAccess(currentUser, resource))
        ...
        auto userWatcher = q->commonModule()->instance<nx::vms::client::core::UserWatcher>();
    ...
    {
        if (!q->commonModule()->resourceAccessProvider()->hasAccess(currentUser, intercom))
            return;
        ...
        if (!layoutResource)
        {
            QnLayoutResourcePtr intercomLayout = QnLayoutResource::createFromResource(intercom);
...
IntercomManager::IntercomManager(QObject* parent):
    base_type(parent),
