...
QJsonDocument execute(QJsonObject request)
{
    ...
    const QJSValue result =
        Director::instance()->execute(request.value("source").toString());
    ...
    QJsonObject response = formResponseBase(DirectorJsonInterface::Ok);
    auto jsonResult = result.toVariant().toJsonValue();
