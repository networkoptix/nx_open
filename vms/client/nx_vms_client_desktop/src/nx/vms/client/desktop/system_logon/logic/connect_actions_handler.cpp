    ...
    d(new Private(this))
{
        ...
        [this]()
        {
            ...
            if (NX_ASSERT(actionParameters.hasArgument(Qn::LogonParametersRole)))
            {
                ...
                if (NX_ASSERT(welcomeScreen)
                    && NX_ASSERT(parameters.expectedServerId))
                {
    ...
    auto initializeConnectionUserInteractionDelegate =
        [this]()
        {
            qnClientCoreModule->networkModule()->connectionFactory()->setUserInteractionDelegate(
                std::make_unique<RemoteConnectionUserInteractionDelegate>(mainWindowWidget()));
        };
...
void ConnectActionsHandler::at_reconnectAction_triggered()
{
    ...
    const ConnectionInfo connectionInfo(currentConnection->connectionInfo());
    const QnUuid currentServerId = commonModule()->remoteGUID();
...
void ConnectActionsHandler::at_disconnectAction_triggered()
{
    ...
    const bool wasLoggedIn = context()->user();
...
void ConnectActionsHandler::at_selectCurrentServerAction_triggered()
{
    ...
    if (!NX_ASSERT(serverId != commonModule()->remoteGUID(), "Checked in the action conditions"))
        return;
...
std::optional<ConnectActionsHandler::CloudSystemEndpoint>
    ConnectActionsHandler::cloudSystemEndpoint(const QString& systemId) const
{
    if (!NX_ASSERT(qnCloudStatusWatcher->status() != QnCloudStatusWatcher::LoggedOut))
        return std::nullopt;

    NX_DEBUG(this, "Connecting to the cloud system %1", systemId);

    auto system = qnSystemsFinder->getSystem(systemId);
    if (!system || !system->isReachable())
    {
        NX_WARNING(this, "System %1 is no longer accessible", systemId);
        return std::nullopt;
    }
    ...
    if (url.isEmpty())
    {
        NX_WARNING(this, "No connection to the system %1 is found", systemId);
        return std::nullopt;
    }
