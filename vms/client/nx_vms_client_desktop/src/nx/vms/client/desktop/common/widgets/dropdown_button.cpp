    ...
    base_type(parent)
{
    ...
    menu->setProperty(nx::style::Properties::kMenuAsDropdown, true);
    setMenu(menu);
...
QPoint DropdownButton::menuPosition() const
{
    ...
    return Geometry::movedInto(globalRect, screen()->geometry()).topLeft().toPoint();
}
...
void DropdownButton::setAdjustSize(bool enable)
{
    m_adjustSize = enable;
}
