...
QString getUrlForSearch(const QnResourcePtr& resource)
{
    ...
    // virtual camera's URL consists of prefix and physical ID, both are unwanted.
    if (resource->flags().testFlag(Qn::virtual_camera))
        return QString();
...
bool matches(const QString& searchString, const QnResourcePtr& resource)
{
    ...
    if (resourceGroupContainsString(searchString, resource))
        return true;

    auto searchWords = uniqueSearchWords(searchString);
    const auto matches = matchSearchWords(searchWords, resource);

    // Deduce result by loose AND logic: each search word should match at least one
    // resource parameter.
...
{
    QStringList result = searchString.split(" ", Qt::SkipEmptyParts);
