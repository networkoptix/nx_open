...
Manager::Manager(WindowContext* windowContext, QObject* parent):
    QObject(parent),
    WindowContextAware(windowContext)
{
    m_root = new Action(NoAction, windowContext, this);
    ...
    if (!parent->children().isEmpty())
    {
        foreach(Action* action, parent->children())
        {
            ...
            if (action->textFactory())
                replacedText = action->textFactory()->text(parameters, windowContext());
