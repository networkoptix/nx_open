...
void WatermarkPainter::setWatermark(nx::core::Watermark watermark)
{
    ...
    m_image = QImage(); //< Destroy current image because now we need a new one.
}
...
{
    m_image = nx::core::retrieveWatermarkImage(m_watermark, size);
    m_size = size;
}
