    ...
    WindowContext* context)
{
    ...
    // TODO: #sivanov Looks like generic permission check should be used instead. Check shall not
    // depend on the window context at all.
    QnUserResourcePtr owner = parameters.argument<QnUserResourcePtr>(Qn::UserResourceRole);
    bool ownResources = owner && owner == context->workbenchContext()->user();
...
ActionVisibility NewUserLayoutCondition::check(const Parameters& parameters, WindowContext* context)
{
    ...
    if (!user || user == context->workbenchContext()->user())
        return InvisibleAction;
    ...
    const auto accessController = qobject_cast<AccessController*>(
        context->workbenchContext()->accessController());
