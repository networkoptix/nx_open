    ...
    const QRectF& sceneRect)
{
    ...
    info.type = segment.intersects(leftBorder, &info.point);
    ...
    info.type = segment.intersects(rightBorder, &info.point);
    if (info.type != QLineF::NoIntersection)
    {
    ...
    info.type = segment.intersects(topBorder, &info.point);
    if (info.type != QLineF::NoIntersection)
    {
        info.point.setY(0);
        if (sceneRect.contains(info.point))
    ...
    info.type = segment.intersects(bottomBorder, &info.point);
    if (info.type != QLineF::NoIntersection)
    {
