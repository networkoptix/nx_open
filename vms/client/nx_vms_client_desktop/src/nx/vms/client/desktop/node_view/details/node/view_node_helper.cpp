...
bool ViewNodeHelper::isHeader(const ViewNodeData& data)
{
    return data.property(isHeaderProperty).toBool();
}

bool ViewNodeHelper::isHeader(const NodePtr& node)
{
    return node && isHeader(node->data());
}

bool ViewNodeHelper::isHeader(const QModelIndex& index)
{
    return isHeader(nodeFromIndex(index));
}
...
void ViewNodeHelper::ViewNodeHelper::forEachLeaf(const NodePtr& root, const ViewNodeHelper::ForEachNodeCallback& callback)
{
    forEachNodeInternal(root, callback, true);
}

void ViewNodeHelper::ViewNodeHelper::forEachNode(const NodePtr& root, const ViewNodeHelper::ForEachNodeCallback& callback)
{
    forEachNodeInternal(root, callback, false);
}

NodePtr ViewNodeHelper::createSeparatorNode(bool isHeader, int groupSortOrder)
{
    auto data = ViewNodeDataBuilder().separator().withGroupSortOrder(groupSortOrder).data();
    if (isHeader)
        ViewNodeDataBuilder(data).header();
