    ...
    ui(new Ui::EulaDialog())
{
    ...
    ui->eulaView->controller()->setRedirectLinksToDesktop(true);
...
{
    const auto eulaHtmlStyle = generateCssStyle();
    ...
    ui->eulaView->controller()->setHtml(eulaText, QUrl("qrc://"));
...
bool EulaDialog::acceptEulaHtml(const QString& html, int version, QWidget* parent)
{
    ...
    if (const auto match = headerRegExp.match(html); match.hasMatch())
    {
        const auto title = match.capturedView(1);
