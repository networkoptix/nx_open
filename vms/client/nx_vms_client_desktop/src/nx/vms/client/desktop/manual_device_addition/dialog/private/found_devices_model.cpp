...
IdsSet extractIds(const std::vector<nx::vms::api::DeviceModelForSearch>& devices)
{
    ...
    for (const auto& device: devices)
        result.insert(device.physicalId);
...
QVariant FoundDevicesModel::getCheckStateData(const QModelIndex& index) const
{
    ...
    return m_deviceRowState[device(index).physicalId].second
        ? Qt::Checked
        : Qt::Unchecked;
}
...
QVariant FoundDevicesModel::getForegroundColorData(const QModelIndex& index) const
{
    ...
    const auto deviceRowState = m_deviceRowState[m_devices.at(index.row()).physicalId];
...
QVariant FoundDevicesModel::data(const QModelIndex& index, int role) const
{
    ...
    switch (role)
    {
        ...
        case presentedStateRole:
            return m_deviceRowState[device(index).physicalId].first;
    ...
    int role)
{
    ...
    {
        const auto physicalId = m_devices[index.row()].physicalId;
        const bool newCheckState = value.value<Qt::CheckState>() != Qt::Unchecked;

        auto& deviceRowState = m_deviceRowState[physicalId];
    ...
    {
        const auto physicalId = device(index).physicalId;
    ...
    int role) const
{
    ...
    switch (section)
    {
        ...
        case Columns::presentedStateColumn:
            return QString("%1, %2").arg(tr("%n devices total", "", rowCount()))
                .arg(
                    tr("%n new", "Numerus: %n is the count of new devices",
                    rowCount() - deviceCount(alreadyAddedState)));
        default:
            return QVariant();
    }
}
