...
TEST_F(UpdateVerificationTest, bestVariantVersionSelection)
{
    ...
    const FindPackageResult result = info.findPackage(update::Component::server, os::ubuntu16);
...
TEST_F(UpdateVerificationTest, variantVersionBoundaries)
{
    ...
    const FindPackageResult result = info.findPackage(update::Component::server, os::ubuntu18);
...
TEST_F(UpdateVerificationTest, emptyOsVersionAndVersionedPackage)
{
    ...
    const FindPackageResult result = info.findPackage(update::Component::server, os::ubuntu);
    ASSERT_TRUE(std::holds_alternative<update::FindPackageError>(result));
    ASSERT_EQ(
        std::get<update::FindPackageError>(result),
        update::FindPackageError::osVersionNotSupported);
...
TEST_F(UpdateVerificationTest, aSyntheticTest)
{
    ...
    const FindPackageResult result = info.findPackage(update::Component::server, os::ubuntu14);
    ASSERT_TRUE(std::holds_alternative<update::Package>(result));
    ASSERT_EQ(std::get<update::Package>(result).file, "ubuntu.zip");
}
