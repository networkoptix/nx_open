...
public:
    ...
    QVariant data(const QModelIndex& index, int role = Qt::DisplayRole) const override
    {
        ...
        switch(role)
        {
            ...
            case QnSystemsModel::IsConnectableRoleId:
                return m_systems[row].isConnectable;
            default:
                return QVariant();
        }
    }
    ...
    void updateSystemAt(int row, const SystemInfo& info)
    {
        ...
        if (prevInfo.isConnectable != info.isConnectable) roles << QnSystemsModel::IsConnectableRoleId;
...
public:
    ...
    QVector<SystemInfo> systems()
    {
        ...
        for (int i = 0; i < rowCount(); i++)
        {
            ...
            bool isConnectable = mi.data(QnSystemsModel::IsConnectableRoleId).toBool();

            result.append({name, systemId, localId, address, isNew, isCloud, isConnectable});
        }
...
protected:
    void SetUp() override
    {
        ...
        uuid1 = QnUuid::createUuid();
        uuid2 = QnUuid::createUuid();
...
{
    //                  name     systemId                               localId  address      isNew  isCloud isConnectable
    SystemInfo system0 {"Name1", QnUuid::createUuid().toSimpleString(), "00011", "127.0.0.1", false, false,  true};
    SystemInfo system1 {"Name2", QnUuid::createUuid().toSimpleString(), "00022", "127.0.0.1", true,  false,  true};
...
TEST_F(OrderedSystemsModelDataTest, addSystem)
{
    ...
    //                  name     systemId                               localId  address      isNew  isCloud isConnectable
    SystemInfo system6 {"N",     QnUuid::createUuid().toSimpleString(), "00001", "127.0.0.1", false, false,  true};
    SystemInfo system7 {"N",     QnUuid::createUuid().toSimpleString(), "00002", "4.3.2.1",   false, false,  true};
