...
namespace Qee {
    ...
    public:
        ...
        Token(TokenType type, const QStringView &text, int pos): m_type(type), m_text(text), m_pos(pos) {}

        TokenType type() const { return m_type; }
        QStringView text() const { return m_text; }
    ...
    public:
        ...
        void requireSize(int size) const {
            if(m_size != size)
                throw IllegalArgumentException(QString("Function '%1' is expected to have %3 arguments, %2 provided.").arg(m_name).arg(m_size).arg(size));
        }

        void requireSize(int minSize, int maxSize) const {
            if(m_size < minSize || m_size > maxSize)
                throw IllegalArgumentException(QString("Function '%1' is expected to have %3-%4 arguments, %2 provided.").arg(m_name).arg(m_size).arg(minSize).arg(maxSize));
        }
