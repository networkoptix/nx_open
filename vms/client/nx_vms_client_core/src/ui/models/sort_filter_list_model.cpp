...
bool QnSortFilterListModelPrivate::filterOut(int sourceRow)
{
    ...
    const bool rowIsFilteredOut = isRowRejected || isFilteredByWildcard(sourceRow);
    if (rowIsFilteredOut)
        m_filteredRows.insert(sourceRow);

    return rowIsFilteredOut;
}
...
void QnSortFilterListModelPrivate::removeSourceRow(int sourceRow)
{
    ...
    if (mappedIndex == -1) //< Row is filtered out.
    {
        ...
        m_filteredRows.remove(sourceRow);
        updateAcceptedRowCount();
        return;
    }
    ...
    const QVector<int>& roles)
{
    ...
    const auto targetRoles = nx::utils::toQSet(roles);
