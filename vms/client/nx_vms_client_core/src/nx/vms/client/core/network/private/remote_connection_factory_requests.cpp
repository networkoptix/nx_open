    ...
    ContextPtr context)
{
    auto storeCertificateFunction =
        [context](const nx::network::ssl::CertificateChain& chain)
        {
...
void RemoteConnectionFactoryRequestsManager::checkDigestAuthentication(ContextPtr context)
{
    ...
    if (context->info.credentials.authToken.type != AuthTokenType::password)
    {
        ...
        if (context->purpose != Context::Purpose::connectInCompatibilityMode)
        {
            NX_DEBUG(this,
                "Unexpected auth token type %1",
                context->info.credentials.authToken.type);
    ...
    nx::cloud::db::api::Connection* cloudConnection)
{
    ...
    request.grant_type = IssueTokenRequest::GrantType::refreshToken;
    request.scope =
