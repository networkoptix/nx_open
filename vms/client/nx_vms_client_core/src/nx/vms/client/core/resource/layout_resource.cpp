...
const auto unsavedChangedGuard(const LayoutResourcePtr& layout)
{
    return nx::utils::ScopeGuard(
        [layout, wasUnsaved = layout && layout->canBeSaved()]
        {
...
qreal LayoutResource::cellSpacingValue(CellSpacing spacing)
{
    switch (spacing)
    {
        case CellSpacing::None:
            return 0.0;
        case CellSpacing::Small:
            return nx::vms::api::LayoutData::kDefaultCellSpacing;
        case CellSpacing::Medium:
            return nx::vms::api::LayoutData::kDefaultCellSpacing * 2;
        case CellSpacing::Large:
...
bool LayoutResource::isEqualCellSpacing(CellSpacing spacing, qreal value)
{
...
void LayoutResource::setPredefinedCellSpacing(CellSpacing spacing)
{
...
void LayoutResource::restoreFromSnapshot()
{
...

    {
        ...
        {
            if (core::getResourceByDescriptor(item.resource))
...
{
    const auto systemContext = qobject_cast<core::SystemContext*>(this->systemContext());
