...
struct RemoteConnectionFactory::Private: public /*mixin*/ QnCommonModuleAware
{
    ...
    {
        if (context && context->info.expectedServerId
            && *context->info.expectedServerId != context->moduleInformation.id)
        {
    ...
    void fixCloudConnectionInfoIfNeeded(ContextPtr context)
    {
        ...
        if (context->info.userType == nx::vms::api::UserType::cloud && isRestApiSupported(context))
