 ...
 */
class NX_VMS_CLIENT_CORE_API NotificationGuard
{
public:
...
NX_VMS_CLIENT_CORE_API void dataChanged(
    IEntityNotification* entityNotification, const QVector<int>& roles, int first, int count = 1);

NX_VMS_CLIENT_CORE_API NotificationGuard insertRowsGuard(
    IEntityNotification* entityNotification, int first, int count = 1);

NX_VMS_CLIENT_CORE_API NotificationGuard removeRowsGuard(
    IEntityNotification* entityNotification, int first, int count = 1);

NX_VMS_CLIENT_CORE_API NotificationGuard moveRowsGuard(IEntityNotification* entityNotification,
    const AbstractEntity* source, int first, int count,
    const AbstractEntity* desination, int beforeRow);

NX_VMS_CLIENT_CORE_API NotificationGuard layoutChangeGuard(
