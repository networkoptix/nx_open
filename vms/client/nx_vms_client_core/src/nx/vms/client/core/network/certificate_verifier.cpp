...
struct CertificateVerifier::Private
{
        ...
        const nx::network::ssl::CertificateChain& chain)
    {
        ...
        auto cache = certificateCache();
        if (!cache)
            return Status::notFound;

        const auto connectionCertificatePublicKey =
            cache->certificate(serverId, CertificateType::connection);
        const auto autoGeneratedCertificatePublicKey =
            cache->certificate(serverId, CertificateType::autogenerated);
...
nx::network::ssl::VerifyCertificateFunc CertificateVerifier::makeVerifyCertificateFunc(
    const QnUuid& serverId)
{
    return
        [this, serverId](const nx::network::ssl::CertificateChain& chain)
        {
            NX_MUTEX_LOCKER lock(&d->mutex);
            return d->verifyCertificateByCache(serverId, chain) == Status::ok;
        };
}
...
nx::network::ssl::AdapterFunc CertificateVerifier::makeAdapterFunc(const QnUuid& serverId)
{
    ...
    return nx::network::ssl::makeAdapterFunc(makeVerifyCertificateFunc(serverId));
}
    ...
    CertificateVerifier::CertificateType certType) const
{
    ...
    {
        case CertificateVerifier::CertificateType::autogenerated:
            if (auto it = d->autoGeneratedCertificates.find(serverId);
                it != d->autoGeneratedCertificates.end())
            {
                return it->second;
            }
            break;
        case CertificateVerifier::CertificateType::connection:
            if (auto it = d->connectionCertificates.find(serverId);
                it != d->connectionCertificates.end())
            {
                return it->second;
            }
            break;
