...
struct FfmpegImageProvider::Private
{
    ...
    QImage image;
    ThumbnailStatus status = ThumbnailStatus::Invalid;
...
ThumbnailStatus FfmpegImageProvider::status() const
{
    return d->status;
}
...
{
    if (d->status == ThumbnailStatus::Loading)
        return;

    d->status = ThumbnailStatus::Loading;
        ...
        [resource = d->resource, position = d->position, thread = thread(), callback]()
        {
            const auto image =
                [resource, position]() -> QImage
                {
                    ...
                    {
                        archiveDelegate->seek(position.count(), /*findIFrame*/ true);
                    }
