...
CameraThumbnailManager::ThumbnailData::ThumbnailData():
    status(ThumbnailStatus::Invalid),
...
void CameraThumbnailManager::selectCamera(const QnVirtualCameraResourcePtr& camera)
{
    ...
    if (!camera)
    {
        emit statusChanged(ThumbnailStatus::Invalid);
    ...
    if (data.status == ThumbnailStatus::Invalid || data.status == ThumbnailStatus::NoData)
...
void CameraThumbnailManager::refreshSelectedCamera()
{
    ...
    {
        data.status = ThumbnailStatus::NoData;
        return;
    }
...
ThumbnailStatus CameraThumbnailManager::status() const
{
    if (!m_selectedCamera)
...
ThumbnailStatus CameraThumbnailManager::statusForCamera(
...
bool CameraThumbnailManager::hasThumbnailForCamera(const QnVirtualCameraResourcePtr& camera) const
{
    ...
    return iter->status == ThumbnailStatus::Loaded
        || iter->status == ThumbnailStatus::Refreshing;
}
            ...
            const nx::network::http::HttpHeaders& headers)
        {
            ...
            if (success && !isJsonError && !imageData.isEmpty())
            {
                ...
                if (!data.thumbnail.isNull())
                    data.status = ThumbnailStatus::Loaded;
            }

            if (request.camera == m_selectedCamera && oldStatus != data.status)
                emit statusChanged(data.status);

            if (data.status != ThumbnailStatus::Loaded)
                return;
...
bool CameraThumbnailManager::isUpdating(ThumbnailStatus status) const
{
    ...
    ThumbnailStatus status) const
{
    switch (camera->getStatus())
    {
        ...
        case nx::vms::api::ResourceStatus::online:
            return (status == ThumbnailStatus::NoData);
