...
CloudStatusWatcher::CloudStatusWatcher(QObject* parent): base_type(parent), d(new Private(this))
{
    ...
    connect(this,
        ...
        d.get(),
        &CloudStatusWatcher::Private::setRecentCloudSystems);
    connect(this,
        &CloudStatusWatcher::recentCloudSystemsChanged,
        this,
        [this]() { appContext()->coreSettings()->recentCloudSystems = recentCloudSystems(); });
}
...
void CloudStatusWatcher::resendActivationEmail(const QString& email)
{
    ...
    {
        const bool success = result == ResultCode::ok || result == ResultCode::partialContent;
...
{
    setCloudEnabled(result != ResultCode::networkError && result != ResultCode::serviceUnavailable
...
void CloudStatusWatcher::Private::updateSystemsInternal(int triesCount, quint64 requestId)
{
    ...
    updateSystemsRequestTimer.setRemainingTime(kUpdateSystemsRequestTimeout);
    ...
    ResultCode result, IssueTokenResponse response, const GrantType& grantType)
{
    ...
    m_authData.credentials.authToken = nx::network::http::BearerAuthToken(response.access_token);
...
void CloudStatusWatcher::Private::validateAccessToken()
{
    auto callback = nx::utils::AsyncHandlerExecutor(this).bind(
        [this](ResultCode result, ValidateTokenResponse response)
        {
            ...
            {
                updateStatusFromResultCode(result);
                // updateStatusFromResultCode() may have set status to Offline (e.g. on a
                // transient network error); schedule a retry of issueAccessToken() so the
                // connection recovers on its own instead of staying Offline until the next
                // unrelated trigger (e.g. a manual re-login).
                scheduleConnectionRetryIfNeeded();
                return;
            }
...
void CloudStatusWatcher::Private::scheduleConnectionRetryIfNeeded()
{
    ...
    QTimer::singleShot(kReconnectInterval,
        this,
        [this]()
        {
            if (!cloudConnection || status != CloudStatusWatcher::Offline)
    ...
    CloudStatusWatcher::Status newStatus, CloudStatusWatcher::ErrorCode newErrorCode)
{
    ...
    if (isNewStatus && status == CloudStatusWatcher::LoggedOut
...
void CloudStatusWatcher::Private::setRecentCloudSystems(
    const QnCloudSystemList& newRecentCloudSystems)
{
...
void CloudStatusWatcher::Private::suppressCloudInteraction(TimePoint::duration timeout)
{
