...
void TargetPeerConnector::initiateDirectConnection()
{
    ...
    m_targetPeerSocket = nx::network::SocketFactory::createStreamSocket(
    ...
    if (!setSocketOptionsResult)
    {
        ...
        NX_INFO(this, nx::format("Failed to set socket options. %1")
...
{
    NX_VERBOSE(this, nx::format("Direct connect completed with result %1")
...
{
    NX_VERBOSE(this, nx::format("Connect to %1 failed by timeout.").args(m_targetEndpoint));
    m_targetPeerSocket.reset();
