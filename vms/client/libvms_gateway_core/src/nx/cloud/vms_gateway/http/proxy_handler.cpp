    ...
    ProxyTargetDetectedHandler handler)
{
            ...
            .isCloudHostname(targetHost.target.address.toString()))
    {
        ...
        {
            NX_DEBUG(this, nx::format("It is forbidden by settings to proxy to non-cloud address (%1)")
    ...
    const auto headers = m_runTimeOptions.enforcedHeaders(targetHost.target);
    ...
    TargetHost* const targetHost)
{
    ...
    {
        NX_DEBUG(this, nx::format("It is forbidden by settings to specify target in url (%1)")
    ...
    nx::network::http::insertOrReplaceHeader(
        &request->headers,
        nx::network::http::HttpHeader("Host", targetEndpoint.toString()));
    ...
    TargetHost* const targetHost)
{
    ...
    QList<QStringView> pathItems = QStringView(path).split('/', Qt::SkipEmptyParts);
    if (pathItems.isEmpty())
        return nx::network::http::StatusCode::badRequest;

    // Parse first path item, expected format: [protocol:]address[:port].
    auto targetParts = pathItems[0].split(':', Qt::SkipEmptyParts);
    ...
    {
        const auto position = pathItems[0].data() - path.constData();
        NX_ASSERT(position > 0);
        request->requestLine.url = path.mid(position - 1);  //-1 to include '/'
    }
