...
using VisualizerData = decltype(SpectrumData::data);
...
VisualizerData generateEmptyData(qint64 elapsedMs)
{
    ...
    const int size = VoiceSpectrumAnalyzer::bandsCount();
    const int maxIdx = size * 2 - 1;

    VisualizerData result(size, 0.0);
...
int VoiceSpectrumItem::VisualizerDataGenerator::linesCount() const
{
    ...
    NX_ASSERT(false, "Wrong line parameters");
    return 0;
}
...
VisualizerData VoiceSpectrumItem::VisualizerDataGenerator::getUpdatedData()
{
    ...
    auto currentData = interpolate(
        nx::vms::client::core::appContext()->voiceSpectrumAnalyzer()->getSpectrumData().data,
        linesCount());
