...
WebAdminController::WebAdminController(QObject* parent):
    QObject(parent)
{
    m_credentials = nx::vms::client::core::EncodedCredentials(qnSettings->startupParameters().url);
}
...
{
    m_credentials = nx::vms::client::core::EncodedCredentials(login, password);
}
