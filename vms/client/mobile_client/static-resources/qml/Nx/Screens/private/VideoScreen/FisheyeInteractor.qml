...
NxObject
{
    ...
    readonly property var viewMode: fisheyeShader && fisheyeShader.helper
        ...
        : MediaDewarpingParams.FisheyeCameraMount.wall
    ...
    function updateZoom(x, y, deltaPower)
    {
        ...
        if (viewMode != MediaDewarpingParams.FisheyeCameraMount.wall
            && fisheyeShader.fov(currentScale) >= 90.0)
        {
            updateScaleCenter(x, y)
    ...
    function rotationMatrix(degrees)
    {
        switch (viewMode)
        {
            case MediaDewarpingParams.FisheyeCameraMount.table:
                return Utils3D.rotationZ(Utils3D.radians(degrees.y)).times(
                    Utils3D.rotationX(Utils3D.radians(degrees.x)))

            case MediaDewarpingParams.FisheyeCameraMount.ceiling:
                return Utils3D.rotationZ(-Utils3D.radians(degrees.y)).times(
                    Utils3D.rotationX(Utils3D.radians(degrees.x)))

            default: // MediaDewarpingParams.FisheyeCameraMount.wall
