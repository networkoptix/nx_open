...
package com.nxvms.mobile.media;
...
public class QnVideoDecoder
{
    ...
    private boolean doInit(String codecName, int texName, int width, int height)
        throws Exception
    {
        if (width <= 0 || height <= 0)
        {
        ...
        m_surfaceTexture = new SurfaceTexture(texName, false);
    ...
    public boolean init(final String codecName, int texName, final int width, final int height)
    {
        long result = runCatching("QnVideoDecoder.init(): ",
            new Callable<Long>()
            {
                public Long call()
                    throws Exception
                {
                    if (doInit(codecName, texName, width, height))
                        return (long) 0;
                    else
                        return (long) -1;
                }
        ...
        throws Exception
    {
        ...
        if (inputBufferId >= 0)
        {
            ByteBuffer inputBuffer = m_codec.getInputBuffer(inputBufferId);
