...
def insignia_executable(wix_directory):
    return os.path.join(wix_directory, 'insignia.exe')


def extract_engine_command(wix_directory, bundle, output):
    return [insignia_executable(wix_directory), '-ib', bundle, '-o', output]


def reattach_engine_command(wix_directory, engine, bundle, output):
    return [insignia_executable(wix_directory), '-ab', engine, bundle, '-o', output]


def extract_engine(wix_directory, bundle, output):
    execute_command(extract_engine_command(wix_directory, bundle, output))


def reattach_engine(wix_directory, engine, bundle, output):
    execute_command(reattach_engine_command(wix_directory, engine, bundle, output))
