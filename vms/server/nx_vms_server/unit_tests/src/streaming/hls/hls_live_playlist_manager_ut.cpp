...
TEST(hls_LivePlaylistManager, general)
{
    ...
    MediaStreamCache mediaCache(
        ...
        maxCacheSize.count());
    nx::vms::server::hls::LivePlaylistManager hlsPlaylistManager(
        &mediaCache,
        targetDuration.count(),
        nx::vms::server::Settings::kDefaultHlsRemovedLiveChunksToKeep);
         ...
         curTimestamp += frameStep)
    {
        ...
        std::vector<nx::vms::server::hls::AbstractPlaylistManager::ChunkData> chunks;
