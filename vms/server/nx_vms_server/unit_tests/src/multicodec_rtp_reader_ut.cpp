...
TEST( QnMulticodecRtpReader, DISABLED_streamFetchingOverRTSP )
{
    ...
    resource->setIdUnsafe( QUuid::createUuid() );
    ...
    std::unique_ptr<QnMulticodecRtpReader> rtspStreamReader( new QnMulticodecRtpReader( resource, nullptr ) );
...
TEST( QnMulticodecRtpReader, DISABLED_rtpParsingPerformance )
{
    ...
    for( int i = 0; i < 1; ++i )
    {
        std::unique_ptr<QnMulticodecRtpReader> rtspStreamReader(
            new QnMulticodecRtpReader(resource, nullptr));
