...
HRESULT GetDisabledNICSWithEmptyMac(
    IWbemServices* pSvc, std::vector<_bstr_t>& paths, const QnPathToMacAndDeviceClassMap& devices)
{
    HRESULT hres;

    IEnumWbemClassObject* pEnumerator = NULL;
    hres = pSvc->ExecQuery(L"WQL",
        L"SELECT * FROM Win32_NetworkAdapter WHERE PhysicalAdapter=true AND NetEnabled=false",
        WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, NULL, &pEnumerator);

    if (FAILED(hres))
    {
        NX_WARNING(QnLog::HWID_LOG,
            lm("Unable to get disabled NICS, code=%1: %2")
...
HRESULT ExecuteMethodAtPaths(
    IWbemServices* pSvc, const TCHAR* methodName, const std::vector<_bstr_t>& paths)
{
    HRESULT hres = S_OK;

    for (const _bstr_t& path : paths)
    {
        NX_VERBOSE(QnLog::HWID_LOG,
            lm("ExecMethod '%1' is called for %2").args(methodName, containerString(paths)));
        ...
        {
            NX_WARNING(QnLog::HWID_LOG,
                lm("ExecMethod '%1' for '%2' has failed, code=%3: %3")
        ...
        NX_VERBOSE(QnLog::HWID_LOG,
            lm("GetCallStatus '%1' is called for %2").args(methodName, containerString(paths)));
        ...
        {
            NX_WARNING(QnLog::HWID_LOG,
                lm("GetCallStatus '%1' for %2 has failed, code=%3: %4")
                    .args(methodName, path, hres, _com_error(hres).ErrorMessage()));
        }
    }

    NX_DEBUG(QnLog::HWID_LOG,
        lm("Method '%1' for %2 returned %3").args(methodName, containerString(paths), hres));
    return hres;
}
...
static QString execQueryForHWID1(IWbemServices* pSvc, const BSTR fieldName, const BSTR objectName)
{
    ...
    IEnumWbemClassObject* pEnumerator = NULL;
    hres = pSvc->ExecQuery(_T("WQL"), reqStr,
        WBEM_FLAG_FORWARD_ONLY | WBEM_FLAG_RETURN_IMMEDIATELY, NULL, &pEnumerator);
...
typedef QString (*ExecQueryFunction)(
    IWbemServices* pSvc, const BSTR fieldName, const BSTR objectName);

static void fillHardwareInfo(
    IWbemServices* pSvc, ExecQueryFunction execQuery, QnHardwareInfo& hardwareInfo)
{
    ...
    hardwareInfo.memorySerialNumber =
    ...
    HardwareIdListType& hardwareIds, QnHardwareInfo& hardwareInfo)
{
    ...
    if (FAILED(hres))
    {
        ...
        {
            std::ostringstream os;
            os << "Failed to initialize COM library. Error code = 0x" << std::ios_base::hex
               << hres;
    ...
    {
        std::ostringstream os;
        os << "Failed to initialize security. Error code = 0x" << std::ios_base::hex << hres;
        if (needUninitialize)
            CoUninitialize();
        throw HardwareIdError(os.str());
    }
    ...
    if (FAILED(hres))
    {
        ...
        os << "Failed to create IWbemLocator object."
           << " Err code = 0x" << std::ios_base::hex << hres;
        if (needUninitialize)
            CoUninitialize();
        throw HardwareIdError(os.str());
    }
    ...
    {
        std::ostringstream os;
        os << "Could not connect. Error code = 0x" << std::ios_base::hex << hres;
        pLoc->Release();
        if (needUninitialize)
            CoUninitialize();
        throw HardwareIdError(os.str());
    ...
    {
        std::ostringstream os;
        os << "Could not set proxy blanket. Error code = 0x" << std::ios_base::hex << hres;
        pSvc->Release();
        pLoc->Release();
        if (needUninitialize)
            CoUninitialize();
    ...
    if (GetDisabledNICSWithEmptyMac(pSvc, paths, devices) == S_OK)
    {
        if (!paths.empty())
        {
            ...
            {
#if 1
                // Wait up to kInterfaceWaitingTries * kInterfaceWaitingTime milliseconds for all
                // interfaces to be enabled
                for (int i = 0; i < kInterfaceWaitingTries; i++)
                {
                    findMacAddresses(pSvc, devices);

                    int emptyMacs = std::count_if(devices.cbegin(), devices.cend(),
                        [](const auto& item) { return item.second.mac.isEmpty(); });
                    if (emptyMacs == 0)
                    {
                        break;
                    }
