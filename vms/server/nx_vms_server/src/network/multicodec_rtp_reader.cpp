...
QnAbstractMediaDataPtr QnMulticodecRtpReader::getNextData()
{
    ...
    if (isRtpFail || elapsed > m_rtpFrameTimeoutMs)
    {
        ...
        if (elapsed > m_rtpFrameTimeoutMs) {
            ...
            NX_WARNING(this, "%1: RTP read timeout. Reopen stream", m_logName);
        }
        else {
...
QnAbstractMediaDataPtr QnMulticodecRtpReader::getNextDataUDP()
{
    ...
    while (m_RtpSession.isOpened() && !m_pleaseStop)
    {
        if (m_dataTimer.elapsed() > m_rtpFrameTimeoutMs)
        {
            NX_DEBUG(this, "%1: Failed to read from UDP socket, timeout expired: %2 ms",
                m_logName, m_rtpFrameTimeoutMs);
            break;
        }
        while (m_gotData)
        {
