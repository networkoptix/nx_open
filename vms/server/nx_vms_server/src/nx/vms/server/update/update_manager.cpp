...
void UpdateManager::finish()
{
    ...
    globalSettings()->setInstalledPersistentUpdateStorage(
        globalSettings()->targetPersistentUpdateStorage());
...
downloader::ResultCode UpdateManager::addDownload(
...
void UpdateManager::managePersistentDownloads()
{
    const auto variants = {
        std::make_pair(globalSettings()->targetUpdateInformation(), update::kTargetKey),
        std::make_pair(globalSettings()->installedUpdateInformation(), update::kInstalledKey)
    };
    ...
    for (const auto& p: variants)
    {
        ...
        if (!persistentServers.servers.contains(moduleGUID()))
            continue;
    ...
    for (const auto& package: packages)
    {
        ...
        if (!hasFile)
        {
            addDownload(package, QDir(storageData->url).absoluteFilePath(
                "downloads/" + nx::update::rootUpdatesDirectoryForDownloader()));
        }
    }
...
void UpdateManager::setUpdatePersistentStorageServers(
    const QList<QnUuid>& serverList, const QString& version, bool manuallySet)
{
    ...
    if (version == update::kTargetKey)
        globalSettings()->setTargetPersistentUpdateStorage(QJson::serialized(updateStorage));
    else
        globalSettings()->setInstalledPersistentUpdateStorage(QJson::serialized(updateStorage));

    globalSettings()->synchronizeNow();
}

update::PersistentUpdateStorage UpdateManager::updatePersistentStorageServers(
    const QString& version) const
{
    QByteArray data;
    if (version == update::kTargetKey)
        data = globalSettings()->targetPersistentUpdateStorage();
    else
        data = globalSettings()->installedPersistentUpdateStorage();

    auto result = QJson::deserialized<update::PersistentUpdateStorage>(data);
...
QnStorageSpaceDataList UpdateManager::availableStorages() const
{
