...
ftplib::ftplib()
{
    ...
    if (WSAStartup(MAKEWORD(1, 1), &wsa))
    {
        ...
        throw std::runtime_error("Network init error");
    }
    #endif
    ...
    if (mp_ftphandle == NULL)
        throw std::runtime_error("Ftp handle allocation failed");
        //perror("calloc");
...
int ftplib::FtpAcceptConnection(ftphandle *nData, ftphandle *nControl)
{
    ...
    else
    {
        ...
        {
            l = sizeof(addr);
            sData =(int)accept(nData->handle, &addr, &l);
            i = errno;
            net_close(nData->handle);
            if (sData > 0)
            {
...
int ftplib::FtpOpenPort(ftphandle *nControl, ftphandle **nData, transfermode mode, int dir, char *cmd)
{
    ...
    sData = (int)socket(PF_INET,SOCK_STREAM,IPPROTO_TCP);
    if (sData == -1)
    {
        //perror("socket");
        return -1;
    ...
    {
    char buf[256];
    sprintf(buf,"REST %lld", (long long int) mp_ftphandle->offset);
...
int ftplib::TestControlConnection()
{
    if (!FtpSendCmd("PASV",'2', mp_ftphandle))
        return 0;
    return 1;
}
