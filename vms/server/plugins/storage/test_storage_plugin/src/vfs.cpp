...
{
    WCHAR buf[2048];
    MultiByteToWideChar(CP_UTF8, 0, fileName.c_str(), (int) fileName.size(), buf, 2048);

    HANDLE hFile = CreateFileW(
        buf, GENERIC_READ, 0, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
    if (!hFile)
        return 0;
...
bool utils::buildVfsFromJson(const char* jsonString, const char* rootPath, VfsPair* outVfsPair)
{
    ...
    if (*errorBuf != 0)
    {
        LOG("[TestStorage, JSON]: failed to parse json string: %s", jsonString);
        return false;
    }
