...
int AxisMediaEncoder::getMediaUrl( char* urlBuf ) const
{
    ...
    snprintf(urlBuf, nxcip::MAX_TEXT_LEN,
        "rtsp://%s:%d/axis-media/media.amp?%s", QByteArray(m_cameraManager->cameraInfo().url).split(':').at(0).constData(), m_rtspPort, paramsStr.data() );
