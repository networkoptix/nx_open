...
bool EventFilter::match(const EventPtr& event) const
{
    ...
    // TODO: #amalov The current way of cache access introduced to not disrupt existing tests.
    // Think of a better way, probably filter->rule->engine pointer chain.
    auto& eventCache = Engine::instance()->eventCache();
    const auto cacheKey = event->cacheKey();

    if (eventCache.eventWasCached(cacheKey))
    {
    ...
    eventCache.cacheEvent(cacheKey);

    // Assuming all events with the same resourceKey() will be generated by the same peer,
    // so it may be checked on the origin side.
    if (!eventCache.checkEventState(event))
        return false;
