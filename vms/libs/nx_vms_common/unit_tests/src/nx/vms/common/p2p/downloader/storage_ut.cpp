...
protected:
    ...
    {
        workingDirectory.setPath(testDir());
    ...
    {
        const auto fullFilePath = workingDirectory.absoluteFilePath(fileName);
        utils::createRandomFile(fullFilePath, size);
...
TEST_F(DistributedFileDownloaderStorageTest, findDownloadsForDownloadedFiles)
{
    ...
    downloaderStorage.loadExistingDownloads(true);
...
TEST_F(DistributedFileDownloaderStorageTest, foundFileFullPath)
{
    ...
    storage.loadExistingDownloads(true);
