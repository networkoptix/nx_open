...
static const auto kOwnerRoleId = QnUserRolesManager::predefinedRoleId(Qn::UserRole::owner);
static const auto kAdminRoleId = QnUserRolesManager::predefinedRoleId(Qn::UserRole::administrator);
static const auto kViewerRoleId = QnUserRolesManager::predefinedRoleId(Qn::UserRole::viewer);
...
TEST_F(ResourceAccessSubjectsCacheTest, PredefinedRoles)
{
    ...
    u2->setRawPermissions(GlobalPermission::viewerPermissions);
    EXPECT_EQ(cache()->allSubjects().size(), 4);
    EXPECT_EQ(cache()->allUsersInRole(kOwnerRoleId).size(), 1);
    EXPECT_EQ(cache()->allSubjectsInRole(kOwnerRoleId).size(), 1);
    EXPECT_EQ(cache()->allUsersInRole(kAdminRoleId).size(), 0);
...
{
    const auto user = addUser(GlobalPermission::admin, "user");
    EXPECT_EQ(cache()->allSubjects().size(), 1);
    EXPECT_EQ(cache()->allUsersInRole(kAdminRoleId).size(), 1);
    EXPECT_EQ(cache()->allSubjectsInRole(kAdminRoleId).size(), 1);
    EXPECT_EQ(cache()->allUsersInRole(kViewerRoleId).size(), 0);
    ...
    QnUserResourcePtr userCopy(new QnUserResource(user->userType()));
    ec2::fromApiToResource(userData, userCopy);
    userCopy->setRawPermissions(GlobalPermission::viewerPermissions);
