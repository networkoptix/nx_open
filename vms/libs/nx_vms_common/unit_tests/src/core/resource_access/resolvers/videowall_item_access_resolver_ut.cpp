...
TEST_F(VideowallItemAccessResolverTest, videowallMatrixLayouts)
{
    manager->setOwnResourceAccessMap(kTestSubjectId, {{kAllVideoWallsGroupId, kTestAccessRights}});
    NX_ASSERT_TEST_SUBJECT_CHANGED();
    ...
    const auto item3 = addVideoWallItem(videowall, {});
    ...
    videowall->matrices()->addItem(matrix);
    NX_ASSERT_TEST_SUBJECT_CHANGED();
    ASSERT_EQ(resolver->accessRights(kTestSubjectId, layout1), AccessRight::view);
    ASSERT_EQ(resolver->accessRights(kTestSubjectId, layout2), AccessRight::view);
    ASSERT_EQ(resolver->accessRights(kTestSubjectId, layout3), AccessRight::view);

    matrix.layoutByItem[item2] = {};
