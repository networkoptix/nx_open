...
inline void checkMarkerBit(BitStreamReader& reader, const std::string& description)
{
    if (!reader.getBit())
        throw BitStreamException("Invalid marker bit: " + description);
}
...
bool VideoObjectLayer::read(const uint8_t* data, int size)
{
    ...
    catch (const BitStreamException& ex)
    {
