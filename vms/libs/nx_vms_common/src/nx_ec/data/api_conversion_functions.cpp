...
void fromResourceToApi(const QnScheduleTask& src, ScheduleTaskData& dst)
{
    ...
    dst.metadataTypes = src.metadataTypes;
}
...
void fromApiToResource(const CameraAttributesData& src, const QnCameraUserAttributesPtr& dst)
{
    ...
    dst->backupContentType = src.backupContentType;
    dst->backupPolicy = src.backupPolicy;
}
    ...
    QnCameraUserAttributePool* attributesPool)
{
    ...
    //TODO #akolesnikov using QnCameraUserAttributePool here is not good
    QnCameraUserAttributesPtr attributes(new QnCameraUserAttributes);
    fromApiToResource(static_cast<const CameraAttributesData&>(src), attributes);
    attributesPool->update(dst->getId(), *attributes);
    ...
    QnCameraUserAttributePool* attributesPool)
{
    ...
    //TODO #akolesnikov using QnCameraUserAttributePool here is not good
    auto attributes = attributesPool->getCopy(src->getId());
    fromResourceToApi(attributes, static_cast<CameraAttributesData&>(dst));
...
{
    for (const auto& part: source.split(L';', Qt::SkipEmptyParts))
    {
