    ...
    QThread* thread)
{
    ...
    static const QString kUniqueIdParam("uniqueId");
    ...
    for (int i = 0; i < cameras.size(); i++)
    {
        ...
        parameters.insert(kUniqueIdParam + number, camera.uniqueId);
    }
    ...
    const QUrl& url)
{
    ...
    {
        // No direct route exists, proxy via the nearest server.
        auto nearestServer = commonModule->currentServer();
        if (!nearestServer)
            return false;

        nx::utils::Url nearestUrl(server->getApiUrl());
        if (nearestServer->getId() == commonModule->moduleGUID()) //< Server-side option.
