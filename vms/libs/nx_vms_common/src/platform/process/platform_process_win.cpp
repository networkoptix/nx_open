...
void QnWindowsProcess::setPriority(Priority priority) {
    Q_D(QnWindowsProcess);
    if(!d->valid) {
        NX_ASSERT(false, "Process is not valid, could not set priority.");
        return;
    }

    DWORD systemPriority = d->qnToSystemPriority(priority);
    if(systemPriority == INVALID_PRIORITY_CLASS) {
        NX_ASSERT(false, "Invalid process priority value '%1'.", static_cast<int>(priority));
        return;
    }

    if(SetPriorityClass(d->handle, systemPriority) == 0) {
        NX_WARNING(this, "Could not set priority for process.");
        return;
    }
}
