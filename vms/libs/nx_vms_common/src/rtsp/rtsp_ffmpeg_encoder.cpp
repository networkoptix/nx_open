...
bool QnRtspFfmpegEncoder::getNextPacket(QnByteArray& sendBuffer)
{
    ...
    nx::streaming::rtp::buildRtpHeader(
        ...
        rtpMarker,
        m_media->timestamp,
        nx::streaming::rtp::kNxPayloadType,
        m_sequence++);
...
QString QnRtspFfmpegEncoder::getSdpMedia(bool isVideo, int trackId, int port)
{
    ...
    stream << nx::streaming::rtp::kNxPayloadType << "\r\n";
    stream << "a=control:trackID=" << trackId << "\r\n";
    stream << "a=rtpmap:" << nx::streaming::rtp::kNxPayloadType << " " << kNxPayloadTypeName <<
        "/" << 1'000'000 <<"\r\n";

    if (!m_codecParamsData.isEmpty())
    {
        stream << "a=fmtp:" << nx::streaming::rtp::kNxPayloadType << " config=" <<
            m_codecParamsData.toBase64() <<"\r\n";
    }
