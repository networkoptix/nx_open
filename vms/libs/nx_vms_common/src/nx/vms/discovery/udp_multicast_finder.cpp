    ...
    const nx::vms::api::ModuleInformationWithAddresses& information)
{
    m_updateTimer.post(
        ...
        {
            NX_DEBUG(this, nx::format("Set module information: %1").arg(information));
...
void UdpMulticastFinder::addNewSenders()
{
    for (const auto& ip: nx::network::getLocalIpV4HostAddressList())
    {
        ...
        if (insert.second)
        {
            insert.first->second = makeSocket({ip, 0});
            if (insert.first->second)
            {
                joinMulticastGroup(ip);
                if (!m_ownModuleInformation.empty())
...
void UdpMulticastFinder::updateInterfacesInAioThread()
{
    ...
    NX_DEBUG(this, nx::format("Update interfaces to %1").container(localIpList));
    removeObsoleteSenders();
    addNewSenders();

    NX_VERBOSE(this, nx::format("Schedule update interfaces in %1").arg(m_updateInterfacesInterval));
...
std::unique_ptr<network::UDPSocket> UdpMulticastFinder::makeSocket(const nx::network::SocketAddress& endpoint)
{
    ...
    {
        NX_DEBUG(this, nx::format("New socket %1").arg(socket->getLocalAddress()));
        return socket;
    }

    NX_DEBUG(this, nx::format("Failed to create socket %1: %2").args(
...
void UdpMulticastFinder::joinMulticastGroup(const nx::network::HostAddress& ip)
{
    ...
    {
        NX_DEBUG(this, nx::format("Joined group %1 on %2").args(m_multicastEndpoint.address, ip));
        return; //< Ok.
    }
...
void UdpMulticastFinder::sendModuleInformation(Senders::iterator senderIterator)
{
    ...
    {
        NX_DEBUG(this, nx::format("Multicasts are disabled by function"));
    ...
    socket->sendToAsync(&m_ownModuleInformation, multicastEndpoint,
            ...
            SystemError::ErrorCode code, nx::network::SocketAddress destination, size_t)
        {
            if (code == SystemError::noError)
            {
                NX_VERBOSE(this, nx::format("Successfully sent from %1 to %2").args(
            ...
            {
                NX_WARNING(this, nx::format("Failed to send from %1 to %2: %3").args(
