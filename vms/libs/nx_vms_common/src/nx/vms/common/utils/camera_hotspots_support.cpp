...
bool supportsCameraHotspots(const QnResourcePtr& cameraResource)
{
    ...
    const auto allowedPtzCapabilities =
        Ptz::Capability::ContinuousFocusCapability | Ptz::Capability::ContinuousFocusCapability;

    const auto operationalPtzCapabilities =
        camera->getPtzCapabilities(nx::vms::common::ptz::Type::operational);

    if ((operationalPtzCapabilities & ~allowedPtzCapabilities) != Ptz::Capability::NoPtzCapabilities)
        return false;
