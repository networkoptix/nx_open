    ...
    float* outValue) const
{
    ...
    if (lineStream.fail() || labelFromLine != expectedLabel)
        return warning(nx::format("Missing label \"%1\"").args(expectedLabel));

    lineStream >> *outValue;
    if (lineStream.fail())
        return warning(nx::format("Invalid float value for label \"%1\"").args(expectedLabel));

    // Skip trailing ",", if any.
    std::string trailing;
    lineStream >> trailing;
    if (!trailing.empty() && trailing != ",")
        return warning(nx::format("Unexpected trailing after label \"%1\"").args(expectedLabel));
    ...
    QRectF* outRect) const
{
    ...
    if (!parseFloat(lineStream, line, "x", &x))
        return false;
    if (x < 0 || x > 1)
        return warning(nx::format("\"x\" should be in range [0..1], but is %2").args(x));

    if (!parseFloat(lineStream, line, "y", &y))
    if (y < 0 || y > 1)
        return warning(nx::format("\"y\" should be in range [0..1], but is %2").args(y));

    if (!parseFloat(lineStream, line, "width", &width))
        return false;
    if (width < 0 || x + width > 1)
        return warning(nx::format("\"width\" should be in range [0..1 - x], but is %2").args(width));
    ...
    int* outObjectCount) const
{
    ...
    if (lineStream.fail())
        return warning(nx::format("Missing label \"%1\"").args(kMetadataTimestampMsLabel));

    lineStream >> *outMetadataTimestampMs;
    if (lineStream.fail())
        return warning(nx::format("Invalid int64 value for label \"%1\"").args(kMetadataTimestampMsLabel));
    ...
    for (;;)
    {
        ...
        if (label.empty() || !lineStream)
            return warning(nx::format("Missing label \"%1\"").args(kObjectsLabel));
    }

    lineStream >> *outObjectCount;
    if (lineStream.fail())
        return warning(nx::format("Invalid object count value after label \"%1\"").args(kObjectsLabel));

    // Skip trailing ":", if any.
    std::string trailing;
    lineStream >> trailing;
    if (!trailing.empty() && trailing != ":")
        return warning(nx::format("Unexpected trailing after label \"%1\"").args(kObjectsLabel));
    ...
    while (std::getline(f, packetLineString))
    {
        ...
        for (int i = 0; i < objectCount; ++i)
        {
            ...
            if (!std::getline(f, rectLineString))
                return error(nx::format("Unable to read line %1 for rect #%2").args(line, objectCount));
...
bool MetadataLogParser::saveToFile(const QString& filename) const
{
    ...
    for (const auto& packet: m_packets)
    {
        ...
        {
            logLine(nx::format("    x %1, y %2, width %3, height %4").args(
