...
int QnRtspClient::readBinaryResponse(quint8* data, int maxDataSize)
{
    ...
    {
        while (m_responseBufferLen < sizeof(FrameHeader))
        {
            const int bytesRead = readSocketWithBuffering(m_responseBuffer + m_responseBufferLen,
                /*bufSize*/ sizeof(FrameHeader) - m_responseBufferLen,
                /*readSome*/ true);
            if (bytesRead <= 0)
                return bytesRead;
            m_responseBufferLen += bytesRead;
    ...
    if (m_responseBufferLen < sizeof(FrameHeader) || '$' != m_responseBuffer[0])
        return 0;
