...
QSet<QnResourcePtr> QnLayoutResource::layoutResources(QnResourcePool* resourcePool,
...
{
    const auto items = layoutResources();

    NX_DEBUG(this, "Layout Url=%1, ID=%2", getUrl(), getId());

    for (const auto &item: items)
    {
        NX_DEBUG(this, "  Item Resource  Url=%1, ID=%2", item->getUrl(), item->getId());
    }
}
