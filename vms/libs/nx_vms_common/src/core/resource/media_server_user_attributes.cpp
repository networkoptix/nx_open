    ...
    Notifiers& notifiers)
{
    if (m_data.allowAutoRedundancy != right.m_data.allowAutoRedundancy)
        notifiers.push_back([](auto server) { emit server->redundancyChanged(server); });

    if (m_data.serverName != right.m_data.serverName)
        notifiers.push_back([](auto server) { emit server->nameChanged(server); });

    if (m_data.backupBitrateBytesPerSecond != right.m_data.backupBitrateBytesPerSecond)
        notifiers.push_back([](auto server) { emit server->backupBitrateLimitsChanged(server); });
