...
Request HTTPClient::updateRequest(const Request& srcRequest)
{
    ...
    QString auth = QLatin1String(createHttpQueryAuthParam(request.auth.user(), request.auth.password(), authInfo.realm, srcRequest.method.toStdString(), nonce));
...
void HTTPClient::updateAuthParams(const QUuid& serverId, const Response& response)
{
    for (const auto& header: response.headers)
    {
        ...
        {
            auto params = parseAuthData(header.second.mid(header.second.indexOf(' ')).toUtf8(), ',');
