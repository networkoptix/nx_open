...
inline __m128 CalcWeights(float x, float y)
{
    ...
    __m128 floorValue = _mm_cvtepi32_ps(_mm_cvtps_epi32(value));
...
static inline quint8 GetPixelImpl(quint8* buffer, int stride, float x, float y, int width, int height)
{
...
quint8 QnDewarpingImageFilter::GetPixel(
    quint8* buffer, int stride, float x, float y, int width, int height)
{
    SseRoundTowardZero mode;
    return GetPixelImpl(buffer, stride, x, y, width, height);
}
    ...
    for (int plane = 0; plane < descr->nb_components && outputFrame->data[plane]; ++plane)
    {
        ...
        for (int y = 0; y < h; ++y)
        {
            ...
            for (int x = 0; x < w; ++x)
            {
                ...
                quint8 pixel = GetPixelImpl(frame->data[plane],
...
void QnDewarpingImageFilter::updateDewarpingParameters(const QSize& imageSize, qreal aspectRatio)
{
...
{
    updateDewarpingParameters(imageSize, aspectRatio);
