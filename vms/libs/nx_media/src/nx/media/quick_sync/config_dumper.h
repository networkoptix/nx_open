...
#define SERIALIZE_INFO_ELEMENT(arrName,name) {sstr<<prefix<<(#arrName)<<("[")<<arrIdx<<("].")<<(#name)<<(":")<<info.arrName[arrIdx].name<<std::endl;}
#define SERIALIZE_INFO_ARRAY_ELEMENT(arrName,name) {sstr<<prefix<<(#arrName)<<("[")<<arrIdx<<("].")<<(#name)<<(":");for(unsigned int idx=0;idx<(sizeof(info.arrName[0].name)/sizeof(info.arrName[0].name[0]));idx++){sstr<<info.arrName[arrIdx].name[idx]<<" ";};sstr<<std::endl;}
#define END_PROC_ARRAY }
#define SERIALIZE_INFO(name) {sstr<<prefix<<(#name)<<":"<<info.name<<std::endl;}
#define SERIALIZE_INFO_ARRAY(name) {sstr<<prefix<<(#name)<<(":");for(unsigned int idx=0;idx<(sizeof(info.name)/sizeof(info.name[0]));idx++){sstr<<info.name[idx]<<" ";};sstr<<std::endl;}
#define SERIALIZE_INFO_MEMORY(name,numElems) {sstr<<prefix<<(#name)<<(":");for(unsigned int idx=0;idx<info.numElems;idx++){sstr<<info.name[idx]<<" ";};sstr<<std::endl;}

inline void dumpFrameInfoStruct(std::ostream& sstr,const std::string& prefix, mfxFrameInfo& info) {
    ...
    char strFourCC[5]={};
    memcpy(strFourCC,&info.FourCC,4);
...
inline void dumpMfxInfoMFXStruct(std::ostream& sstr,const std::string& prefix, mfxInfoMFX& info) {
    SERIALIZE_INFO_ARRAY(reserved);

    SERIALIZE_INFO(LowPower);
    SERIALIZE_INFO(BRCParamMultiplier);

    dumpFrameInfoStruct(sstr, prefix + "FrameInfo.", info.FrameInfo);
    SERIALIZE_INFO(CodecId);
    char strID[5]={};
    memcpy(strID,&info.CodecId,4);
    ...
    //SERIALIZE_INFO(EnableReallocRequest); // not supported at 2017R3
    ...
    //SERIALIZE_INFO_ARRAY(SamplingFactorH); // not supported at 2017R3
    //SERIALIZE_INFO_ARRAY(SamplingFactorV); // not supported at 2017R3
...
void SerializeVPPCompInputStream(std::ostream& sstr,std::string prefix,mfxVPPCompInputStream& info)
{
    SERIALIZE_INFO(DstX);
    SERIALIZE_INFO(DstY);
    SERIALIZE_INFO(DstW);
...
void SerializeExtensionBuffer(std::ostream& sstr,std::string prefix,mfxExtBuffer* pExtBuffer)
{
    char name[6]="    .";
    memcpy(name,&pExtBuffer->BufferId,4);
    std::string strName(name);
    prefix+=std::string(strName.begin(),strName.end());
    ...
    case MFX_EXTBUFF_VPP_COMPOSITE:
        {
            ...
            {
                char streamName[32];
                sprintf(streamName,("InputStream[%d]."),i);
...
void SerializeVideoParamStruct(std::ostream& sstr,std::string sectionName,mfxVideoParam& info,bool shouldUseVPPSection)
{
    std::string prefix;
    ...
    if(shouldUseVPPSection)
    {
        ...
        dumpFrameInfoStruct(sstr,"vpp.In.",info.vpp.In);
        dumpFrameInfoStruct(sstr,"vpp.Out.",info.vpp.Out);
    }
    else
    {
...
inline void dumpVideoParamStruct(std::ostream& sstr,const std::string& sectionName, mfxVideoParam& info) {
    std::string prefix;
    ...
    SERIALIZE_INFO(reserved3);
    SERIALIZE_INFO(AsyncDepth);
    dumpMfxInfoMFXStruct(sstr, "mfx.",info.mfx);
