...
void Sound::clearBuffers()
{
    ...
    // loop until we delete all buffers.
    for (;;)
    {
        ALint remaining = 0;

        // When the source is stopped, all buffers are in PROCESSED state.
        alGetSourcei(m_source, AL_BUFFERS_PROCESSED, &remaining);

        if (remaining <= 0)
            return;

        const auto unqueueCount = std::min(remaining, kMaxUnqueueCount);
        ...
        alDeleteBuffers(unqueueCount, m_tmpBuffer);
        checkOpenALErrorDebug(m_device);

        if (unqueueCount >= remaining)
            return;
    }
}
