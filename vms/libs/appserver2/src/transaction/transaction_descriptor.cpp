...
struct ModifyCameraAttributesAccess
{
        ...
        const nx::vms::api::CameraAttributesData& param)
    {
        ...
        if (camera)
        {
            ...
            // can be not changed.
            nx::vms::api::CameraAttributesData origin;
            ec2::fromResourceToApi(
                commonModule->cameraUserAttributesPool()->getCopy(param.cameraId), origin);
            if (origin == param)
                return Result();
        }
        else
        ...
        if (param.scheduleEnabled && !camera->isLicenseUsed())
        {
            ...
            CamLicenseUsageHelper licenseUsageHelper(commonModule, /*watchCameraChanges*/ false);
        ...
        const auto isServer =
            [resourcePool = commonModule->resourcePool(), db, &id = params.id]() -> bool
            {
                if (QnResourcePtr resource = resourcePool->getResourceById<QnResource>(id))
                    return resource.dynamicCast<QnMediaServerResource>();
