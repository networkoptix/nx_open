    ...
    std::unique_ptr<nx::network::AbstractCommunicatingSocket> socket)
{
            ...
            duration_cast<milliseconds>(kSocketSendTimeout).count()))
    {
        ...
        NX_DEBUG(QnLog::EC2_TRAN_LOG.join(this),
            nx::format("Error setting socket write timeout for transaction connection %1 received from %2")
    ...
    const nx::utils::Url& remotePeerUrl)
{
    NX_VERBOSE(QnLog::EC2_TRAN_LOG.join(this),
        nx::format("doOutgoingConnect. remotePeerUrl = %1").arg(remotePeerUrl));
...
{
    NX_DEBUG(QnLog::EC2_TRAN_LOG.join(this),
        nx::format("Connection to peer %1 canceled from state %2")
            .args(m_remotePeer.id.toString(), toString(getState())));
    setState(Error);
}
...
{
    NX_VERBOSE(QnLog::EC2_TRAN_LOG.join(this),
        nx::format("onSomeBytesRead. errorCode = %1, bytesRead = %2")
            .args((int)errorCode, bytesRead));
    ...
    if (errorCode || bytesRead == 0)   //error or connection closed
    {
        ...
        {
            NX_DEBUG(QnLog::EC2_TRAN_LOG.join(this), nx::format("Peer %1 timed out. Disconnecting..."),
    ...
    {
        NX_VERBOSE(QnLog::EC2_TRAN_LOG.join(this), nx::format("Connection to %1 is closed. Not reading anymore")
    ...
    {
        NX_DEBUG(QnLog::EC2_TRAN_LOG.join(this), lit("Error parsing data from peer %1. Disconnecting...").
    ...
    const nx::ConstBufferRefType& tranDataWithHeader)
{
    ...
    switch (m_remotePeer.dataFormat)
    {
        case Qn::JsonFormat:
            ...
            {
                NX_ASSERT(false);
                NX_WARNING(QnLog::EC2_TRAN_LOG.join(this), lit("Error deserializing JSON data from peer %1. Disconnecting...").
                    arg(m_remotePeer.id.toString()));
        ...
        case Qn::UbjsonFormat:
            ...
            {
                NX_ASSERT(false);
                NX_WARNING(QnLog::EC2_TRAN_LOG.join(this), lit("Error deserializing Ubjson data from peer %1. Disconnecting...").
                    arg(m_remotePeer.id.toString()));
                setStateNoLock(State::Error);
                return;
            }
        ...
        default:
            NX_WARNING(QnLog::EC2_TRAN_LOG.join(this), lit("Received unkown format from peer %1. Disconnecting...").
                arg(m_remotePeer.id.toString()));
            setStateNoLock(State::Error);
            return;
    }
    ...
    const nx::ConstBufferRefType& tranData)
{
            ...
            Qn::EC2_BASE64_ENCODING_REQUIRED_HEADER_NAME) == "true")
    {
        ...
        {
            NX_WARNING(QnLog::EC2_TRAN_LOG.join(this), lit("Error parsing data (2) from peer %1. Disconnecting...").
                arg(m_remotePeer.id.toString()));
            return setStateNoLock(State::Error);
        }
    }
    ...
    const QByteArray& requestBuf)
{
    ...
    {
        NX_WARNING(QnLog::EC2_TRAN_LOG.join(this), lit("Error parsing incoming data (3) from peer %1. Disconnecting...").
            arg(m_remotePeer.id.toString()));
        return setStateNoLock(State::Error);
    }
...
{
    NX_DEBUG(QnLog::EC2_TRAN_LOG.join(this),
        nx::format("Connection to peer %1 failure. Disconnecting...")
        .args(m_remotePeer.id.toString()));
    setState(Error);
}
    ...
    size_t bytesRead)
{
    ...
    {
        NX_DEBUG(QnLog::EC2_TRAN_LOG.join(this),
            nx::format("transaction connection %1 received from %2 failed: %3")
    ...
    {
        NX_DEBUG(QnLog::EC2_TRAN_LOG.join(this),
            nx::format("transaction connection %1 received from %2 has been closed by remote peer")
            .args(m_connectionGuid, m_outgoingDataSocket->getForeignAddress().toString()));
        return setStateNoLock(State::Error);
    }
...
void QnTransactionTransportBase::serializeAndSendNextDataBuffer()
{
    ...
    NX_VERBOSE(QnLog::EC2_TRAN_LOG.join(this), lit("Sending data buffer (%1 bytes) to the peer %2").
    ...
    size_t bytesSent)
{
    ...
    if (errorCode)
    {
        NX_DEBUG(QnLog::EC2_TRAN_LOG.join(this), nx::format("Failed to send %1 bytes to %2. %3")
            .args(m_dataToSend.front().encodedSourceData.size(),
    ...
    const nx::network::http::AsyncHttpClientPtr& client)
{
    ...
    NX_VERBOSE(QnLog::EC2_TRAN_LOG.join(this),
        nx::format("at_responseReceived. statusCode = %1").arg(statusCode));
...
{
    NX_VERBOSE(QnLog::EC2_TRAN_LOG.join(this), "at_httpClientDone");
...
void QnTransactionTransportBase::processTransactionData(const nx::Buffer& data)
{
    ...
    {
        NX_WARNING(QnLog::EC2_TRAN_LOG.join(this), lit("Error processing incoming data (4) from peer %1. Disconnecting...").
            arg(m_remotePeer.id.toString()));
        return setStateNoLock(State::Error);
    }
}
