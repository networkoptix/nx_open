    ...
    const SoftwareVersion& version)
{
    ...
    RegisterSystemUriProtocolHandlerResult result { .success=true };

    result.previousVersion = SoftwareVersion(registryEditor.value(kVersionKey).toString());

    if (result.previousVersion >= version)
        return result; //< Handler already registered.
...
bool runAsAdministratorOnWindows(const QString& applicationBinaryPath,
