...
bool AdtsHeader::decodeFromFrame(const uint8_t* const buffer, int bufferSize)
{
    ...
    try
    {
        nx::utils::BitStreamReader reader(buffer, buffer + bufferSize);
...
void AACCodec::buildADTSHeader(quint8* buffer, int frameSize)
{
    ...
    for (uint i = 0; i < std::size(aac_sample_rates); i++)
    ...
    m_channels_index = 0;
    for (uint i = 0; i < std::size(aac_channels); i++)
...
{
    try {
        nx::utils::BitStreamReader reader((const quint8*) data.data(), (const quint8*) data.data() + data.size());
