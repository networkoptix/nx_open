...
void NALUnit::write_rbsp_trailing_bits(nx::utils::BitStreamWriter& writer)
{
    writer.putBit(1);
...
void NALUnit::write_byte_align_bits(nx::utils::BitStreamWriter& writer)
{
    int rest = 8 - (writer.getBitsCount() & 7);
    if (rest == 8)
        return;
...
void NALUnit::writeSEGolombCode(nx::utils::BitStreamWriter& bitWriter, qint32 value)
{
    if (value <= 0)
...
void NALUnit::writeUEGolombCode(nx::utils::BitStreamWriter& bitWriter, quint32 value)
{
    /*
    int maxVal = 0;
    int x = 2;
...
void SPSUnit::serializeHDRParameters(nx::utils::BitStreamWriter& writer)
{
...
bool SliceUnit::moveHeaderField(int fieldOffset, int newLen, int oldLen)
{
    ...
    if (bitDiff > 0)
    {
        if (pps->entropy_coding_mode_flag)
        {
            ...
            nx::utils::moveBits(m_nalBuffer, fieldOffset, fieldOffset + bitDiff, m_fullHeaderLen-fieldOffset);
        ...
        else
        {
            ...
            nx::utils::moveBits(m_nalBuffer, fieldOffset, fieldOffset+bitDiff, oldLenInBits);
...
bool SliceUnit::increasePicOrderFieldLen(int newLen, int oldLen)
{
    ...
    if (bitDiff > 0)
    {
        if (pps->entropy_coding_mode_flag)
        {
            ...
            nx::utils::moveBits(m_nalBuffer, picOrderBitPos, picOrderBitPos + bitDiff, m_fullHeaderLen-picOrderBitPos);
        ...
        else
        {
            ...
            oldLenInBits -= trailingBits;
            int newLenInBits = oldLenInBits + bitDiff;
            nx::utils::moveBits(m_nalBuffer, picOrderBitPos, picOrderBitPos+bitDiff, oldLenInBits);
            m_nalBufferLen = newLenInBits / 8;
            if (newLenInBits % 8 == 0)
                m_nalBuffer[m_nalBufferLen] = 0x80; // trailing bits
            else {
...
int SliceUnit::serializeSliceHeader(nx::utils::BitStreamWriter& bitWriter, const QMap<quint32, SPSUnit*>& spsMap,
...
void SliceUnit::write_dec_ref_pic_marking(nx::utils::BitStreamWriter& bitWriter)
{
    if(nal_unit_type == nuSliceIDR)
    {
...
void SliceUnit::write_pred_weight_table(nx::utils::BitStreamWriter& bitWriter)
{
...
void SliceUnit::write_ref_pic_list_reordering(nx::utils::BitStreamWriter& bitWriter)
{
...
void SEIUnit::serialize_pic_timing_message(const SPSUnit& sps, nx::utils::BitStreamWriter& writer, bool seiHeader)
{
    if (seiHeader) {
...
void SEIUnit::serialize_buffering_period_message(const SPSUnit& sps, nx::utils::BitStreamWriter& writer, bool seiHeader)
{
    if (seiHeader) {
        writer.putBits(8, nuSEI);
