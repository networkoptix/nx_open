...
via inheriting and extending. Such types are called Base Library; their definition can be found in
the file `default_type_library.json` supplied with the SDK.
...
- `"flags"`: Flag set (String)
    ...
    - `nonIndexable` - Objects of such types will be stored in the video archive as usual, but will
        ...
        they will be visible when watching live video fro the camera (and only when the Objects do
