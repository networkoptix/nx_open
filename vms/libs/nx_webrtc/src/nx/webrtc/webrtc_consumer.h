...
public:
    ...
    using StatusHandler = std::function<void(StreamStatus)>;
...
public:
    ...
    void seek(int64_t timestampUs, StatusHandler&& handler);
    void changeQuality(nx::vms::api::StreamIndex stream, StatusHandler&& handler);
    void pause(StatusHandler&& handler);
    void resume(StatusHandler&& handler);
    void nextFrame(StatusHandler&& handler);
...
private:
    // Main.
    AbstractCameraDataProvider::OnDataReady m_dataConsumedHandler;
    ...
    std::deque<nx::Buffer> m_mediaBuffers;
