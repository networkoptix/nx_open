...
{
    QRect body = QnGeometry::eroded(geometry, bodyMargin);
    if (!isPartOfScreen())
        return body;
...
QRect QnVideowallManageWidgetPrivate::targetRect(const QRect& rect) const
{
    ...
    return QnGeometry::expanded(
        QnGeometry::aspectRatio(m_unitedGeometry), rect, Qt::KeepAspectRatio).toRect();
}
...
QnVideowallManageWidgetPrivate::ItemTransformations QnVideowallManageWidgetPrivate::
transformationsAnchor(const QRect& geometry, const QPoint& pos) const
{
    QRect leftAnchor(
        QnGeometry::dilated(
            QRect(geometry.left(), geometry.top(), 0, geometry.height()),
            transformationOffset));
    QRect centerXAnchor(
        QnGeometry::dilated(
            QRect(geometry.center().x(), geometry.top(), 0, geometry.height()),
            transformationOffset));
    QRect rightAnchor(
        QnGeometry::dilated(
            QRect(geometry.left() + geometry.width(), geometry.top(), 0, geometry.height()),
            transformationOffset));

    QRect topAnchor(
        QnGeometry::dilated(
            QRect(geometry.left(), geometry.top(), geometry.width(), 0),
            transformationOffset));
    QRect centerYAnchor(
        QnGeometry::dilated(
            QRect(geometry.left(), geometry.center().y(), geometry.width(), 0),
            transformationOffset));
    QRect bottomAnchor(
        QnGeometry::dilated(
            QRect(geometry.left(), geometry.top() + geometry.height(), geometry.width(), 0),
            transformationOffset));
    ...
    bool* multiScreen) const
{
    ...
    for (const auto& m_screen: m_screens)
    {
        ...
        for (const auto& part: m_screen.parts)
        {
            ...
            const auto overlapping = 1.0 * QnGeometry::area(intersected)
                / QnGeometry::area(part.geometry);
...
QList<T> bestMatchingGeometry(const QList<T>& source, const QRect& geometry, int count)
{
    ...
    std::partial_sort_copy(
        ...
        {
            return QnGeometry::area(left.geometry.intersected(geometry)) > QnGeometry::area(
