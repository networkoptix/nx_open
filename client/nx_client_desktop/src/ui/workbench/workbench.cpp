...
namespace nx_workbench = nx::client::desktop::ui::workbench;
...
void addLayoutCreators(nx::client::desktop::ui::workbench::LayoutsFactory* factory)
{
    const auto standardLayoutCreator =
        [](const QnLayoutResourcePtr& resource, QObject* parent) -> QnWorkbenchLayout*
        {
            ...
            return resource->data(Qn::IsSpecialLayoutRole).toBool()
                ? new nx_workbench::SpecialLayout(resource, parent)
    ...
    m_inLayoutChangeProcess(false)
{
    ...
    const auto factory = nx_workbench::LayoutsFactory::instance(this);
...
void QnWorkbench::update(const QnWorkbenchState &state) {
    ...
    {
        auto layout = resourcePool()->getResourceById<QnLayoutResource>(state.layoutUuids[i]);
        if (!layout)
            continue;
...
//             continue;
        ...
        if (layout->getId() == state.currentLayoutId)
            setCurrentLayout(workbenchLayout);
    }
...
//         if (resourcePool()->isAutoGeneratedLayout(resource))
//             continue;
