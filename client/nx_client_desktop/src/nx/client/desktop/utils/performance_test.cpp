...
void PerformanceTest::detectLightMode()
{
    ...
    if (nx::utils::AppInfo::isLinux())
    {
        ...
        NX_INFO(typeid(PerformanceTest), lm("QnPerformanceTest: CPU: \"%1\" poor: %2").arg(cpuName).arg(poorCpu));
        ...
        if (context->create() && context->makeCurrent(surface.get()))
        {
            ...
            NX_INFO(typeid(PerformanceTest), lm("QnPerformanceTest: Renderer: \"%1\" poor: %2").arg(renderer).arg(poorGpu));
        }
    }
