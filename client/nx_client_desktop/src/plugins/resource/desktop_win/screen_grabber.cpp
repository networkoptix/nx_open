...
void QnScreenGrabber::allocateTmpFrame(int width, int height, AVPixelFormat format)
{
    ...
    int numBytes = av_image_get_buffer_size(format, m_tmpFrameWidth, m_tmpFrameHeight, 1);
    m_tmpFrameBuffer = (quint8*)av_malloc(numBytes * sizeof(quint8));
    av_image_fill_arrays(
        m_tmpFrame->data,
        m_tmpFrame->linesize,
        m_tmpFrameBuffer,
        format, m_tmpFrameWidth, m_tmpFrameHeight, 1);
...
bool QnScreenGrabber::direct3DDataToFrame(void* opaque, AVFrame* pFrame)
{
    ...
    if (pFrame->width == 0)
    {
        int videoBufSize = av_image_get_buffer_size(format(), width(), height(), 1);
