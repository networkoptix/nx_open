...
constexpr milliseconds kTimeout = 2min;
...
{
    m_timer->setInterval(kTimeout.count());
...
void QnRestartPeerTask::at_commandSent(int status, int handle)
{
    ...
    connect(server.data(), &QnMediaServerResource::statusChanged,
        this, &QnRestartPeerTask::at_resourceStatusChanged);
    m_timer->start();
}
...
void QnRestartPeerTask::at_resourceStatusChanged()
{
    ...
    else
    {
        ...
        {
            disconnect(server.data(), &QnMediaServerResource::statusChanged,
                this, &QnRestartPeerTask::at_resourceStatusChanged);
            finishPeer();
        }
    }
