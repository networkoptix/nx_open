...
void QnUpdateUploader::sendPreambule() {
    ...
    if (!m_peers.isEmpty()) {
        ...
        connection2()->getUpdatesManager(Qn::kSuperUserAccess)->sendUpdatePackageChunk(m_updateId, md5.toLatin1(), -1, m_peers, ec2::DummyHandler::instance(), &ec2::DummyHandler::onRequestDone);
    }
...
void QnUpdateUploader::sendNextChunk() {
    ...
    if (!m_peers.isEmpty()) {
        ...
        connection2()->getUpdatesManager(Qn::kSuperUserAccess)->sendUpdatePackageChunk(m_updateId, data, offset, m_pendingPeers, ec2::DummyHandler::instance(), &ec2::DummyHandler::onRequestDone);
