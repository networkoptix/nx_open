...
QList<QAction *> QnOpenCurrentUserLayoutActionFactory::newActions(const QnActionParameters &, QObject *parent) {
    QnLayoutResourceList layouts = qnResPool->getResourcesWithParentId(QnUuid()).filtered<QnLayoutResource>(); /* Multi-videos will go here. */
    if(context()->user())
    ...
    foreach(const QnLayoutResourcePtr &layout, layouts) {
        ...
        if (QnResourcePool::isAutoGeneratedLayout(layout))
            continue;
