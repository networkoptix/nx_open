...
void QnWorkbenchActionHandler::at_eventManager_actionReceived(const QnAbstractBusinessActionPtr &businessAction) {
    ...
    case BusinessActionType::PlaySound: {
            const QLatin1String speechPrefix("speech://"); //TODO: #GDM move to common module?

            QString filename = businessAction->getParams().getSoundUrl();
            if (filename.startsWith(speechPrefix)) {
                AudioPlayer.sayTextAsync(filename.mid(speechPrefix));
                qDebug() << "speech action received" << filename.mid(speechPrefix);
            } else {
