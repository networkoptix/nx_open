    ...
    if(m_type == Qn::ResourceNode || m_type == Qn::ItemNode || m_type == Qn::EdgeNode) {
        ...
        if (!index.isNull()) {
            ...
            if(m_resource.isNull()) {
                ...
                if (qnResPool->isAutoGeneratedLayout(m_resource.dynamicCast<QnLayoutResource>())) {
                    QnResourcePtr parent = m_resource->getParentResource();
                    resourceName = tr("%1's Screen", "%1 means user's name").arg(parent ? parent->getName() : resourceName);
                }
