...
public:
    ...
    QList<TimeFormatToken> parseFormatString(const QString& formatString, Qt::TimeSpanFormat &format) const
    {
        ...
        tokenHash.insert(L'y', Qt::Years);
        tokenHash.insert(L'M', Qt::Months);
        tokenHash.insert(L'w', Qt::Weeks);
        tokenHash.insert(L'd', Qt::Days);
        tokenHash.insert(L'h', Qt::Hours);
        tokenHash.insert(L'm', Qt::Minutes);
        tokenHash.insert(L's', Qt::Seconds);
        tokenHash.insert(L'z', Qt::Milliseconds);
...
QString QTimeSpan::toString(const QString &format) const
{
    ...
    foreach(QTimeSpanPrivate::TimeFormatToken token, tokenList) {
        if (token.type == 0) {
            ...
            int value = *partsHash.value( unit );
            QChar end = L'0';
            formattedString.append (QString(QLatin1String("%1")).arg(value,
                                                      token.length,
                                                      10,
                                                      end) );
        }
