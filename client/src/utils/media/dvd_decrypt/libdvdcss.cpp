...
LIBDVDCSS_EXPORT dvdcss_t dvdcss_open ( char *psz_target )
{
     ...
     */
    dvdcss = (dvdcss_t) malloc( sizeof( struct dvdcss_s ) );
    if( dvdcss == NULL )
    {
        return NULL;
    }
...
#   endif
    ...
    if( psz_cache )
    {
        quint8 p_sector[DVDCSS_BLOCK_SIZE];
                                          ...
                                          int i_flags )
{
    ...
    if( ! memcmp( dvdcss->css.p_title_key, "\0\0\0\0\0", 5 ) )
    {
        ...
        for( i_index = i_ret; i_index; i_index-- )
        {
            ...
            p_buffer = (void *) ((quint8 *)p_buffer + DVDCSS_BLOCK_SIZE);
        }
    }
    else
    {
        ...
        for( i_index = i_ret; i_index; i_index-- )
        {
            _dvdcss_unscramble( dvdcss->css.p_title_key, (quint8*) p_buffer );
            ((quint8*)p_buffer)[0x14] &= 0x8f;
            p_buffer = (void *) ((quint8 *)p_buffer + DVDCSS_BLOCK_SIZE);
        }
    }
                                           ...
                                           int i_flags )
{
    ...
    for( i_index = i_ret; i_index; i_index-- )
    {
        ...
        _dvdcss_unscramble( dvdcss->css.p_title_key, (quint8*) iov_base );
        ((quint8*)iov_base)[0x14] &= 0x8f;

        iov_base = (void *) ((quint8*)iov_base + DVDCSS_BLOCK_SIZE);
