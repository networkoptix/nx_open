...
#if 0
  if(memcmp((quint8 *)tt_srpt->title + 
  ...
  for(i = 0; i < vts_ptt_srpt->nr_of_srpts; i++) {
    ...
    for(j = 0; j < vts_ptt_srpt->title[i].nr_of_ptts; j++) {
      ...
      vts_ptt_srpt->title[i].ptt[j].pgcn 
        = *(quint16*)(((char *)data) + data[i] + 4*j - VTS_PTT_SRPT_SIZE);
      vts_ptt_srpt->title[i].ptt[j].pgn 
        = *(quint16*)(((char *)data) + data[i] + 4*j + 2 - VTS_PTT_SRPT_SIZE);
    }
  }
...
int ifoRead_PTL_MAIT(ifo_handle_t *ifofile) {
  ...
  for(i = 0; i < ptl_mait->nr_of_countries; i++) {
    ...
    pf_temp = (quint16 *)malloc(info_length);
    if(!pf_temp) {
      for(j = 0; j < i ; j++) {
...
static int ifoRead_VOBU_ADMAP_internal(ifo_handle_t *ifofile, 
  ...
  vobu_admap->vobu_start_sectors = (quint32 *)malloc(info_length); 
  ...
  for(i = 0; i < info_length/sizeof(quint32); i++)
...
int ifoRead_VTS_ATRT(ifo_handle_t *ifofile) {
  ...
  CHECK_VALUE((quint32)vts_atrt->nr_of_vtss * (4 + VTS_ATTRIBUTES_MIN_SIZE) + 
