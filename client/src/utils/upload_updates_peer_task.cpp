...
void QnUploadUpdatesPeerTask::doStart() {
    ...
    foreach (const QnId &id, peers()) {
        QnMediaServerResourcePtr server = qnResPool->getIncompatibleResourceById(id, true);
        if (!server)
            continue;
...
void QnUploadUpdatesPeerTask::uploadNextUpdate() {
    ...
    if (!m_uploader->uploadUpdate(m_updateId,
                                  ...
                                  m_peersBySystemInformation[systemInformation]))
    {
        finish(UploadError);
        return;
    }
...
void QnUploadUpdatesPeerTask::at_uploader_finished() {
    foreach (const QnId &id, m_uploader->peers()) {
