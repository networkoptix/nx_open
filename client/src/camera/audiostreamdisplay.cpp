...
template <class T>
static inline void down_mix_to_stereo(T *data, int channels, int len)
{
    T* output = data;
    T* input = data;
...
void CLAudioStreamDisplay::putData(QnCompressedAudioDataPtr data, qint64 minTime)
{
    ...
    if (!data && !m_audioSound) // do not need to check audio device in case of data=0 and no audio device
        return;
