...
int QnDesktopDataProvider::processData(bool flush)
{
    ...
    while (ai && ai->m_audioQueue.size() > 0 && (ai2 == 0 || ai2->m_audioQueue.size() > 0))
    {
        ...
        if (aEncoded > 0)
        {
            QnCompressedAudioDataPtr audio = QnCompressedAudioDataPtr(new QnCompressedAudioData(CL_MEDIA_ALIGNMENT, aEncoded, m_audioCodecCtxPtr));
...
void QnDesktopDataProvider::run()
{
    while (!needToStop() || m_grabber->dataExist())
    {
        ...
        if (!capturedData.opaque || capturedData.w == 0 || capturedData.h == 0)
            continue;
