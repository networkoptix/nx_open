...
static void CryptKey        ( int, int, quint8 const *, quint8 * );
static void DecryptKey      ( quint8,
                              quint8 const *, quint8 const *, quint8 * );

static int  DecryptDiscKey  ( dvdcss_t, quint8 const *, dvd_key_t );
static int  CrackDiscKey    ( dvdcss_t, quint8 * );

static void DecryptTitleKey ( dvd_key_t, dvd_key_t );
static int  RecoverTitleKey ( int, quint8 const *,
                              quint8 const *, quint8 const *, quint8 * );
static int  CrackTitleKey   ( dvdcss_t, int, int, dvd_key_t );

static int  AttackPattern   ( quint8 const[], int, quint8 * );
#if 0
static int  AttackPadding   ( quint8 const[], int, quint8 * );
#endif
...
{
    static quint8 p_garbage[ DVDCSS_BLOCK_SIZE ];  /* we never read it back */
    quint8 p_key[ KEY_SIZE ];
    int i, i_ret = 0;
 ...
 *****************************************************************************/
int _dvdcss_unscramble( dvd_key_t p_key, quint8 *p_sec )
{
    unsigned int    i_t1, i_t2, i_t3, i_t4, i_t5, i_t6;
    quint8        *p_end = p_sec + DVDCSS_BLOCK_SIZE;
...
static void CryptKey( int i_key_type, int i_variant,
                      quint8 const *p_challenge, quint8 *p_key )
{
    /* Permutation table for challenge */
    quint8 pp_perm_challenge[3][10] =
            { { 1, 3, 0, 7, 5, 2, 9, 6, 4, 8 },
              { 6, 1, 9, 3, 8, 5, 7, 4, 0, 2 },
              { 4, 0, 3, 5, 7, 2, 8, 6, 1, 9 } };

    /* Permutation table for variant table for key2 and buskey */
    quint8 pp_perm_variant[2][32] =
    ...
    /* The "secret" key */
    quint8 p_secret[5] = { 0x55, 0xD6, 0xC4, 0xC5, 0x28 };

    quint8 p_bits[30], p_scratch[10], p_tmp1[5], p_tmp2[5];
    quint8 i_lfsr0_o;  /* 1 bit used */
    quint8 i_lfsr1_o;  /* 1 bit used */
    quint8 i_css_variant, i_cse, i_index, i_combined, i_carry;
    quint8 i_val = 0;
    quint32 i_lfsr0, i_lfsr1;
    int i_term = 0;
    int i_bit;
    int i;
 ...
 *****************************************************************************/
static void DecryptKey( quint8 invert, quint8 const *p_key,
                        quint8 const *p_crypted, quint8 *p_result )
{
 ...
 *****************************************************************************/
static int DecryptDiscKey( dvdcss_t dvdcss, quint8 const *p_struct_disckey,
                           dvd_key_t p_disc_key )
{
    quint8 p_verify[KEY_SIZE];
    unsigned int i, n = 0;
...
static int CrackDiscKey( dvdcss_t dvdcss, quint8 *p_disc_key )
{
 ...
 *****************************************************************************/
static int RecoverTitleKey( int i_start, quint8 const *p_crypted,
                            quint8 const *p_decrypted,
                            quint8 const *p_sector_seed, quint8 *p_key )
{
    quint8 p_buffer[10];
    unsigned int i_t1, i_t2, i_t3, i_t4, i_t5, i_t6;
    unsigned int i_try;
...
{
    quint8       p_buf[ DVDCSS_BLOCK_SIZE ];
    const quint8 p_packstart[4] = { 0x00, 0x00, 0x01, 0xba };
    int i_reads = 0;
    int i_encrypted = 0;
 ...
 *****************************************************************************/
static int AttackPadding( quint8 const p_sec[ DVDCSS_BLOCK_SIZE ],
                          int i_pos, quint8 *p_key )
{
