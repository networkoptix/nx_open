...
bool QnStreamRecorder::initFfmpegContainer(const QnConstAbstractMediaDataPtr& mediaData)
{
    ...
    for (size_t i = 0; i < m_recordingContextVector.size(); ++i)
    {
        ...
        bool isTranscode = !m_transcodeFilters.isEmpty()
        ...
        if (auto videoData = std::dynamic_pointer_cast<const QnCompressedVideoData>(mediaData))
        {
            ...
            for (int j = 0; j < videoChannels; ++j)
            {
                ...
                if (isTranscode)
                {
                    ...
                    m_videoTranscoder->setFilterList(QnImageFilterHelper::createFilterChain(
                        m_transcodeFilters,
                        m_videoTranscoder->getResolution()));
...
void QnStreamRecorder::setTranscodeFilters(const QnLegacyTranscodingSettings& filters)
{
