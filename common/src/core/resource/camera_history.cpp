...
void QnCameraHistoryPool::invalidateCameraHistory(const QnVirtualCameraResourcePtr &camera) {
    QMutexLocker lock(&m_mutex);
    m_historyValidCameras.remove(camera->getId());
}
