...
namespace {
    const QString autoGeneratedLayoutKey = lit("autoGenerated");
}
...
void QnResourcePool::addResources(const QnResourceList &resources)
{
    ...
    for (const QnResourcePtr &resource: newResources.values())
    {
        ...
        connect(resource.data(), &QnResource::statusChanged,      this, &QnResourcePool::statusChanged,     Qt::QueuedConnection);
        connect(resource.data(), &QnResource::statusChanged,      this, &QnResourcePool::resourceChanged,   Qt::QueuedConnection);
        connect(resource.data(), &QnResource::resourceChanged,    this, &QnResourcePool::resourceChanged,   Qt::QueuedConnection);
...
bool QnResourcePool::isAutoGeneratedLayout(const QnLayoutResourcePtr &layout) {
    if (!layout)
        return false;

    return layout->hasProperty(autoGeneratedLayoutKey);
}

void QnResourcePool::markLayoutAutoGenerated(const QnLayoutResourcePtr &layout) {
    if (!layout)
        return;

    layout->setProperty(autoGeneratedLayoutKey, true);
}
