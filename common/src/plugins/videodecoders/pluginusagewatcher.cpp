...
:
    ...
    m_sharedMemoryLocker( uniquePluginID + QString::fromAscii("_sync") ),
    m_usageRecordID( 0 ),
    m_usageDataSharedMemory( uniquePluginID + QString::fromAscii("_shared") ),
    m_terminated( false )
{
    ...
    if( !created )
    {
        ...
        {
            NX_LOG( QString::fromAscii("Failed to create shared memory with key %1. %2").arg(m_uniquePluginID).arg(m_usageDataSharedMemory.errorString()), cl_logWARNING );
            return;
        }
    }
    ...
    {
        NX_LOG( QString::fromAscii("Failed to attach to shared memory with key %1. %2").arg(m_uniquePluginID).arg(m_usageDataSharedMemory.error()), cl_logWARNING );
        return;
    }

    if( created )
    {
        NX_LOG( QString::fromAscii("PluginUsageWatcher. Successfully created usage data shared memory region. Initializing..."), cl_logDEBUG1 );
        //initializing
        ...
        {
            NX_LOG( QString::fromAscii("PluginUsageWatcher. Failed to serialize usage data to shared memory region"), cl_logWARNING );
        }
    }
    ...
    if( !isRunning() )
    {
        NX_LOG( QString::fromAscii("PluginUsageWatcher. Failed to start internal thread").arg(m_uniquePluginID), cl_logDEBUG1 );
        return;
    }

    NX_LOG( QString::fromAscii("PluginUsageWatcher. Successfully attached to usage data shared memory region %1").arg(m_uniquePluginID), cl_logDEBUG1 );
}
...
bool PluginUsageWatcher::updateUsageParams()
{
    ...
    if( !m_usageRecordID )
    {
        ...
        NX_LOG( QString::fromAscii("PluginUsageWatcher. Selected usage record id %1").arg(m_usageRecordID), cl_logDEBUG1 );
    }
...
bool PluginUsageWatcher::lockSharedMemory()
{
    ...
    {
        NX_LOG( QString::fromAscii("PluginUsageWatcher. Failed to lock semaphore. id %1. error: %2").
...
bool PluginUsageWatcher::unlockSharedMemory()
{
    ...
    if( m_sharedMemoryLocked )
    {
        ...
        NX_LOG( QString::fromAscii("PluginUsageWatcher. Failed to release semaphore. id %1. error: %2").
            arg(m_uniquePluginID).arg(m_sharedMemoryLocker.errorString()), cl_logERROR );
    }
    return !m_sharedMemoryLocked;
}
...
bool PluginUsageWatcher::readExternalUsage( UsageRecordArray* const recArray ) const
{
    ...
    {
        NX_LOG( QString::fromAscii("PluginUsageWatcher. Failed to deserialize usage data from shared memory region"), cl_logWARNING );
        return false;
    }
