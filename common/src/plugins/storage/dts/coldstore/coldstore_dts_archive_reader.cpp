...
//    if (m_eventfileList.size() < 2)    try to read last file
    if (m_eventfileList.size() < 1)
    {
        ...
        {
            NX_LOG("Not enough files, only file is still writing", cl_logWARNING);
            m_loggedError = true;
        }
        return false;
    }

//    NX_LOG("FileQuery time = ", t.elapsed(), cl_logWARNING);

    // Set start and end times
    m_startTime = m_datafileList.at(0).mks;
    if (!openEventFile(m_datafileList.size()-1))
    {
        NX_LOG("openEventFile failed getting end of time", cl_logWARNING);
        return false;
    }
    ...
    if (!openEventFile(0))
    {
        NX_LOG("openEventFile failed", cl_logWARNING);
        return false;
    }

//    NX_LOG("Open1  time = ", t.elapsed(), cl_logWARNING);

    if (!openCSFile(0, 0))
    {
        NX_LOG("openCSFile failed", cl_logWARNING);
        return false;
    }


//    NX_LOG("Open2 time = ", t.elapsed(), cl_logWARNING);    
...
//    if (f_index > (m_datafileList.size()-2) )
    ...
    //NX_LOG("Open_OPEN time = ", t.elapsed(), cl_logWARNING);
    ...
    //NX_LOG("Open_SEEK time = ", t.elapsed(), cl_logWARNING);
    ...
    //NX_LOG("Open_READ time = ", t.elapsed(), cl_logWARNING);
...
//    NX_LOG("==========Open file ", f_index, " event = ", event_index,  cl_logWARNING);
//    NX_LOG("time ", t.elapsed(),  cl_logWARNING);
...
//    if (f_index > (m_eventfileList.size()-2) )
    if (f_index > (m_eventfileList.size()-1) )   // Try to read last file
    {
        ...
        if (m_loggedError == false)
        {
            NX_LOG("openEventFile not enough files", cl_logWARNING);
            m_loggedError = true;
        }
    ...
    if (m_csConnection->Open(
        ...
        0,  &time_current) != Veracity::ISFS::STATUS_SUCCESS)
    {
        NX_LOG("openEventFile CS file open failed", cl_logWARNING);
        return false;
    }
    ...
    //NX_LOG("OpenEvent_OPEN time = ", t.elapsed(), cl_logWARNING);
    ...
    if (result != Veracity::ISFS::STATUS_SUCCESS)
    {
        NX_LOG("openEventFile CS file read failed", cl_logWARNING);
        Q_ASSERT(false);
        return false;
    }
    ...
    //NX_LOG("========OpenEvent_READ time = ", t.elapsed(), cl_logWARNING);
    ...
    {    
        ...
        if (!doc.setContent(m_fileContent))
        {    
            NX_LOG("openEventFile setContent failed", cl_logWARNING);
            return false;
        }
    }
    ...
    {
        NX_LOG("openEventFile root tag find failed", cl_logWARNING);
        return false;
    }
    ...
    if (lst.size()<1)
    {
        NX_LOG("openEventFile framesPerSecond failed", cl_logWARNING);
        return false;
    }
    ...
    for (int i = 0; i < lst.size(); ++i)
    {
        ...
        {
           NX_LOG("openEventFile frameNumber failed", cl_logWARNING);
           return false;
        }
        QDomElement fileOffsetEl = node.firstChildElement(QLatin1String("fileOffset"));
        if (fileOffsetEl.isNull())
        {
            NX_LOG("openEventFile fileOffset failed", cl_logWARNING);
            return false;
        }
...
//    NX_LOG("openEventFile time = ", t.elapsed(), cl_logWARNING);
