...
int QnMediaServerConnection::mergeSystemAsync(const QUrl &url, const QString &user, const QString &password, const QString &currentPassword, bool ownSettings, bool oneServer, bool ignoreIncomatible, QObject *target, const char *slot) {
    QnRequestParamList params;
    params << QnRequestParam("url", url.toString());
    params << QnRequestParam("user", user);
    params << QnRequestParam("password", password);
    ...
    params << QnRequestParam("ignoreIncompatible", ignoreIncomatible ? lit("true") : lit("false"));
