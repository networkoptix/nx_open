#include "h2645_rtp_parser_base.h"
...
static const char kH264NalPrefix[4] = {0x00, 0x00, 0x00, 0x01};
static const char kH264NalShortPrefix[3] = {0x00, 0x00, 0x01};

} // namespace

H2645ParserBase::H2645ParserBase():
...
void H2645ParserBase::setSDPInfo(QList<QByteArray> lines)
{
                    ...
                    if (pos >= 0)
                    {
                        ...
                        for (QByteArray nal : nalUnits)
                        {
...

                            {
                                ...
                                QByteArray startCode(kH264NalPrefix, sizeof(kH264NalPrefix));
                                QByteArray startCodeShort(kH264NalShortPrefix, sizeof(kH264NalShortPrefix));
                            ...
                            m_sdpSpsPps << QByteArray(kH264NalPrefix, sizeof(kH264NalPrefix)).append(nal);
                        }
                    }
                }
            }
...
int H2645ParserBase::getSpsPpsSize() const
{
    int result = 0;
...
void H2645ParserBase::backupCurrentData(quint8* bufferStart)
{
...
void H2645ParserBase::serializeSpsPps(QnByteArray& dst)
{
    for (int i = 0; i < m_sdpSpsPps.size(); ++i)
        dst.uncheckedWrite(m_sdpSpsPps[i].data(), m_sdpSpsPps[i].size());
}

void H2645ParserBase::decodeSpsInfo(const QByteArray& data)
{
    try
    {
        m_sps.decodeBuffer(
...
bool H2645ParserBase::isBufferOverflow() const
{
    int addHeaderSize = 0;
...
QnCompressedVideoDataPtr H2645ParserBase::createVideoData(
...
)
{
    ...
    for (size_t i = 0; i < m_chunks.size(); ++i)
    {
        ...
        if (m_chunks[i].nalStart)
        {
            ...
            result->m_data.uncheckedWrite(kH264NalPrefix, sizeof(kH264NalPrefix));
...
bool H2645ParserBase::clearInternalBuffer()
{
    m_chunks.clear();
    m_videoFrameSize = 0;
    m_keyDataExists
...
bool H2645ParserBase::isIFrame(const quint8* data, int dataLen) const
{
    if (dataLen < 2)
        return false;
...
bool H2645ParserBase::isFirstSliceNal(
...
void H2645ParserBase::updateNalFlags(const quint8* data, int dataLen)
{
...
bool H2645ParserBase::isPacketStartsNewFrame(
    const quint8* curPtr,
...
bool H2645ParserBase::isSliceNal(quint8 nalUnitType) const
{
    return nalUnitType >= nuSliceNonIDR && nalUnitType <= nuSliceIDR;
}

bool H2645ParserBase::processData(
