...
int RTPSession::readBinaryResponce(quint8* data, int maxDataSize)
{
    while (m_tcpSock->isConnected())
    {
        while (m_responseBufferLen < 4) {
            int readed = readSocketWithBuffering(m_responseBuffer+m_responseBufferLen, 4 - m_responseBufferLen);
            if (readed <= 0)
                return readed;
            m_responseBufferLen += readed;
        }
...
int RTPSession::readSocketWithBuffering( quint8* buf, size_t bufSize )
{
    size_t bufSizeBak = bufSize;
    ...
    for( ;; )
    {
        if( m_additionalReadBufferSize > 0 )
        {
            ...
            memcpy( buf, m_additionalReadBuffer, bytesToCopy );
            m_additionalReadBufferSize -= bytesToCopy;
            if( m_additionalReadBufferSize > 0 )
                memmove( m_additionalReadBuffer, m_additionalReadBuffer+bytesToCopy, m_additionalReadBufferSize );
            bufSize -= bytesToCopy;
            buf += bytesToCopy;
        }
