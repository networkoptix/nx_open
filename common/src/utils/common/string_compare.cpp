...
int qnNaturalStringCompare( const QString & lhs, const QString & rhs, Qt::CaseSensitivity caseSensitive )
{
	int ii = 0;
	int jj = 0;
...
{
	return qnNaturalStringCompare( lhs, rhs, Qt::CaseSensitive ) < 0;
}

bool qnNaturalStringCaseInsensitiveLessThan( const QString & lhs, const QString & rhs )
{
	return qnNaturalStringCompare( lhs, rhs, Qt::CaseInsensitive ) < 0;
}

QStringList qnNaturalStringSort( const QStringList & list, Qt::CaseSensitivity caseSensitive )
{
	QStringList retVal = list;
	if ( caseSensitive == Qt::CaseSensitive )
		qSort( retVal.begin(), retVal.end(), qnNaturalStringLessThan );
	else
		qSort( retVal.begin(), retVal.end(), qnNaturalStringCaseInsensitiveLessThan );
	return retVal;
}
