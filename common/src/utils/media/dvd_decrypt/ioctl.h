...
int ioctl_ReportChallenge   ( int, int *, quint8 * );
int ioctl_ReportKey1        ( int, int *, quint8 * );
int ioctl_ReportASF         ( int, int *, int * );
int ioctl_InvalidateAgid    ( int, int * );
int ioctl_SendChallenge     ( int, int *, quint8 * );
int ioctl_SendKey2          ( int, int *, quint8 * );
...
#define INIT_SSC( TYPE, SIZE ) \
    ...
    ssc.SRB_BufPointer = (quint8 *)p_buffer; \
    ssc.SRB_BufLen = (SIZE); \
    WinInitSSC( &ssc, (TYPE) );
#endif
...
#define INIT_CPT( TYPE, SIZE ) \
    ...
    p_buffer = (quint8 *) p_cpt + sizeof( CAM_PASS_THRU ); \
