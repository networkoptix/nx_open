 ...
 *****************************************************************************/
int ioctl_ReadDiscKey( int i_fd, int *pi_agid, quint8 *p_key )
{
    int i_ret;
    ...
    {
        DWORD tmp;
        quint8 buffer[DVD_DISK_KEY_LENGTH];
 ...
 *****************************************************************************/
int ioctl_ReadTitleKey( int i_fd, int *pi_agid, int i_pos, quint8 *p_key )
{
    int i_ret;
    ...
    {
        DWORD tmp;
        quint8 buffer[DVD_TITLE_KEY_LENGTH];
        PDVD_COPY_PROTECT_KEY key = (PDVD_COPY_PROTECT_KEY) &buffer;
 ...
 *****************************************************************************/
int ioctl_ReportChallenge( int i_fd, int *pi_agid, quint8 *p_challenge )
{
    int i_ret;
    ...
    {
        DWORD tmp;
        quint8 buffer[DVD_CHALLENGE_KEY_LENGTH];
        PDVD_COPY_PROTECT_KEY key = (PDVD_COPY_PROTECT_KEY) &buffer;
 ...
 *****************************************************************************/
int ioctl_ReportKey1( int i_fd, int *pi_agid, quint8 *p_key )
{
    int i_ret;
    ...
    {
        DWORD tmp;
        quint8 buffer[DVD_BUS_KEY_LENGTH];
        PDVD_COPY_PROTECT_KEY key = (PDVD_COPY_PROTECT_KEY) &buffer;
 ...
 *****************************************************************************/
int ioctl_SendChallenge( int i_fd, int *pi_agid, quint8 *p_challenge )
{
    int i_ret;
 ...
 *****************************************************************************/
int ioctl_SendKey2( int i_fd, int *pi_agid, quint8 *p_key )
{
    int i_ret;
...
int ioctl_ReportRPC( int i_fd, int *p_type, int *p_mask, int *p_scheme )
{
    ...
    *p_scheme = dvdbs.rpcScheme;
    ...
    {
        DWORD tmp;
        quint8 buffer[DVD_RPC_KEY_LENGTH];
        PDVD_COPY_PROTECT_KEY key = (PDVD_COPY_PROTECT_KEY) &buffer;
...
static int SolarisSendUSCSI( int i_fd, struct uscsi_cmd *p_sc )
{
    ...
    static void * (*p_get_handle) ( qint32 );
...
static void WinInitSPTD( SCSI_PASS_THROUGH_DIRECT *p_sptd, int i_type )
{
    ...
    p_sptd->Cdb[ 0 ] = i_type;
    p_sptd->Cdb[ 8 ] = (quint8)(p_sptd->DataTransferLength >> 8) & 0xff;
    p_sptd->Cdb[ 9 ] = (quint8) p_sptd->DataTransferLength       & 0xff;
    p_sptd->CdbLength = 12;
...
static void WinInitSSC( struct SRB_ExecSCSICmd *p_ssc, int i_type )
{
    ...
    p_ssc->CDBByte[ 8 ] = (quint8)(p_ssc->SRB_BufLen >> 8) & 0xff;
    p_ssc->CDBByte[ 9 ] = (quint8) p_ssc->SRB_BufLen       & 0xff;
    p_ssc->SRB_CDBLen   = 12;
