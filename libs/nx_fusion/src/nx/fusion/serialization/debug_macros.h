...
struct EnumDebugSerializer
{
    QDebug& operator()(QDebug& stream, const T& value)
    {
        static_assert(nx::reflect::IsInstrumentedEnumV<T>,
            "The enum should be instrumented with nx_reflect. "
                "See nx_reflect/README.md for details.");
        return stream << QString::fromStdString(nx::reflect::toString(value));
    }
};
