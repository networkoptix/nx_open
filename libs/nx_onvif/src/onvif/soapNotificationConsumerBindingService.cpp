...
int NotificationConsumerBindingService::ssl_run(int port)
{
    if (!soap_valid_socket(this->soap->master) && !soap_valid_socket(this->bind(NULL, port, 100)))
        return this->soap->error;
    for (;;)
...
static int serve___onvifEvents_ncb__Notify(struct soap*, NotificationConsumerBindingService*);
...
int NotificationConsumerBindingService::dispatch(struct soap* soap)
{
    ...
    if (!soap_match_tag(soap, soap->tag, "oasisWsnB2:Notify"))
        return serve___onvifEvents_ncb__Notify(soap, this);
    return soap->error = SOAP_NO_METHOD;
}

static int serve___onvifEvents_ncb__Notify(struct soap *soap, NotificationConsumerBindingService *service)
{
    struct __onvifEvents_ncb__Notify soap_tmp___onvifEvents_ncb__Notify;
    soap_default___onvifEvents_ncb__Notify(soap, &soap_tmp___onvifEvents_ncb__Notify);
    if (!soap_get___onvifEvents_ncb__Notify(soap, &soap_tmp___onvifEvents_ncb__Notify, "-onvifEvents-ncb:Notify", NULL))
        return soap->error;
    if (soap_body_end_in(soap)
     || soap_envelope_end_in(soap)
     || soap_end_recv(soap))
        return soap->error;
    soap->error = service->Notify(soap_tmp___onvifEvents_ncb__Notify.oasisWsnB2__Notify);
    if (soap->error)
        return soap->error;
