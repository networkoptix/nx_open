...
/******************************************************************************/
/*
 Contains additional fields for struct soap.

 Responsible for patch management in gsoap functions.
 There are 5 standard functions for type T in gsoap:
  soap_default_T - assigning a default value to T
  soap_T2s - conversion of T into std::string
  soap_s2T - conversion of std::string into T
  soap_in_T - reading T from XML element
  soap_out_T - writing T into XML element
 Each of them is implemented according to XML standards. Alas, some devices break standards:
 they send incorrect XML's or wait for incorrect XMLs (ignoring correct ones), they use incorrect
 string representations of standard types and so on.
 To workaround these bugs, patches are added into gsoap source code. The struct soap_workarounds
 contains boolean flags, that activate/deactivate patches.
*/
struct SOAP_CMAC soap_workarounds
{
    /**
     * Manages patch in "soap_xsd__duration2s".
     * Use short format of duration representation instead of standard (full) format
