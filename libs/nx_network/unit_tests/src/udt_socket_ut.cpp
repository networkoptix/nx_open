...
public:
    ~SocketUdt()
    {
        if (m_createStreamSocketFunc)
            SocketFactory::setCreateStreamSocketFunc(std::move(*m_createStreamSocketFunc));

        if (m_createStreamServerSocketFunc)
            SocketFactory::setCreateStreamServerSocketFunc(std::move(*m_createStreamServerSocketFunc));
    }
    ...
    void setUdtSocketFunctions()
    {
        setCreateStreamSocketFunc(
            [](auto&&...) -> std::unique_ptr<AbstractStreamSocket>
            {
                return std::make_unique<UdtStreamSocket>(AF_INET);
            });

        setCreateStreamServerSocketFunc(
            [](bool /*sslRequired*/, std::optional<int> /*ipVersion*/)
...
private:
    std::optional<SocketFactory::CreateStreamSocketFuncType> m_createStreamSocketFunc;
    std::optional<SocketFactory::CreateStreamServerSocketFuncType> m_createStreamServerSocketFunc;
...
protected:
    ...
    void assertServerSocketHasConnected()
    {
        ...
        ASSERT_EQ(SystemError::noError, connectResultCode)
            << SystemError::toString(connectResultCode);
...
TEST_F(SocketUdtRendezvous, reusing_rendezvous_connection_stress_test)
{
    ...
    ASSERT_EQ(SystemError::noError, connectorResultCode)
        << SystemError::toString(connectorResultCode);
...
// TODO: #akolesnikov Allocate UDP socket and use that socket's port.
const SocketAddress UdtSocketTypeSet::serverEndpointForConnectShutdown =
    SocketAddress(HostAddress::localhost, 45431);

INSTANTIATE_TYPED_TEST_SUITE_P(UdtSocketStream, StreamSocketAcceptance, UdtSocketTypeSet);
...
INSTANTIATE_TYPED_TEST_SUITE_P(UdtSocketV6Stream, StreamSocketAcceptance, UdtSocketV6TypeSet);
...
INSTANTIATE_TYPED_TEST_SUITE_P(UdtStreamSocket, SocketOptions, UdtStreamSocketFactory);
