...
private:
    ...
    nx::utils::SyncQueue<std::tuple<AsyncMessagePipeline*, SystemError::ErrorCode>> m_connectionClosedEvents;
    ...
    void initializeHttpPipe()
    {
        ...
        m_httpPipes.back()->registerCloseHandler(
            [this, ptr = m_httpPipes.back().get()](SystemError::ErrorCode reason)
            {
                m_connectionClosedEvents.push(std::make_tuple(ptr, reason));
            });
