...
TYPED_TEST_P(HttpTunneling, server_destructor_blocks_until_all_callbacks_have_returned)
{
    this->givenTunnellingServer();
    ...
    auto done = std::async([promise = std::move(promise)]() mutable {
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
        // Resuming handler execution.
        promise.set_value();
    });

    // Expecting this to block until handler completion.
    this->deleteTunnellingServer();
...
REGISTER_TYPED_TEST_SUITE_P(HttpTunneling,
    ...
    server_destructor_blocks_until_all_callbacks_have_returned);
