...
const std::string DelayingRequestHandler::PATH = "/tst";
...
TEST_F(HttpAsyncServerConnectionTest, connectionRemovedBeforeRequestHasBeenProcessed)
{
    ...
    const auto url = url::Builder("http://127.0.0.1/")
        .setPort(m_testHttpServer->serverAddress().port)
        .setPath(DelayingRequestHandler::PATH).toUrl();
...
const std::string PipeliningTestHandler::PATH = "/tst";
...
protected:
    ...
    void whenSendMultipleRequests(int count)
    {
        ...
        m_socket = SocketFactory::createStreamSocket(ssl::kAcceptAnyCertificate);
        ...
        for (; msgCounter < count; ++msgCounter)
        {
            nx::network::http::insertOrReplaceHeader(
                &request.headers,
                nx::network::http::HttpHeader("Seq", std::to_string(msgCounter)));
            //sending request
    ...
    void thenMultipleResponsesAreReceived(int count)
    {
        ...
        while (msgCounter > 0)
        {
            ...
            if (m_httpMsgReader.state() == HttpStreamReader::ReadState::messageDone)
            {
                ...
                ASSERT_EQ(nx::utils::stoi(seqIter->second), count - msgCounter);
                --msgCounter;
            }
        }
...
private:
    ...
    void parseDataRead()
    {
        ...
        ASSERT_TRUE(m_httpMsgReader.parseBytes(
            nx::ConstBufferRefType(m_readBuf.data(), m_dataSize),
            &bytesParsed));
        m_readBuf.erase(0, bytesParsed);
...
protected:
    ...
    static const std::string kProtocolToUpgradeTo;
...
const std::string HttpAsyncServerConnectionUpgrade::kTestPath("/HttpAsyncServerConnectionUpgrade");
const std::string HttpAsyncServerConnectionUpgrade::kProtocolToUpgradeTo("TEST/1.0");
