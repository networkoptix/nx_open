...
protected:
    ...
    {
        return nx::format("http://%1/test").arg(m_testHttpServer->serverAddress().toString());
    }
...
private:
    ...
    void init()
    {
        m_testHttpServer->registerStaticProcessor("/test", nx::Buffer(kPublicIpAddress.toUtf8()), "text/plain");
