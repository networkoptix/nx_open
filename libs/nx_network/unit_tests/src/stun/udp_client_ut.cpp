...
{
public:
    static constexpr char kServerResponseNonce[] = "correctServerResponse";
...
protected:
    ...
    void whenSendRequestToUnknownHost()
    {
        ...
        nx::network::SocketGlobals::instance()
            .blockHost(endpoint.address.toString());
        m_blockedHosts.push_back(endpoint.address.toString());
...
TEST_F(UdpClient, client_response_injection)
{
    ...
    client.sendRequestTo(
        ...
        [&mtx, &cond, &response, &responseErrorCode](
            ...
            Message msg)
        {
            NX_MUTEX_LOCKER lk(&mtx);
            responseErrorCode = errorCode;
            response = std::move(msg);
            cond.wakeAll();
        });

    //TODO #akolesnikov waiting for message to be sent
    ...
    for (int i = 0; i < kPacketsToInject; ++i)
    {
        ...
        {
            const nx::Buffer buf(nx::utils::generateRandomName(nx::utils::random::number(100, 400)));
...
// TODO: #akolesnikov add test with authorization
