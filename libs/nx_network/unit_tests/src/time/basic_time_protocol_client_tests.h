...
protected:
    ...
    void whenRequestedTime()
    {
        ...
        m_timeRequestResult = makeSyncCall<qint64, SystemError::ErrorCode, std::chrono::milliseconds>(
            std::bind(&TimeProtocolClientType::getTimeAsync, m_timeProtocolClient.get(), _1));
    }
...
private:
    ...
    std::optional<SocketAddress> m_serverAddress;
    ...
    {
        m_timeServer = std::make_unique<TimeProtocolServer>();
...
TYPED_TEST_SUITE_P(BasicTimeProtocolClient);
...
GTEST_ALLOW_UNINSTANTIATED_PARAMETERIZED_TEST(BasicTimeProtocolClient);
REGISTER_TYPED_TEST_SUITE_P(BasicTimeProtocolClient,
    fetches_time,
