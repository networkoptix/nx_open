    ...
    dispatch(
        [this, handler = std::move(handler)]() mutable
        {
            ...
            for (auto& source: m_acceptors)
            {
                ...
                NX_VERBOSE(this, nx::format("Accept on source(%1)").arg(&source));
    ...
    dispatch(
        [this, &socketAddedPromise, socket = std::move(socket)]() mutable
        {
            ...
            if (m_acceptHandler)
            {
                ...
                source.isAccepting = true;
                NX_DEBUG(this, nx::format("Accept on source(%1) when added").arg(&source));
...
{
    NX_VERBOSE(this, nx::format("Remove socket(%1)").arg(pos));
    ...
    removeByIterator(
        [this, pos]()
        {
            ...
            {
                NX_ASSERT(false, nx::format("pos = %1, m_acceptors.size() = %2")
...
{
    NX_VERBOSE(this, nx::format("Accepted socket(%1) (%2) from source(%3)")
    ...
    // TODO: #akolesnikov Better not cancel accepts on every socket accepted.

    NX_VERBOSE(this, nx::format("Cancel all other accepts"));
...
void AggregateAcceptor::removeByIterator(FindIteratorFunc findIterator)
{
    ...
    dispatch(
        [this, &socketRemovedPromise, &findIterator]()
        {
            ...
            NX_DEBUG(this, nx::format("Acceptor(%1) is removed")
