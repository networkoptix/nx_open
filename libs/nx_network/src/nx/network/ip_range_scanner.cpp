    ...
    int portToScan)
{
    ...
    dispatch(
        [this, startAddr, endAddr, portToScan, callback = std::move(callback)]() mutable
        {
            ...
            m_startIpv4 = ntohl(startAddr.ipV4()->s_addr);
            m_endIpv4 = ntohl(endAddr.ipV4()->s_addr);
...
bool IpRangeScanner::startHostCheck()
{
    ...
    auto clientIter =
        m_ipCheckers.insert(std::make_unique<http::AsyncClient>(ssl::kAcceptAnyCertificate)).first;
