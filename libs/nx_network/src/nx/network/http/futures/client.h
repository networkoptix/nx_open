...
public:
    ...
    cf::future<Response> post(const nx::utils::Url& url,
        StringType contentType, BufferType requestBody);
    ...
    cf::future<Response> upgrade(const nx::utils::Url& url, const StringType& targetProtocol);
    cf::future<Response> upgrade(const nx::utils::Url& url,
        const Method::ValueType& method, const StringType& targetProtocol);

    cf::future<Response> connect(const nx::utils::Url& url, const StringType& targetHost);

    using AsyncClient::request;
    cf::future<Response> request(Method::ValueType method, const nx::utils::Url& url);
};
