...
{
    m_downChannelHttpClient = std::make_unique<AsyncClient>(ssl::kDefaultCertificateCheck);
...
{
    m_upChannelHttpClient = std::make_unique<AsyncClient>(ssl::kDefaultCertificateCheck);
    if (m_timeout)
    {
...
{
    // TODO: #akolesnikov Remove if and pass std::unique_ptr<AsyncClient> to this method.
...
void ExperimentalTunnelClient::prepareOpenUpChannelRequest()
{
    ...
    // TODO: #akolesnikov HttpServerConnection currently does not support
...
void ExperimentalTunnelClient::reportTunnelIfReady()
{
    ...
    m_connection = std::make_unique<SeparateUpDownChannelDelegate>(
        ...
        SeparateUpDownChannelDelegate::Purpose::client);
