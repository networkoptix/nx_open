...
public:
        ...
        const network::http::RequestContext& requestContext) const
    {
        const auto xForwardedProtoValue = getHeaderValue(
            requestContext.request.headers, header::XForwardedProto::NAME);
        const bool isSslConnection = requestContext.connection->isSsl()
            || (requestContext.connection->socket()->getForeignAddress().address.isLocalNetwork()
