...
NX_NETWORK_API nx::log::LoggerSettings toLoggerSettings(const Logger& loggerInfo);

NX_NETWORK_API std::set<nx::log::Filter> toFilters(const std::vector<Filter>& filters);
 ...
 */
NX_NETWORK_API std::set<nx::log::Filter> toFilters(
    const nx::log::LevelFilters& levelFilters);

NX_NETWORK_API nx::log::LevelFilters toLevelFilters(const std::vector<Filter>& filters);
...
NX_NETWORK_API Logger toLoggerInfo(
    const nx::log::AbstractLogger* logger,
    const std::set<nx::log::Filter>& effectiveFilters,
    int id);
