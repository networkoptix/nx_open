...
SocketAddress StreamProxy::setProxyDestination(
    const SocketAddress& newDestination)
{
    NX_MUTEX_LOCKER lock(&m_mutex);
    return std::exchange(m_destinationEndpoint, newDestination);
}
    ...
    std::unique_ptr<AbstractStreamSocket> connection)
{
    ...
    {
        NX_DEBUG(this, nx::format("Proxy to %1. Accept connection error. %2")
    ...
    SystemError::ErrorCode completionCode)
{
...

    {
        ...
        NX_VERBOSE(this,
            nx::format("Proxy to %1. Removing completed proxy channel. Completion code: %2")
...
void StreamProxyChannel::start(ProxyCompletionHandler completionHandler)
{
    ...
    m_destinationConnection = SocketFactory::createStreamSocket(ssl::kAcceptAnyCertificate);
    ...
    if (!tuneDestinationConnectionAttributes())
    {
        ...
        NX_DEBUG(this, nx::format("Proxy to %1. Failed to prepare destination connection. %2")
