...
struct SocketGlobalsImpl
{
    ...
    std::map<std::string, QRegExp> hostDisabledProgrammatically;
    std::map<std::string, QRegExp> hostDisabledViaIni;
...
void SocketGlobals::blockHost(const std::string& regexpString)
{
    ...
    m_impl->hostDisabledProgrammatically.emplace(
        regexpString,
        QRegExp(QString::fromStdString(regexpString), Qt::CaseInsensitive, QRegExp::Wildcard));
}
...
static bool matchHost(
    const std::map<std::string, QRegExp>& patterns,
    const HostAddress& host)
{
    for (const auto& p: patterns)
    {
        if (p.second.exactMatch(QString::fromStdString(host.toString())))
            return true;
    }
...
void SocketGlobals::initializeNetworking(const utils::ArgumentParser& arguments)
{
    ...
    arguments.read("aio-thread-pool-size", &aioThreadCount);
