...
std::string NX_NETWORK_API toString(TestTrafficLimitType type)
{
    ...
    NX_CRITICAL(false, nx::format("Unexpected value: %1").arg(static_cast<int>(type)));
    return std::string();
}
...
:
    TestConnection(
        SocketFactory::createStreamSocket(ssl::kAcceptAnyCertificate),
        SocketAddress(
...
TestConnection::~TestConnection()
{
    NX_VERBOSE(this, nx::format("accepted %1. Destroying...").arg(m_accepted));
...
AbstractStreamSocket& TestConnection::socket()
{
    return *m_socket;
}
    ...
    m_lastSequenceReceived(0)
{
    ...
    m_outData = nx::utils::random::generate(m_readBufferSize).toStdString();
...
void TestConnection::onConnected( SystemError::ErrorCode errorCode )
{
    ...
    {
        NX_WARNING(this, nx::format("Connect to %1 error: %2")
...
void TestConnection::startSpamIO()
{
    ...
    NX_VERBOSE(this, nx::format("accepted %1. Sending %2 bytes of data to %3")
    ...
    size_t bytesRead)
{
    if (errorCode == SystemError::noError)
    {
        NX_VERBOSE(this, nx::format("accepted %1. Received %2 bytes of data")
            .arg(m_accepted).arg(bytesRead));
    }
    else
    {
        NX_WARNING(this, nx::format("accepted %1. Receive error: %2")
    ...
    size_t bytesWritten )
{
    if (errorCode != SystemError::noError && errorCode != SystemError::timedOut)
    {
        NX_WARNING(this, nx::format("accepted %1. Send error: %2")
            .arg(m_accepted).arg(SystemError::toString(errorCode)));
...
void TestConnection::prepareConsequentDataToSend(nx::Buffer* buffer)
{
    for (char*
        pos = buffer->data();
...
void TestConnection::verifyDataReceived(const nx::Buffer& buf, size_t bytesRead)
{
...
std::string toString(const ConnectionTestStatistics& data)
{
    return nx::format("Connections online: %1, total: %2. Bytes in/out: %3/%4.")
...
bool RandomDataTcpServer::start(std::chrono::milliseconds rwTimeout)
{
    ...
    {
        NX_ERROR(this, "Error starting server on %1. %2",
            m_localAddress, SystemError::getLastOSErrorText());
    ...
    std::unique_ptr<AbstractStreamSocket> newConnection)
{
    ...
    if (errorCode == SystemError::noError)
    {
        ...
        NX_DEBUG(this, nx::format("Accepted connection %1. local address %2")
...
void ConnectionsGenerator::setSocketFactory(SocketFactoryFunc func)
{
    m_socketFactoryFunc = std::move(func);
}

void ConnectionsGenerator::setConfigureNewSocketFunc(ConfigureSocketFunc func)
{
    m_configureSocketFunc = std::move(func);
}
    ...
    SystemError::ErrorCode code)
{
    ...
    NX_DEBUG(this, nx::format("Connection %1 has finished: %2")
    ...
    std::unique_lock<std::mutex>* const /*lock*/)
{
    while (m_connections.size() < m_maxSimultaneousConnectionsCount)
    {
        auto connection = std::make_unique<TestConnection>(
            ...
            m_trafficLimit,
            m_transmissionMode);
        if (m_configureSocketFunc)
            m_configureSocketFunc(&connection->socket());
...
{
    NX_MUTEX_LOCKER lock(&m_mutex);
    SocketAddress key(nx::utils::buildString('a', std::to_string(m_currentNumber++)));
...
void AddressBinder::add(const SocketAddress& key, SocketAddress address)
{
    ...
    NX_DEBUG(this, nx::format("New address %1 is bound to %2").args(address, key));
}
...
void AddressBinder::remove(const SocketAddress& key, const SocketAddress& address)
{
    ...
    NX_DEBUG(this, nx::format("Address %1 is unbound from %2").args(address, key));
}
...
void AddressBinder::remove(const SocketAddress& key)
{
    ...
    NX_CRITICAL(it != m_map.end());
    m_map.erase(it);
    NX_DEBUG(this, nx::format("Key %1 is removed").arg(key));
}
...
std::optional<SocketAddress> AddressBinder::random(const SocketAddress& key) const
{
    NX_MUTEX_LOCKER lock(&m_mutex);
    const auto it = m_map.find(key);
...
SocketAddress MultipleClientSocketTester::modifyAddress(const SocketAddress& address)
{
    if (m_address == SocketAddress())
    {
        ...
        NX_VERBOSE(this, nx::format("Using %2 instead of %1").args(address, m_address));
    }
