    ...
    OnNewConnectionHandler handler)
{
    ...
    NX_VERBOSE(this, nx::format("cross-nat %1. New stream socket has been requested")
        .arg(m_connectionId));
...
void OutgoingTunnelConnection::proceedWithConnection(
    ...
    std::chrono::milliseconds timeout)
{
    NX_VERBOSE(this, nx::format("cross-nat %1. timeout %2")
    ...
    NX_VERBOSE(this, nx::format("cross-nat %1. Initiating async connect to %2 with timeout %3")
...
void OutgoingTunnelConnection::reportConnectResult(
    ...
    SystemError::ErrorCode errorCode)
{
    NX_VERBOSE(this, nx::format("cross-nat %1. result: %2")
        .arg(m_connectionId).arg(SystemError::toString(errorCode)));
...
{
    NX_DEBUG(this, nx::format("cross-nat %1. Control connection has been closed: %2")
    ...
    nx::network::stun::Message message)
{
    ...
    if (synAck.parse(message))
    {
        ...
        {
            NX_DEBUG(this, nx::format("cross-nat %1. Received SYN response with unexpected "
    ...
    {
        NX_DEBUG(this, nx::format("cross-nat %1. Failed to parse SYN response")
            .arg(m_connectionId));
    }

    NX_VERBOSE(this, nx::format("cross-nat %1. Control connection has been verified")
        .arg(m_connectionId));
}
