...
{
    return nx::format("UDP hole punching acceptor. remote endpoints %1")
    ...
    nx::hpm::api::ResultCode code)
{
    ...
    {
        NX_WARNING(this, "connectionAck error: %1", code);

        // TODO: #muskov code translation
        return executeAcceptHandler(SystemError::connectionAbort);
    }

    if (!m_holePunchingEnabled)
    {
        NX_VERBOSE(this, nx::format("Interrupting UDP hole punching due to configuration"));
    ...
    const SocketAddress& target)
{
    ...
    (*socketIt)->connectAsync(
        ...
        [this, socketIt, target](SystemError::ErrorCode code)
        {
            ...
            NX_DEBUG(this, nx::format("Rendezvous UDT connection from %1 to %2 result: %3, "
