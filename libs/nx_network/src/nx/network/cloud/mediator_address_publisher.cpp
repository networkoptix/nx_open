    ...
    m_cloudCredentialsProvider(cloudCredentialsProvider)
{
    ...
    m_mediatorConnection->setOnReconnectedHandler(
        [this]()
        {
            NX_VERBOSE(this, nx::format("Mediator client reported reconnect"));
...
void MediatorAddressPublisher::updateAddresses(
    ...
    nx::MoveOnlyFunc<void(nx::hpm::api::ResultCode)> updateHandler)
{
    m_mediatorConnection->dispatch(
        [this, addresses = std::move(addresses), handler = std::move(updateHandler)]() mutable
        {
            ...
            NX_DEBUG(this, nx::format("New addresses: %1").container(m_serverAddresses));
            publishAddressesIfNeeded();
        });
}
...
void MediatorAddressPublisher::publishAddressesIfNeeded()
{
    ...
    {
        NX_VERBOSE(this, nx::format("No need to publish addresses: they are already published. Reporting success..."));
    ...
    {
        NX_VERBOSE(this, nx::format("Publish address request has already been issued. Ignoring new one..."));
        return;
    }
...
{
    NX_VERBOSE(this, nx::format("Issuing bind request to mediator..."));
    ...
    m_mediatorConnection->bind(
        ...
        {
            NX_DEBUG(this, nx::format("Publish addresses (%1) completed with result %2")
                .container(m_publishedAddresses).arg(toString(resultCode)));
