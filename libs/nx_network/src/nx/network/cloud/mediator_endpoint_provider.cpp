    ...
    FetchMediatorEndpointsCompletionHandler handler)
{
    ...
    m_mediatorUrlFetcher->get(
            ...
            nx::utils::Url udpUrl)
        {
            ...
            {
                NX_DEBUG(this, nx::format("Fetched mediator tcp (%1) and udp (%2) URLs")
                    .args(tcpUrl, udpUrl));
            ...
            {
                NX_DEBUG(this, nx::format("Cannot fetch mediator address. HTTP %1")
                    .arg((int)resultCode));
            }
...
void MediatorEndpointProvider::initializeUrlFetcher()
{
    ...
    else
    {
        m_mediatorUrlFetcher->setModulesXmlUrl(
            network::AppInfo::defaultCloudModulesXmlUrl(m_cloudHost));
    }
}
