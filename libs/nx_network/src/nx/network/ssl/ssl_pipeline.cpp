...
const std::function<bool(bool isPreverifyOk, X509_STORE_CTX* x509Ctx, const Pipeline* pipeline)>
    kDefaultCertificateCheckCallback =
        [](bool /*isPreverifyOk*/, X509_STORE_CTX* x509Ctx, const Pipeline* pipeline)
        {
    ...
    m_ssl(nullptr, &SSL_free)
{
    ...
    setVerifyCertificateCallback(std::move(verifyCertificateCallback));
}
...
void Pipeline::setVerifyCertificateCallback(VerifyCertificateCallback func)
{
    ...
    if (m_verifyCertificateCallback)
...
int Pipeline::verifyServerCertificateCallback(int preverifyOk, X509_STORE_CTX* x509Ctx)
{
    ...
    return pipeline->m_verifyCertificateCallback(preverifyOk != 0, x509Ctx, pipeline) ? 1 : 0;
}
...
ConnectingPipeline::ConnectingPipeline(
    VerifyCertificateCallback verifyCertificateCallback, Context* context)
    :
    Pipeline(
        context ? context : Context::instance(),
        (context ? context : Context::instance())->clientContext(),
        std::move(verifyCertificateCallback))
{
