    ...
    m_proxyConverter(nullptr)
{
    ...
    if (isServerSide)
        ...
        m_sslPipeline = std::make_unique<ConnectingPipeline>(std::move(verifyCertificateCallback), context);
...
ClientStreamSocket::ClientStreamSocket(
    ...
    :
    base_type(
        context, std::move(delegate), /*isServerSide*/ false, std::move(verifyCertificateCallback))
{
}
