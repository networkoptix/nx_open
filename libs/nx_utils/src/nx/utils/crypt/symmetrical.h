 ...
 */
NX_UTILS_API QByteArray generateAesExtraKey();

/**
 * Encode data with the provided key using XOR cypher.
 */
NX_UTILS_API QByteArray encodeSimple(
 ...
 */
NX_UTILS_API QByteArray encodeAES128CBC(
    const QByteArray& data,
 ...
 * '\0' symbol. E.g. these below work fine with utf-8 strings, but not with utf-16(32) ones.
 */
NX_UTILS_API QByteArray decodeAES128CBC(
    const QByteArray& data,
    const std::array<uint8_t, 16>& key);
 ...
 * If key is not provided at all, default hardcoded key is used.
 */
NX_UTILS_API QString encodeHexStringFromStringAES128CBC(
 ...
 * If key is not provided at all, default hardcoded key is used.
 */
NX_UTILS_API QString decodeStringFromHexStringAES128CBC(
    const QString& s, const QByteArray& key = QByteArray());
