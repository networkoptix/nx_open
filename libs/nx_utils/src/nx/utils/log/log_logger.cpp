...
void Logger::setSettings(const LoggerSettings& loggerSettings)
{
    ...
    //< TODO: As of now, there is no way to address the "unused variable" warning when using structured binding here.
    for (const auto& filterAndLevel: m_settings.level.filters)
        m_filters.insert(filterAndLevel.first);
}
