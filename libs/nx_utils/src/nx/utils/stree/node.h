...
// TODO: #akolesnikov: Remove when NX_LOG supports filtering by message class.
...
public:
        ...
        AbstractResourceWriter* const out) const override
    {
        ...
        if (!in.get(m_matchResId, &value))
        {
            #if defined(NX_STREE_ENABLE_DEBUG_LOGGING)
                NX_VERBOSE(this,
                    nx::format("Condition. Resource (%1) not found in input data").arg(m_matchResId));
            #endif
            return;
        }
        ...
        if (it == m_children.end())
        {
            #if defined(NX_STREE_ENABLE_DEBUG_LOGGING)
                NX_VERBOSE(this,
                    nx::format("Condition. Could not find child by value %1").arg(value.toString()));
            #endif
            return;
        }

        #if defined(NX_STREE_ENABLE_DEBUG_LOGGING)
            NX_VERBOSE(this, nx::format("Condition. Found child with value %1 by search value %2")
