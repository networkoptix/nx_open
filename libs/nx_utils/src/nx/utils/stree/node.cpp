...
void ResPresenceNode::get(const AbstractResourceReader& in, AbstractResourceWriter* const out) const
{
        ...
        NX_VERBOSE(this, nx::format("Condition. Selecting child by resource %1").arg(m_matchResId));
    #endif
    ...
    if (!m_children[intVal])
    {
        #if defined(NX_STREE_ENABLE_DEBUG_LOGGING)
            NX_VERBOSE(this,
                nx::format("Presence Condition. Could not find child by value %1").arg(intVal));
        #endif
        return;
    }

    #if defined(NX_STREE_ENABLE_DEBUG_LOGGING)
        NX_VERBOSE(this, nx::format("Presence Condition. Found child by search value %1").arg(intVal));
    #endif
