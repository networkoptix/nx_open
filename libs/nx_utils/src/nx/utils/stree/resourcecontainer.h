...
public:
    ...
    virtual std::vector<QVariant> getAsVariantList(int resId) const
    {
        std::vector<QVariant> result;
        QVariant head;
        if (getAsVariant(resId, &head))
    ...
    std::vector<std::string> getList(int resId) const
    {
        std::vector<std::string> result;
        std::vector<QVariant> untypedValueList = getAsVariantList(resId);
        for (const auto& untypedValue : untypedValueList)
        {
            if (untypedValue.canConvert<QString>())
                result.emplace_back(untypedValue.toString().toStdString());
            else if (untypedValue.canConvert<QnUuid>())
                result.emplace_back(untypedValue.value<QnUuid>().toStdString());
            else
                return {};
        }
        return result;
    ...
    template<typename T> std::optional<T> get(int resID) const
    {
        T val;
        if (!get(resID, &val))
            return std::nullopt;
    ...
    std::optional<QVariant> get(int resID) const
    {
        QVariant val;
        if (!get(resID, &val))
            return std::nullopt;
...
public:
    ...
    void putStr(int resID, const std::string_view& value);
    void putStr(int resID, const char* value);
};
...
NX_UTILS_API void copy(const AbstractIteratableContainer& from, AbstractResourceWriter* to);
...
public:
    ...
    std::string toString(const nx::utils::stree::ResourceNameSet& rns) const;
    bool empty() const;
...
{
public:
    SingleResourceContainer() = default;
...
{
public:
    // TODO: #akolesnikov Make initializer a variadic template func.
    ...
    virtual std::vector<QVariant> getAsVariantList(int resId) const override;
...
// TODO: #akolesnikov using variadic template add method makeMultiSourceResourceReader that accepts any number of aguments.
