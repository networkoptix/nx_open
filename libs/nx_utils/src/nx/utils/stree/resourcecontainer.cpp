...
void AbstractResourceWriter::putStr(int resID, const std::string_view& value)
{
    put(resID, QString::fromUtf8(value.data(), value.size()));
}

void AbstractResourceWriter::putStr(int resID, const char* value)
{
    put(resID, QString::fromUtf8(value));
}

//-------------------------------------------------------------------------------------------------

void copy(const AbstractIteratableContainer& from, AbstractResourceWriter* to)
{
    for (auto it = from.begin(); !it->atEnd(); it->next())
        to->put(it->resID(), it->value());
}
...
std::string ResourceContainer::toString(const nx::utils::stree::ResourceNameSet& rns) const
{
    return nx::utils::join(
        m_mediaStreamPameters.begin(), m_mediaStreamPameters.end(), ", ",
        [&rns](std::string* out, const auto& item)
        {
            return buildString(out,
                rns.findResourceByID(item.first).name, ':', item.second.toString().toStdString());
        });
}
...
std::vector<QVariant> MultiSourceResourceReader::getAsVariantList(int resId) const
{
    for (size_t i = 0; i < m_elementCount; ++i)
    {
        if (auto list = m_readers[i]->getAsVariantList(resId); !list.empty())
            return list;
    }
