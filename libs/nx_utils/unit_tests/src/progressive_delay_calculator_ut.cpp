...
{
    ProgressiveDelayCalculator calculator({kInitialDelay, /*delayMultiplier*/ 2, kMaxDelay, 0});
    ...
    ProgressiveDelayCalculator calculator1 =
        ProgressiveDelayCalculator({kInitialDelay, /*delayMultiplier*/ 0, kMaxDelay, 0});
    ASSERT_EQ(calculator1.calculateNewDelay(), kInitialDelay);
    ASSERT_EQ(calculator1.calculateNewDelay(), kInitialDelay);

    ProgressiveDelayCalculator calculator2 =
        ProgressiveDelayCalculator({kInitialDelay, /*delayMultiplier*/ 1, kMaxDelay, 0});
    ASSERT_EQ(calculator2.calculateNewDelay(), kInitialDelay);
    ASSERT_EQ(calculator2.calculateNewDelay(), kInitialDelay);

    ProgressiveDelayCalculator calculator3 =
        ProgressiveDelayCalculator({kInitialDelay, /*delayMultiplier*/ 10, kInitialDelay, 0});
