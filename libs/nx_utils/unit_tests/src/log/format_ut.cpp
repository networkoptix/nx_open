...
TEST(Formatter, str)
{
    ...
    EXPECT_EQ(QLatin1String("QString"), nx::format("%1").arg(QString::fromUtf8("QString")));
    EXPECT_EQ(QLatin1String("MethodTester"), nx::format("%1").arg(MethodTester()));
    EXPECT_EQ(QLatin1String("FunctionTester"), nx::format("%1").arg(FunctionTester()));

    EXPECT_EQ(QLatin1String("A B C D"), nx::format("%1 %2 %3").arg("A").arg("B C").arg("D"));
    EXPECT_EQ(QLatin1String("same same"), nx::format("%1 %2").arg("%2").arg("same"));
