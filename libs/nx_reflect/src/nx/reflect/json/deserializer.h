    ...
    json::DeserializationFlag skipErrors = json::DeserializationFlag::none)
{
    ...
    if (!document.IsObject() && !document.IsArray())
        return DeserializationResult(
            false, "Either an object or an array is expected as a json object", std::string(json));
...
{
    Data data;
    return std::make_tuple(std::move(data), deserialize<Data>(json, &data, skipErrors));
}
