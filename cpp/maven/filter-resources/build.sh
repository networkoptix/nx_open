...
if [ "${box}" == "rpi" ] || [ "${box}" == "bpi" ]; then
     QTCHECK_ARM=`/usr/local/raspberrypi-tools/arm-bcm2708/gcc-linaro-arm-linux-gnueabihf-raspbian/bin/arm-linux-gnueabihf-ldd --root ${libdir}/lib/${build.configuration} ${qt.dir}/lib/libQt5Core.so | grep libglib-2.0.so.0`
