...
void main()
{
    ...
    float blockNum = floor(intCoord.y / 32.0) * blocksPerLine + floor(intCoord.x / 32.0);
    float offset = blockNum * (32.0*32.0) + mod(intCoord.y * 32.0, 32.0*32.0) + mod(intCoord.x, 32.0);
    vec2 mappedCoord = vec2((mod(offset, pixelWidth) + 0.5) / pixelWidth,
                            (floor(offset / pixelWidth) + 0.5) / pixelHeight);

    vec2 uvPixelSize = vec2(pixelWidth / 2.0, pixelHeight / 2.0);
    float blockNumUV = floor(intCoord.y / 64.0) * blocksPerLine + floor(intCoord.x / 32.0);
    float offsetUV = blockNumUV * (16.0*32.0) + mod(intCoord.y * 8.0 , 32.0*16.0) + mod(intCoord.x, 16.0);
