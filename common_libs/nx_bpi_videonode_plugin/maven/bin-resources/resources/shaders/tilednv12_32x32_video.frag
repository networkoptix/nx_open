...
uniform mediump float halfTextureHeight;
...
void main()
{
	...
	mediump float halfY = floor(mod(plane1TexCoord.y, halfTextureHeight));
	...
	mediump vec2 mappedCoord = vec2(
			...
			(blockNum + 0.5) / yBlocksCount);

	mediump vec2 mappedCoordUV = vec2(
			(yInBlockUV * 16.0 + xInBlockUV + 0.5) / 512.0,
			(blockNumUV + 0.5) / yBlocksCount);


	mediump float Y = texture2D(plane1Textures[int(plane1TexCoord.y / halfTextureHeight)], mappedCoord).r;
