...
ClientConnectionValidator::ClientConnectionValidator(
    ...
    :
    m_messagePipeline(this, std::move(connection))
{
    ...
    nx_http::tunneling::ValidateTunnelCompletionHandler handler)
{
    post(
        ...
        {
            NX_VERBOSE(this, lm("Validating STUN connection to %1")
                .args(m_messagePipeline.socket()->getForeignAddress()));
...
void ClientConnectionValidator::closeConnection(
    SystemError::ErrorCode closeReason,
    ConnectionType* /*connection*/)
{
    processConnectionClosure(closeReason);
}
...
void ClientConnectionValidator::processMessage(Message /*message*/)
{
    ...
    NX_VERBOSE(this, lm("STUN connection to %1 has been validated")
        .args(m_connection->getForeignAddress()));

    nx::utils::swapAndCall(m_completionHandler, nx_http::tunneling::ResultCode::ok);
}
...
{
    NX_DEBUG(this, lm("Failed to validate STUN connection to %1. %2")
        .args(m_messagePipeline.socket()->getForeignAddress(), SystemError::toString(reasonCode)));

    m_messagePipeline.pleaseStopSync();

    if (m_completionHandler)
        nx::utils::swapAndCall(m_completionHandler, nx_http::tunneling::ResultCode::ioError);
}
