...
void DebugFps::mark(const char* extraPrefix)
{
    if (d)
    {
        ...
        if (d->prevTimeUs != 0)
        {
            ...
            PRINT << stringFormat("%s[%s] FPS avg %4.1f, dt %3d ms, avg dt %3d ms",
...
void DebugTimer::finish(const char* tag)
{
    if (d)
    {
        ...
        for (int i = 0; i < (int) d->list.size(); ++i)
        {
            ...
            s.append(stringFormat(": %3d ms", int((500 + d->list.at(i)) / 1000)));
        }
        if (d->list.size() > 0)
            s.append(", ");
        s.append(stringFormat("last: %3d ms", int((500 + t) / 1000)));
        const char* const OUTPUT_PREFIX = d->outputPrefix; //< Will be used by PRINT macro.
        PRINT << "TIME [" << d->name << "]: " << s;
    }
}
