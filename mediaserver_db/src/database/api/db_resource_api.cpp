...
QueryCache::QueryCache(Pool* pool) : m_pool(pool) { m_pool->m_caches.insert(this); }

QueryCache::~QueryCache() { m_pool->m_caches.erase(this); }
...
QueryContext::QueryContext(const QSqlDatabase& database, QueryCache::Pool* cachePool)
    : m_database(database), m_getId(cachePool), m_insert(cachePool), m_update(cachePool)
{
}

const QSqlDatabase& QueryContext::database() const { return m_database; }
...
bool insertOrReplaceResource(
    QueryContext* context, const nx::vms::api::ResourceData& data, qint32* internalId)
{
    ...
    *internalId = getResourceInternalId(context, data.id);
    ...
    if (*internalId)
    {
        query = context->update([&](QSqlQuery* q) {
    ...
    {
        query = context->insert([&](QSqlQuery* q) {
            const QString s = R"sql(
