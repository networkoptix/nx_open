...
ignore_attributes = ('for', 'src', 'id', 'name', 'class', 'style',
                     'on-true', 'on-false', 'value', 'enctype', 'action', 'typeahead.*?', 'collapse',
                     'data-toggle','data-target', 'data-placement','data-dismiss',
                     'method', 'close', 'pattern',
                     'type', 'role', 'for', 'target',
                     'width', 'height', 'rows', 'cols', 'maxlength', 'minlength', 'min', 'max', 'colspan', 'rowspan',
                     'live-only', 'records-provider', 'position-provider', 'position-handler', 'play-handler', 'can-play-live',
                     'autocomplete',
                     'process', 'form',
                     'ng-.*?', 'aria-.*?', 'chart-.*?', 'vg-.*?',
...
ignore_single_attributes = ('required', 'tc-chartjs-line', 'disabled', 'readonly', 'dropdown', 'autofocus', 'disable-autocomplete',
                            'validate-field', 'novalidate', 'uib-.*?', 'enter-button')
...
def process_attributes(data):
    ...
    data = re.sub(r'<!--.+?-->', '', data, flags=re.DOTALL | re.MULTILINE)    # Remove all html-comments
    ...
    for attr in ignore_attributes:  # Here we remove all attributes which do not contain language strings.
        data = re.sub('\s+' + attr + '=".+?"', '', data, flags=re.DOTALL | re.MULTILINE)    # remove ignore_attribute="some value" with double-quotes
