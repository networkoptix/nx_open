...
IF NOT [%PASSWORD%] == [] (
    IF NOT [%CERTIFICATE%] == [] (
        call ${environment.dir}\bin\signtool.exe sign /td sha256 /fd sha256 /tr http://tsa.startssl.com/rfc3161 /ac ${sign.intermediate.cer} /a /v /p ${sign.password} /d "${company.name} ${display.product.name}" /f ${certificates_path}/wixsetup/${sign.cer} %*
    ) ELSE (
