...
from framework.os_access.windows_remoting.cmd.powershell import start_powershell_script
...
def read_bytes(shell, path):
    script_template = '''
        ...
        '''
    data = bytearray()
    with start_powershell_script(shell, script_template.format(path=path)) as command:
