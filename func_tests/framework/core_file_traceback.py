...
GDB_TIMEOUT_SEC = 600  # Error in gdb commands cause gdb to not quit
...
def create_core_file_traceback(os_access, bin_path, lib_dir, core_path):
    ...
    traceback = os_access.ssh.run_command(
        ['gdb', '--quiet', '--command=%s' % commands_path],
        cwd=WORK_DIR, timeout_sec=GDB_TIMEOUT_SEC)
    return traceback
