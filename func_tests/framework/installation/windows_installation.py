...
class WindowsInstallation(Installation):
    def __init__(self, windows_access, local_installer_path):
    ...
    def _upload_installer(self):
        remote_path = self.os_access.Path.tmp() / self.local_installer_path.name
        if not remote_path.exists():
            remote_path.parent.mkdir(parents=True, exist_ok=True)
            copy_file(self.local_installer_path, remote_path)
        return remote_path
