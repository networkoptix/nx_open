...
class Camera(object):
    ...
    def _make_steps(self, server):  # types: (Mediaserver) -> generator
    ...
    def _make_runners(self, stage_rules):  # type: (dict) -> list
        runners = []
...
class Stand(object):
    def __init__(self, server, config): # type: (Mediaserver, dict) -> None
        ...
        self.cameras = [Camera(server, i, self._stage_rules(c)) for i, c in config.iteritems()]
        self.start_time = time.time()

    def run_all_stages(self, cycle_dalay_s=1):
        ...
        while True:
            ...
            _logger.debug('Wait for cycle delay {} seconds, {} cameras left'.format(
                cycle_dalay_s, cameras_left))

            time.sleep(cycle_dalay_s)
    ...
    def _stage_rules(self, rules):
        ...
        for name, rule in conditional.iteritems():
