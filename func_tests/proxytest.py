...
def compareResults(a, b):
    ...
    if a.length != b.length:
        log(5, "Function %s. Different data lengths returned by %s (%s) and %s (%s)" %
...
#        except HTTPException as err:
            log(3, "FAIL: %s raised %s" % (action, err))
            raise FuncTestError("error " + action, str(err))

    def run(self):
        log(5, "\n=======================================")
        log(5, "Proxy Test Start")
        try:
            self.getUids()
            ...
            #    if not diff.hasDiff():
            #        raise FuncTestError("")
            log(5, "Test complete. Responses are the same.")
            return True
