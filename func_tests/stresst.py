...
class StressTestRunner(object):
    ...
    def print_tail(self):
        ...
        log(5, "Last %.1f seconds: %s" % (tail_end - tail_start, self._stat_str(tail_end - tail_start, fails)))
    ...
    def _check_server_hung(self):
        ...
        if fails[None] == 0 or fails[None] == self._tail[i][1][None]: # i.e. there was no more successes after i's
            log(3,  "FAIL: No successes for the last %.1f seconds!" % (passed - self._tail[i][0],))
            self._hang = True
            return True
        return False
    ...
    def simple_test(self, setBatch=None):
        if setBatch is not None:
            self._batch = setBatch
        log(5, "Testing with %s parallel workers. %s" % (
            self._threadNum,
        ...
        if self._assert:
            ...
            if self._batch is not None:
                log(3, "FAIL:" if self._hang else "OK")
        del self._workers[:]
    ...
    def full_test(self):
        self.initSteps()
        log(5, "Test 1: Normal requests.")
        self.simple_test(self.steps[0])
        if not self.wasHang():
            log(5, "Test 2: Flood of interrupted requests, than try some normal ones.")
            self._reinit()
...
def preprocessArgs(args):
    ...
    if args.reports is not None:
        if args.reports > 0:
            ...
            log(3, "ERROR: Wrong report period value: %s" % args.reports)
            sys.exit(1)
    if args.batch is not None or args.full is not None:
        args.mix = True
    if args.mix and (args.heavy or args.proto):
        log(3, "ERROR: --proto and --heavy options meaningless in the mix, batch and full modes.")
        sys.exit(1)
    host = args.host.split(':', 1)
    if len(host) > 1:
        if host[1] != str(args.port):
            log(3, "ERROR: Hostname contains a port number %s and it's not equal to the option --port value %s" % (host[1], args.port))
            sys.exit(1)
