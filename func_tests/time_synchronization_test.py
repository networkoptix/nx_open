...
def system(box, server_factory):
    ...
    primary_box_time = system.primary.os_access.set_time(BASE_TIME)
...
def test_primary_follows_box_time(system):
    primary_box_time = system.primary.os_access.set_time(BASE_TIME + timedelta(hours=20))
    assert wait_until(lambda: system.primary.get_time().is_close_to(primary_box_time)), (
...
def test_change_primary_server(system):
    ...
    new_primary_box_time = new_primary.os_access.set_time(BASE_TIME + timedelta(hours=5))
