...
def env_networks(env_builder, box, server):
    net_1 = '10.10.1/24'
    net_2 = '10.10.2/24'
    box_1 = box(ip_address_list=[net_1])
    box_2 = box(ip_address_list=[net_2])
...
def test_proxy_request_to_another_network(env_networks):
    env_networks.middle.merge_systems(env_networks.one)
    env_networks.middle.merge_systems(env_networks.two)
    test_response = env_networks.one.rest_api.ec2.testConnection.GET(
        headers={'X-server-guid': env_networks.two.ecs_guid})
    assert test_response['ecsGuid'] == env_networks.two.ecs_guid
    servers = env_networks.one.rest_api.ec2.getMediaServersEx.GET(
        id=env_networks.two.ecs_guid,
        headers={'X-server-guid': env_networks.two.ecs_guid})
