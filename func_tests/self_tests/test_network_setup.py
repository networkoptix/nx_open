...
def test_setup_basic(vm_types, hypervisor):
    machines_conf = [
        {'alias': 'first', 'type': 'linux'},
        {'alias': 'second', 'type': 'linux'},
        {'alias': 'router-1', 'type': 'linux'},
        {'alias': 'router-2', 'type': 'linux'},
        ]
    structure = {
        '10.254.1.0/24': {
            'first': None,
    ...
    with many_allocated(vm_types, machines_conf) as nodes:
        ip_addresses = setup_networks(nodes, hypervisor, structure, reachability)
