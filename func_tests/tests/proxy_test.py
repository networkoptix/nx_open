...
def test_ping(iterations, proxy, proxy_headers):
    with Session() as session:
        for _ in range(iterations):
            session.get(proxy.api.http.url('api/ping'), headers=proxy_headers).raise_for_status()
...
def test_ping_alternate_addressee(proxy, proxy_headers, iterations):
    ...
    with Session() as session:
        for _ in range(iterations):
            session.get(proxy.api.http.url('api/ping')).raise_for_status()
            session.get(proxy.api.http.url('api/ping'), headers=proxy_headers).raise_for_status()
    assert not proxy.installation.list_core_dumps()


def test_redirect(proxy, proxy_headers):
    auth = HTTPDigestAuth(proxy.api.http.user, proxy.api.http.password)
    with Session() as session:
        redirect_response = session.get(proxy.api.http.url(''), auth=auth, headers=proxy_headers, allow_redirects=False)
        assert 300 <= redirect_response.status_code <= 399, "Expected 3xx but got: %r" % redirect_response
        follow_url = proxy.api.http.url(redirect_response.headers['Location'])
...
def test_404(proxy, proxy_headers):
    auth = HTTPDigestAuth(proxy.api.http.user, proxy.api.http.password)
    with Session() as session:
        response = session.get(proxy.api.http.url('') + 'nonexistent', auth=auth, headers=proxy_headers)
