    ...
    m_decoder( decoder )
{
    ...
    NX_VERBOSE(this, QString::fromLatin1("QuickSyncVideoDecoder::QuicksyncDXVAPictureData::QuicksyncDXVAPictureData(). surf(%1) ref %2").
        arg((size_t)m_d3dSurface, 0, 16).arg(refCount));
}
...
#endif
{
    ...
    {
        NX_ERROR (this, QString::fromLatin1("Failed to initialize DXVA frame surface allocator. %1").arg(m_d3dFrameAllocator.getLastErrorText()));
        return;
    }
#endif
...
#endif
{
    ...
    NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder initialization start %1").arg(t1));
    if( !m_d3dFrameAllocator.initialize() )
    {
        NX_ERROR (this, QString::fromLatin1("Failed to initialize DXVA frame surface allocator. %1").arg(m_d3dFrameAllocator.getLastErrorText()));
        return;
    ...
    NX_DEBUG(this, QString::fromLatin1("DXVA frame surface allocator initialized in %1 ms").arg(t2-t1));
#endif
...
bool QuickSyncVideoDecoder::decode( const QnConstCompressedVideoDataPtr data, QSharedPointer<CLVideoDecoderOutput>* const outFrame )
{
    ...
    if( data && data->data.size() > 0 )
    {
        ...
        NX_VERBOSE(this, QString("QuickSyncVideoDecoder::decode. data.size = %1, current fps = %2, timer %3, millis since prev input frame %4%5").
            arg(data->data.size()).arg(m_sourceStreamFps).arg(getUsecTimer()/1000).arg(millisSincePrevInputFrame).
            arg(QString::fromLatin1(delayedInputFrame ? ". Delayed input frame" : "")));
    ...
    if( decoderProducedFrame )
    {
        if( m_totalOutputFrames == 0 )
            NX_DEBUG(this, QString::fromLatin1( "QuickSyncVideoDecoder. First decoded frame delayed for %1 input frame(s). clock %2" ).arg(m_totalInputFrames-1).arg(GetTickCount()));
        ++m_totalOutputFrames;
        if( currentClock - m_prevOutPictureClock > 1000 )
            NX_DEBUG(this, QString::fromLatin1( "QuickSyncVideoDecoder. Warning! There was no out picture for %1 ms" ).arg(currentClock - m_prevOutPictureClock));
    ...
    //NX_DEBUG(this, QString::fromLatin1( "QuickSyncVideoDecoder::decode. exit. timer %1, lr-timer %2" ).arg(currentClock).arg(GetTickCount()));
         ...
         )
    {
        ...
        while( (m_state != done) && (opStatus >= MFX_ERR_NONE) )
        {
            ...
            if( opStatus >= MFX_ERR_NONE && m_syncPoint )
            {
                ...
                NX_VERBOSE(this, QString::fromLatin1("QuickSyncVideoDecoder. sync operation result %1").arg(mfxStatusCodeToString(opStatus)),
        ...
        switch( opStatus )
        {
            case MFX_ERR_NONE:
            {
                if( !gotDisplayPicture )
                {
                    NX_LOG( QString::fromLatin1("QuickSyncVideoDecoder. Providing picture at output. Out frame timestamp %1, frame order %2, corrupted %3").
                        arg(decodedFrameCtx->mfxSurface.Data.TimeStamp).arg(decodedFrameCtx->mfxSurface.Data.FrameOrder).arg(decodedFrameCtx->mfxSurface.Data.Corrupted));
                    if( m_prevTimestamp > decodedFrameCtx->mfxSurface.Data.TimeStamp )
                    {
                        NX_INFO(this, QString::fromLatin1("QuickSyncVideoDecoder. Warning! timestamp decreased by %1. Current 90KHz timestamp %2, previous %3. Ignoring frame...").
                ...
                else
                {
                    ...
                    NX_WARNING (this, QString::fromLatin1("QuickSyncVideoDecoder. Warning! Got second picture from Intel media decoder during single decoding step! Ignoring..."));
                }
                break;
            }
            ...
            default:
            {
                ...
                {
                    NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. Recursion detected!"));
                    return false;
                }
                NX_ERROR(this, QString::fromLatin1("QuickSyncVideoDecoder. Critical error occured during Quicksync decode session %1").
                    arg(mfxStatusCodeToString(opStatus)));
        ...
        if( inputStream->DataLength > 0 )
        {
            ...
            if( inputStream->DataOffset == prevInputStreamDataOffset && inputStream->DataOffset > 0 )
            {
                ...
                NX_INFO(this, QString::fromLatin1("Warning! Intel media decoder left %1 bytes of data (status %2). Dropping these bytes... Total bytes dropped %3").
...
bool QuickSyncVideoDecoder::decode( mfxBitstream* const inputStream, QSharedPointer<CLVideoDecoderOutput>* const outFrame )
{
         ...
         )
    {
        ...
        for( ;; )
        {
            ...
            {
                NX_DEBUG(this, QString::fromLatin1("Can't find unused surface for decoding! Waiting for some async operation to complete..."));
            ...
            NX_DEBUG(this, QString::fromLatin1("Got unused surface for decoding. Proceeding with async operation"));
            break;
        }
        if( inputStream->DataLength == 0 )
            NX_DEBUG(this, QString::fromLatin1("Draining decoded frames with NULL input"));
        ...
        if( opStatus == MFX_ERR_NONE && opCtx.syncPoint )
        {
            ...
            NX_DEBUG(this, QString::fromLatin1("Started new async decode operation (pts %1). There are %2 operations in total").
                arg(inputStream->TimeStamp).arg(m_currentOperations.size()));
        }
...
void QuickSyncVideoDecoder::setOutPictureSize( const QSize& outSizeVal )
{
    ...
    NX_INFO (this, QString::fromLatin1("Quicksync decoder. Setting up scaling to %1x%2").arg(m_outPictureSize.width()).arg(m_outPictureSize.height()));
...
bool QuickSyncVideoDecoder::init( mfxU32 codecID, mfxBitstream* seqHeader )
{
    ...
    //NX_ERROR (this, QString::fromLatin1("mark1 %1 ms. %2").arg(t3-t2).arg(t3));
    ...
    //NX_ERROR (this, QString::fromLatin1("mark2 %1 ms. %2").arg(t4-t3).arg(t4));
    ...
    //NX_ERROR (this, QString::fromLatin1("mark3 %1 ms. Total %2 ms. %3").arg(t5-t4).arg(t5-t1).arg(t5));
    ...
    NX_DEBUG(this, QString::fromLatin1("Quicksync decoder initialization took %1 ms").arg(GetTickCount()-t1));
...
bool QuickSyncVideoDecoder::initMfxSession()
{
    ...
    if( m_parentSession )
    {
        ...
        {
            NX_ERROR (this, QString::fromLatin1("Failed to clone parent Intel media SDK session. Status %1").arg(mfxStatusCodeToString(status)));
            return false;
        }
        ...
        if( status != MFX_ERR_NONE )
        {
            NX_ERROR (this, QString::fromLatin1("Failed to join Intel media SDK session with parent session. Status %1").arg(mfxStatusCodeToString(status)));
            //m_mfxSession.Close();
            //return false;
        }
    }
    else
    {
        ...
        if( status != MFX_ERR_NONE )
        {
            NX_ERROR (this, QString::fromLatin1("Failed to create Intel media SDK session. Status %1").arg(mfxStatusCodeToString(status)));
            m_mfxSession.Close();
            return false;
        }
    }
    ...
    if( status != MFX_ERR_NONE )
    {
        NX_ERROR (this, QString::fromLatin1("Failed to set IDirect3DDeviceManager9 pointer to MFX session. Status %1").arg(mfxStatusCodeToString(status)));
        m_mfxSession.Close();
        return false;
    }
#endif
    ...
    if( status != MFX_ERR_NONE )
    {
        NX_ERROR (this, QString::fromLatin1("Failed to set buffer allocator to Intel media SDK session. Status %1").arg(mfxStatusCodeToString(status)));
        m_mfxSession.Close();
        return false;
    }
    ...
    if( status != MFX_ERR_NONE )
    {
        NX_ERROR (this, QString::fromLatin1("Failed to set frame allocator to Intel media SDK session. Status %1").arg(mfxStatusCodeToString(status)));
        m_mfxSession.Close();
        return false;
    }
...
bool QuickSyncVideoDecoder::initDecoder( mfxU32 codecID, mfxBitstream* seqHeader )
{
    ...
    NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. Initializing decoder..."));
    ...
    //NX_ERROR (this, QString::fromLatin1("mark01 %1 ms").arg(GetTickCount()));
    ...
    //NX_ERROR (this, QString::fromLatin1("mark01.4 %1 ms").arg(GetTickCount()));
...
#endif
    //NX_ERROR (this, QString::fromLatin1("mark01.5 %1 ms").arg(GetTickCount()));
    switch( status )
    {
        case MFX_ERR_NONE:
            break;
        ...
        case MFX_ERR_MORE_DATA:
            NX_WARNING (this, QString::fromLatin1("Need more data to parse sequence header"));
        ...
        default:
            if( status > MFX_ERR_NONE )
            {
                ...
                {
                    NX_WARNING (this, QString::fromLatin1("Warning. Intel Media SDK can't use hardware acceleration for decoding"));
                    m_hardwareAccelerationEnabled = false;
                }
                break;
            }
            NX_ERROR (this, QString::fromLatin1("Failed to decode stream header during Intel media SDK session. Status %1").arg(mfxStatusCodeToString(status)));
            m_decoder.reset();
            return false;
    }
    m_cachedSequenceHeader.clear();

    //NX_ERROR (this, QString::fromLatin1("mark01.6 %1 ms").arg(GetTickCount()));

    NX_INFO(this, QString::fromLatin1("Successfully decoded stream header with Intel Media decoder! Picture resolution %1x%2, stream profile/level: %3/%4").
    ...
    //NX_ERROR (this, QString::fromLatin1("mark01.7 %1 ms").arg(GetTickCount()));
    ...
    {
        NX_ERROR (this, QString::fromLatin1("Failed to query surface information for decoding from Intel media SDK session. Status %1").arg(mfxStatusCodeToString(status)));
        m_decoder.reset();
        return false;
    }
    ...
    //NX_ERROR (this, QString::fromLatin1("mark01.9 %1 ms").arg(GetTickCount()));

    allocateSurfacePool( &m_decoderSurfacePool, &decodingAllocRequest, &m_decodingAllocResponse );
    //NX_ERROR (this, QString::fromLatin1("mark02 %1 ms").arg(GetTickCount()));
    ...
    if( status < MFX_ERR_NONE )  //ignoring warnings
    {
        NX_ERROR (this, QString::fromLatin1("Failed to initialize Intel media SDK decoder. Status %1").arg(mfxStatusCodeToString(status)));
    ...
    //NX_ERROR (this, QString::fromLatin1("mark03. clock %1 ms").arg(GetTickCount()));
    ...
    NX_INFO(this, QString::fromLatin1("Intel Media decoder successfully initialized! Picture resolution %1x%2, "
...
bool QuickSyncVideoDecoder::initProcessor()
{
    ...
    NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. Initializing VPP processor..."));
    ...
    if( status < MFX_ERR_NONE )
    {
        NX_ERROR (this, QString::fromLatin1("Failed to query VPP. Status %1").arg(mfxStatusCodeToString(status)));
        m_processor.reset();
        return false;
    }
    else if( status > MFX_ERR_NONE )
    {
        NX_WARNING (this, QString::fromLatin1("Warning from quering VPP. Status %1").arg(mfxStatusCodeToString(status)));
    }

    status = m_processor->QueryIOSurf( &m_processingParams, request );
    if( status < MFX_ERR_NONE )
    {
        NX_ERROR (this, QString::fromLatin1("Failed to query surface information for VPP from Intel media SDK session. Status %1").arg(mfxStatusCodeToString(status)));
        m_processor.reset();
        return false;
    }
    else if( status > MFX_ERR_NONE )
    {
        NX_WARNING (this, QString::fromLatin1("Warning from quering VPP (io surf). Status %1").arg(mfxStatusCodeToString(status)));
    }
    ...
    if( status != MFX_ERR_NONE )
    {
        NX_ERROR (this, QString::fromLatin1("Failed to initialize Intel media SDK processor. Status %1").arg(mfxStatusCodeToString(status)));
        m_processor.reset();
        return false;
    }
    ...
    NX_INFO(this, QString::fromLatin1("Intel Media processor (scale to %1x%2) successfully initialized!. Allocated %3 surfaces in %4 memory").
...
bool QuickSyncVideoDecoder::initOpenCL()
{
    ...
    {
        NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. clGetPlatformIDs failed. %1").arg(prevCLErrorText()));
        return false;
    }
    if( numPlatforms == 0 )
    {
        NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. No OpenCL platform IDs found"));
        return false;
    }
    ...
    //if( m_prevCLStatus != CL_SUCCESS )
    //{
    //    NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. clGetPlatformInfo failed. %1").arg(prevCLErrorText()));
    //    return false;
    //}
    //NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. Supported CL extension: %1").arg(QString::fromLatin1(platformExtensionsStr)));
    ...
    {
        NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. Seems like no cl_intel_dx9_media_sharing extension on this platform... %1"));
        return false;
    }
    ...
    if( m_prevCLStatus != CL_SUCCESS )
    {
        NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. clGetDeviceIDs failed. %1").arg(prevCLErrorText()));
        return false;
    }
    if( numOfDevices == 0 )
    {
        NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. No OpenCL device IDs found"));
        return false;
    }
    ...
    if( m_prevCLStatus != CL_SUCCESS )
    {
        NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. clCreateContext failed. %1").arg(prevCLErrorText()));
        return false;
    }
        ...
        ++triesCounter )
    {
        ...
        switch( it->state )
        {
            ...
            case issWaitingVPP:
                ...
                if( !it->outPicture.data() )
                {
                    ...
                    NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. Cannot start VPP of frame (pts %1), since there is no unused vpp_out surface").
                        arg(it->decodedFrame->mfxSurface.Data.TimeStamp));
                    break;  //to next surface
                }
                ...
                NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. starting frame (pts %1) post-processing. Result %2").
                    arg(it->decodedFrame->mfxSurface.Data.TimeStamp).arg(mfxStatusCodeToString(opStatus)));
                break;
            ...
            //        //Quick Sync does not eat access_unit_delimiter at the end of a coded frame
            //        m_totalBytesDropped += inputStream->DataLength;
            //        NX_DEBUG(this, QString::fromLatin1("Warning! Intel media decoder left %1 bytes of data (status %2). Dropping these bytes... Total bytes dropped %3").
            //            arg(QString::number(inputStream->DataLength)).
            //            arg(QString::number(opStatus)).
            //            arg(QString::number(m_totalBytesDropped)),
            //            cl_logINFO );
        ...
        if( opStatus < MFX_ERR_NONE )
        {
            switch( opStatus )
            {
                ...
                case MFX_ERR_INCOMPATIBLE_VIDEO_PARAM:
                default:
                    NX_ERROR (this, QString::fromLatin1("Critical error occured during Quicksync decode session %1").arg(mfxStatusCodeToString(opStatus)));
                    resetMfxSession();
                    *operationReadyForDisplay = NULL;
                    return;
            }
    ...
    if( surfacePool == &m_decoderSurfacePool )
    {
        ...
        for( size_t i = 0; i < surfacePool->size(); ++i )
            if( surfacePool->at(i)->mfxSurface.Data.Locked == 0 )
                ++unusedSurfaces;
        NX_VERBOSE(this, QString::fromLatin1("There are %1 unused decoder surfaces").arg(unusedSurfaces));
    }
        ...
        for( size_t i = 0; i < surfacePool->size(); ++i )
        {
            ...
            if( surfaceCtx.syncCtx.usageCounter > 0 )
            {
                NX_LOG( QString::fromLatin1("QuickSyncVideoDecoder. Invalidated frame (0x%1) refs, waiting for refs to be released... (externalRefCounter = %2, Locked = %3)").
                    arg((size_t)&surfaceCtx.syncCtx, 0, 16).arg(surfaceCtx.syncCtx.externalRefCounter).arg(surfaceCtx.mfxSurface.Data.Locked));
            }
            //waiting for the surface to become unused (usage counter must drop to zero)
            ...
            _InterlockedDecrement16( (short*)&surfaceCtx.mfxSurface.Data.Locked );
            surfaceCtx.didWeLockedSurface = false;
            NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. Cancelled frame (0x%1) rendering (externalRefCounter = %2, Locked = %3). It took %4 ms").
                arg((size_t)&surfaceCtx.syncCtx, 0, 16).arg(surfaceCtx.syncCtx.externalRefCounter).arg(surfaceCtx.mfxSurface.Data.Locked).arg(GetTickCount()-t1));
            return surfacePool->at(i);
        }
    ...
    bool convertNV12ToYV12 )
{
    ...
    if( m_clContext && m_frameAllocator.gethdl( decodedFrameCtx->mfxSurface.Data.MemId, &handle ) == MFX_ERR_NONE )
    {
        ...
        if( m_prevCLStatus != CL_SUCCESS )
        {
            NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. clCreateFromDX9MediaSurfaceINTEL failed. %1").arg(prevCLErrorText()));
        }
    }
#endif
...
mfxStatus QuickSyncVideoDecoder::readSequenceHeader( mfxBitstream* const inputStream, mfxVideoParam* const streamParams )
{
        ...
        switch( *curNalu & 0x1f )
        {
            case nuSPS:
            {
                ...
                if( m_sps.get() )
                    ...
                    {
                        NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. SPS id changed from %1 to %2. Reinitializing...").
                            arg(m_sps->seq_parameter_set_id).arg(sps->seq_parameter_set_id));
                        m_reinitializeNeeded = true;
                    }
                    else if( (m_sps->pic_width_in_mbs != sps->pic_width_in_mbs) || (m_sps->pic_height_in_map_units != sps->pic_height_in_map_units) )
                    {
                        NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. Picture size in SPS has changed from %1.%2 to %3.%4. Reinitializing...").
                            arg(m_sps->getWidth()).arg(m_sps->getHeight()).arg(sps->getWidth()).arg(sps->getHeight()));
                        m_reinitializeNeeded = true;
                    }
    ...
    {
        NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. Found and parsed sequence header"));
        return MFX_ERR_NONE;
    }
...
{
    NX_ERROR (this, QString::fromLatin1("Resetting Quicksync decode session..."));
...
mfxStatus QuickSyncVideoDecoder::scaleFrame( const mfxFrameSurface1& from, mfxFrameSurface1* const to )
{
    ...
    //if( res != D3D_OK )
    //{
    //    NX_ERROR(this, QString::fromLatin1("QuickSyncVideoDecoder. Scaling. CheckDeviceFormatConversion failed. %1").
    //        arg(QString::fromWCharArray(DXGetErrorDescription(res))));
    ...
    if( res != D3D_OK )
    {
        NX_ERROR(this, QString::fromLatin1("QuickSyncVideoDecoder. Scaling. Failed to get src surface desc. %1").
            arg(QString::fromWCharArray(DXGetErrorDescription(res))));
        return MFX_ERR_UNKNOWN;
    }
    ...
    NX_VERBOSE(this, QString::fromLatin1("QuickSyncVideoDecoder. Scaling decoded frame to %1x%2...").
        arg(m_outPictureSize.width()).arg(m_outPictureSize.height()));
    ...
    if( res != D3D_OK )
    {
        NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. Scaling. Failed to lock d3d device. %1").arg(QString::fromWCharArray(DXGetErrorDescription(res))));
        return MFX_ERR_UNKNOWN;
    }
    ...
    if( res != D3D_OK )
    {
        NX_ERROR (this, QString::fromLatin1("QuickSyncVideoDecoder. Scaling. Failed to scale frame. %1").arg(QString::fromWCharArray(DXGetErrorDescription(res))));
        return MFX_ERR_UNKNOWN;
    }
    ...
    //NX_DEBUG(this, QString::fromLatin1("QuickSyncVideoDecoder. Frame has been successfully scaled to %1x%2. It took %3 ms (%4 ms to locking)").
    //    arg(m_outPictureSize.width()).arg(m_outPictureSize.height()).arg(t3-t1).arg(t2-t1));
...
mfxStatus QuickSyncVideoDecoder::doDecodingStep( mfxBitstream* const inputStream, mfxFrameSurface1** decodedFrame )
{
    ...
    for( ;; )
    {
...
#endif
        {
            ...
            NX_ERROR (this, QString::fromLatin1("No empty frame for decoding. Waiting for a frame to get free..."));
...
mfxStatus QuickSyncVideoDecoder::doProcessingStep( QSharedPointer<SurfaceContext> decodedFrameCtx, QSharedPointer<SurfaceContext>* const scaledFrameCtx )
{
    for( ;; )
    {
        *scaledFrameCtx = findUnlockedSurface( &m_scaledFramePool );
        ...
        {
            NX_ERROR (this, QString::fromLatin1("No empty frame for processing. Waiting for a frame to get free..."));
            //waiting for a surface to get free
            ::Sleep( MS_TO_WAIT_FOR_DEVICE );
            continue;
        }
