...
#include <nx/mediaserver_plugins/metadata/deepstream/openalpr/openalpr_callbacks.h>
...
OpenAlprPipelineBuilder::OpenAlprPipelineBuilder(Plugin* plugin):
    base_type(plugin)
{
}
    ...
    const gstreamer::ElementName& pipelineName)
{
    ...
    auto pipeline =
        std::make_unique<OpenAlprPipeline>(pipelineName, m_plugin);
