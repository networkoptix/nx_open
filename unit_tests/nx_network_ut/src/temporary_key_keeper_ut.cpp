...
TEST(TemporayKeyKeeper, MakeExpiration)
{
    TemporayKeyKeeper<nx::String> keeper({kTimeout, /*prolongLifeOnUse*/ false});
...
TEST(TemporayKeyKeeper, AddExpiration)
{
    TemporayKeyKeeper<nx::String> keeper({kTimeout, /*prolongLifeOnUse*/ false});
...
TEST(TemporayKeyKeeper, Prolongation)
{
    TemporayKeyKeeper<nx::String> keeper({kTimeout, /*prolongLifeOnUse*/ true});
...
TEST(TemporayKeyKeeper, Options)
{
    TemporayKeyKeeper<nx::String> keeper({kTimeout, /*prolongLifeOnUse*/ false});
    const auto key1 = keeper.make(*kUser1);
    const auto key2 = keeper.make(*kUser2);
    ...
    keeper.setOptions(TemporayKeyOptions{kTimeout * 2, /*prolongLifeOnUse*/ false});
    std::this_thread::sleep_for(kTimeout / 2);
    ASSERT_EQ(kUser1, keeper.get(key1)); //< Still accesible with new life time.
    ASSERT_EQ(kUser2, keeper.get(key2));

    keeper.setOptions(TemporayKeyOptions{kTimeout * 2, /*prolongLifeOnUse*/ true});
    ASSERT_EQ(kUser1, keeper.get(key1)); //< Prolong.
...
static void testBindings(TemporayKeyKeeper<nx::String>* keeper)
{
...
TEST(TemporayKeyKeeper, BindingUpdate)
{
    TemporayKeyKeeper<nx::String> keeper({kTimeout, /*prolongLifeOnUse*/ false});
    testBindings(&keeper);
...
TEST(TemporayKeyKeeper, BindingRemoveByValue)
{
    TemporayKeyKeeper<nx::String> keeper({kTimeout, /*prolongLifeOnUse*/ false});
    testBindings(&keeper);
...
TEST(TemporayKeyKeeper, BindingRemoveByBinding)
{
    TemporayKeyKeeper<nx::String> keeper({kTimeout, /*prolongLifeOnUse*/ false});
    testBindings(&keeper);
...
TEST(TemporayKeyKeeper, BindingExpiration)
{
    TemporayKeyKeeper<nx::String> keeper({kTimeout, /*prolongLifeOnUse*/ true});
    testBindings(&keeper);
