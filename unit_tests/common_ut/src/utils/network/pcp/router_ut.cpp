...
TEST(PcpRouter, StaticMap)
{
    ...
    const auto header = QByteArray("02010000100e0000") + dsh::reversed(localhost.ipv6()).toHex();
    const auto nonce = dsh::reversed(mapping.nonce).toHex();
    ...
    const auto header2 = QByteArray("02810000100e0000") + dsh::reversed(localhost.ipv6()).toHex();
    const auto response = header2 + nonce + QByteArray("0000000034123412") +
            dsh::reversed(external.ipv6()).toHex();
