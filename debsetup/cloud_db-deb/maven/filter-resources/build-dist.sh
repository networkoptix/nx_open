...
(cd $STAGEBASE; zip -y ./cloud_db-update-${platform}-${arch}-$VERSION.${buildNumber}.zip ./* -i *.*)
mv $STAGEBASE/cloud_db-update-${platform}-${arch}-$VERSION.${buildNumber}.zip ${project.build.directory}
