...
{
    static const char kEOFMarker = '\0';

    Q_D(QnTestCameraProcessor);
    char buffer[128];
    int messageSize = 0;
    while (!QByteArray::fromRawData(buffer, messageSize).contains(kEOFMarker))
    {
        if (m_needStop)
            return;

        int readed = d->socket->recv(buffer + messageSize, sizeof(buffer) - messageSize);
        if (readed <= 0)
        {
    ...
    QByteArray cameraUrl((const char*) buffer, messageSize);
