...
void QnResourceStatusWatcher::updateResourceStatusAsync(const QnResourcePtr &resource)
{
    ...
    if (resource.dynamicCast<QnMediaServerResource>())
        QnAppServerConnectionFactory::getConnection2()->getResourceManager(Qn::kSuperUserAccess)->setResourceStatusLocal(resource->getId(), resource->getStatus(), this, &QnResourceStatusWatcher::requestFinished2);
    else
        QnAppServerConnectionFactory::getConnection2()->getResourceManager(Qn::kSuperUserAccess)->setResourceStatus(resource->getId(), resource->getStatus(), this, &QnResourceStatusWatcher::requestFinished2);
}
