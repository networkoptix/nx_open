...
int QnCurrentUserRestHandler::executeGet(const QString &, const QnRequestParams &, QnJsonRestResult &result, const QnRestConnectionProcessor* owner) 
{
    ...
    {
        result.setError(QnJsonRestResult::CantProcessRequest, tr("Internal server error. Can't execute query 'getUsers'"));
        return CODE_INTERNAL_ERROR;
    }
    if (users.isEmpty()) {
        result.setError(QnJsonRestResult::CantProcessRequest, tr("Can't determine current user"));
        return CODE_INTERNAL_ERROR;
    }
