    ...
    bool isSearchAction)
{
    ...
    resource->setMAC(nx::network::MacAddress(info->macAddress));
...
bool HanwhaResourceSearcher::parseSunApiData(const QByteArray& data, SunApiData* outData)
{
    ...
    outData->macAddress = nx::network::MacAddress(QLatin1String(data.data() + kMacAddressOffset));
    ...
    QnResourceList* resultResourceList)
{
    ...
    auto resourceAlreadyFound =
        [](const QnResourceList* resultResourceList, const nx::network::MacAddress& macAddress)
        {
            return std::any_of(
                resultResourceList->cbegin(), resultResourceList->cend(),
                [&macAddress](const QnResourcePtr& resource)
                {
                    return nx::network::MacAddress(resource->getUniqueId()) == macAddress;
                });
        };
    ...
    const QByteArray& /*xmlDevInfo*/)
{
    ...
    nx::network::MacAddress cameraMac(devInfo.udn.split(L'-').last());
    if (cameraMac.isNull())
        cameraMac = nx::network::MacAddress(devInfo.serialNumber);
    if (cameraMac.isNull())
    {
