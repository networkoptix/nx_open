...
void HanwhaArchiveDelegate::setPlaybackMode(PlaybackMode mode)
{
    ...
    auto& rtspClient = m_streamReader->rtspClient();
    rtspClient.addRequestHeader(QnRtspClient::kPlayCommand, nx_http::HttpHeader("Require", "onvif-replay"));
    rtspClient.addRequestHeader(QnRtspClient::kPlayCommand, nx_http::HttpHeader("Immediate", "yes"));
    switch (mode)
    {
        case PlaybackMode::ThumbNails:
            rtspClient.addRequestHeader(QnRtspClient::kPlayCommand, nx_http::HttpHeader("Frames", "Intra"));
        ...
        case PlaybackMode::Export:
            rtspClient.addRequestHeader(QnRtspClient::kPlayCommand, nx_http::HttpHeader("Rate-Control", "no"));
