    ...
    const oasisWsnB2__NotificationMessageHolderType& notification, time_t minNotificationTime)
{
    ...
    {
        if (inputSourceNames.find(it->name.toLower()) != inputSourceNames.cend())
        {
            portSourceIter = it;
            break;
        }
    ...
    if (portSourceIter == handler.source.end()  //< source is not port
        || kStateStrings.find(handler.data.name.toLower()) == kStateStrings.cend())
    {
        return;
    }
        ...
        const QnAbstractStreamDataProvider* dataProvider) const
{
    ...
    if (layoutStr.isEmpty())
        videoLayout = QnMediaResource::getVideoLayout(dataProvider);
    else
        videoLayout = QnCustomResourceVideoLayout::fromString(layoutStr);
