    ...
    const QString& model)
{
    ...
    {
        NX_DEBUG(typeid(OnvifResourceInformationFetcher), lit("Discovery----:  ignore ONVIF camera %1 (%2-%3) because it supported by native driver").
            arg(host).
            arg(manufacturer).
            arg(model));
        return true;
    ...
    {
        NX_DEBUG(typeid(OnvifResourceInformationFetcher), lit("Discovery----:  ignore ONVIF camera %1 (%2-%3) because it is in the special blocking list").
            arg(host).
            arg(manufacturer).
            arg(model));
        return true;
