...
APPLICATION_NAME = 'Network Optix Media Proxy'
...
def gen_version_h():
    ...
    print >> version_h, '#define VER_ORIGINALFILENAME_STR    "mediaproxy.exe"'
...
index_dirs(('src',), 'src/const.pro', 'src/mediaproxy.pro', exclude_dirs=EXCLUDE_DIRS, exclude_files=EXCLUDE_FILES)
instantiate_pro('src/mediaproxy.pro', {'BUILDLIB' : BUILDLIB, 'FFMPEG' : ffmpeg_path, 'EVETOOLS_DIR' : tools_path})

if platform() == 'win32':
    os.system('qmake -tp vc -o src/mediaproxy.vcproj src/mediaproxy.pro')
elif platform() == 'mac':
    ...
    os.system('qmake -spec macx-g++ CONFIG-=release CONFIG+=debug -o build/Makefile.debug src/mediaproxy.pro')
    os.system('qmake -spec macx-g++ CONFIG-=debug CONFIG+=release -o build/Makefile.release src/mediaproxy.pro')
elif platform() == 'linux':
    if os.path.exists('src/Makefile.debug'):
        os.unlink('src/Makefile.debug')
    ...
    os.system('qmake -spec linux-g++ CONFIG-=release CONFIG+=debug -o build/Makefile.debug src/mediaproxy.pro')
    os.system('qmake -spec linux-g++ CONFIG-=debug CONFIG+=release -o build/Makefile.release src/mediaproxy.pro')
