    ...
    , const OperationCallback &failedCallback)
{
    ...
    const auto &successful = 
        [info, utcDateTime, timeZone, timestamp, successfulCallback, failedCallback](const QByteArray &data)
    {
        const QJsonObject &object = QJsonDocument::fromJson(data.data()).object();
        if (isErrorReply(object))
        {
        ...
        {
            successfulCallback(info.baseInfo().id, utcDateTime, timeZone, timestamp);
        }
    };
...
void rtu::interfacesListRequest(HttpClient *client
    ...
    , const FailedCallback &failed)
{
    const QUrl url = makeUrl(info.hostAddress
        , info.port, adminUserName(), password, kIfListCmdName);
