...
namespace ec2
{
    namespace db
    {
        ...
        int getResourceInternalId(const QSqlDatabase& database, const QnUuid& guid)
        {
            QSqlQuery query(database);
            query.setForwardOnly(true);
            if (!SqlQueryExecutionHelper::prepareSQLQuery(&query, "SELECT id from vms_resource where guid = ?", Q_FUNC_INFO))
                return 0;
            query.addBindValue(guid.toRfc4122());
            if (!SqlQueryExecutionHelper::execSQLQuery(&query, Q_FUNC_INFO) || !query.next())
                return 0;
        ...
        int getCurrentUserPermissions(const QSqlDatabase& database, int internalUserId)
        {
            QSqlQuery query(database);
            query.setForwardOnly(true);
            if (!SqlQueryExecutionHelper::prepareSQLQuery(&query, "SELECT rights from vms_userprofile where user_id = ?", Q_FUNC_INFO))
                return 0;
