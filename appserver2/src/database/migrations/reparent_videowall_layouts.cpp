...
bool reparentLayout(const QSqlDatabase& database, const QnUuid& layoutId, const QnUuid& videoWallId)
{
    ApiLayoutDataList layouts;
    if (!api::fetchLayouts(database, layoutId, layouts))
        return false;
...
bool reparentVideoWallLayouts(const QSqlDatabase& database)
{
    ...
    for (const auto& item: videoWallItems)
    {
        ...
        if (!reparentLayout(database, item.layoutId, item.videoWallId))
            return false;
    ...
    if (!getAnyVideoWall(database, anyVideoWallId))
        return false;
    ...
    if (!anyVideoWallId.isNull())
    {
        for (const QnUuid& id : unused)
        {
            ...
            if (!reparentLayout(database, id, anyVideoWallId))
                return false;
        }
    }
